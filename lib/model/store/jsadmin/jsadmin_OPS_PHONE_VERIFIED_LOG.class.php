...
function insertOPSPhoneReport($cid,$profileId,$phoneNum,$phoneType,$phoneStatus)
{
    ...
    public function getOPSUserProcessedCount($startDate,$endDate)
    {
        try
        {
            ...
            $sql  = "SELECT COUNT(DISTINCT OPS_USERID,PROFILEID,DATE(ENTRY_DT),PHONE_TYPE) AS CNT,OPS_USERID,DATE(ENTRY_DT) as DT,PHONE_STATUS FROM  jsadmin.OPS_PHONE_VERIFIED_LOG WHERE ENTRY_DT BETWEEN  :DATE_START AND :DATE_END AND (PHONE_STATUS='N' || PHONE_STATUS='B') AND PHONE_TYPE != 'A' GROUP BY OPS_USERID,DATE(ENTRY_DT),PHONE_STATUS";
            $prep = $this->db->prepare($sql);
            $prep->bindValue(":DATE_START", $startDate, PDO::PARAM_STR);
