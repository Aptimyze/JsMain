<?php
class incentive_SALES_CSV_DATA_TEMP extends TABLE
{
	...
	public function truncate()
	{
		try
                {
                        $sql="TRUNCATE TABLE incentive.SALES_CSV_DATA_TEMP";
	...
	public function insertProfile($profileid, $username, $entryDt)
        {
                try
                {
                        $sql = "INSERT IGNORE INTO incentive.SALES_CSV_DATA_TEMP (PROFILEID,USERNAME,ENTRY_DT) VALUES(:PROFILEID,:USERNAME,:ENTRY_DT)";
                        $prep = $this->db->prepare($sql);
        ...
        public function getProfiles()
        {
                try
                {
                        $sql ="SELECT PROFILEID FROM incentive.SALES_CSV_DATA_TEMP ORDER BY ENTRY_DT DESC";
                        $prep = $this->db->prepare($sql);
                        $prep->execute();
        ...
        public function getMaxDate()
        {
                try
                {
                        $sql ="SELECT MAX(ENTRY_DT) as MAX_DT FROM incentive.SALES_CSV_DATA_TEMP";
                        $prep = $this->db->prepare($sql);
                        $prep->execute();
        ...
        public function getAllProfilesCount()
        {
                try
                {
                        $sql ="SELECT count(*) as CNT FROM incentive.SALES_CSV_DATA_TEMP";
                        $prep = $this->db->prepare($sql);
                        $prep->execute();
                        $result=$prep->fetch(PDO::FETCH_ASSOC);
                        $cnt = $result['CNT'];
        ...
        public function getLatestProfilesCount($max_dt)
        {
                try
                {
                        $sql ="SELECT count(*) as CNT FROM incentive.SALES_CSV_DATA_TEMP WHERE ENTRY_DT=:MAX_DT";
                        $prep = $this->db->prepare($sql);
        ...
        public function getScore($profileid)
        {
                try
                {	return;
                        $sql ="SELECT ANALYTIC_SCORE FROM incentive.SALES_CSV_DATA_TEMP WHERE PROFILEID=:PROFILEID";
                        $prep = $this->db->prepare($sql);
			$prep->bindValue(":PROFILEID",$profileid,PDO::PARAM_INT);
                        $prep->execute();
                        $result=$prep->fetch(PDO::FETCH_ASSOC);
	...
	// Dilaer filter
	public function removeInDialerProfiles()
	{
		try
		{
			$sql ="delete incentive.SALES_CSV_DATA_TEMP.* from incentive.SALES_CSV_DATA_TEMP , incentive.IN_DIALER d where incentive.SALES_CSV_DATA_TEMP.PROFILEID=d.PROFILEID";
			$prep = $this->db->prepare($sql);
			$prep->execute();
		}
		catch(Exception $e)
	...
	// Negative treatment filter
        ...
        public function removeProfiles($profileArr)
        {
                try
                {
			...
			if(!$profileStr)
				throw new jsException("","no profiles passed");			
                        $sql ="delete from incentive.SALES_CSV_DATA_TEMP WHERE PROFILEID IN($profileStr)";
                        $prep = $this->db->prepare($sql);
                        $prep->execute();
                }
                catch(Exception $e)
