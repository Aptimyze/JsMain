...
class matchalerts_LowDppMatchalertsCheck extends TABLE
{
	public function __construct($dbname="")
	{
         ...
         */
        public function getProfilesWithInformLimitReached($date,$informTimes){
            try{
                $sql="SELECT PROFILEID,COUNT(*) AS NUMBER FROM matchalerts.LOW_DPP_MATCHALERTS_CHECK WHERE DATE>=:DATE GROUP BY PROFILEID HAVING NUMBER>=:INFORM_TIMES";
        ...
        public function getProfilesWithZeroMatches($totalScript,$currentScript,$date){
            try{
                $sql="SELECT DISTINCT(PROFILEID) FROM matchalerts.LOW_DPP_MATCHALERTS_CHECK WHERE PROFILEID%:TOTAL_SCRIPT=:CURRENT_SCRIPT AND DATE>:DATE";
                $prep = $this->db->prepare($sql);
