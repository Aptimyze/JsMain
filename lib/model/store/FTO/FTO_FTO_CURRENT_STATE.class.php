...
//                        $fields = $fields?$fields:$this->getFields();//Get columns to query
	...
	*/
	public function getLoginWeeklyData($start_dt,$end_dt)
	{
		...
		try
		{
			$sql = "SELECT COUNT(*) AS C,j.GENDER AS GENDER,j.MTONGUE AS MTONGUE,f.STATE_ID AS STATE_ID FROM newjs.JPROFILE j LEFT JOIN FTO.FTO_CURRENT_STATE f ON j.PROFILEID = f.PROFILEID WHERE j.LAST_LOGIN_DT BETWEEN :START_DATE AND :END_DATE GROUP BY j.GENDER,j.MTONGUE,f.STATE_ID";
			$res = $this->db->prepare($sql);
