...
class MOBILE_API_NOTIFICATION_MESSAGE_LOG extends TABLE
{
    ...
    {
        $sql = "SELECT * FROM $this->databaseName.NOTIFICATION_MESSAGE_LOG WHERE NOTIFICATION_KEY=:NOTIFICATION_KEY ORDER BY ENTRY_DT DESC LIMIT 1";
    ...
    public function deleteRecordDateWise($sdate,$edate)
        {
                try{
                        $sql = "delete FROM $this->databaseName.NOTIFICATION_MESSAGE_LOG WHERE ENTRY_DT>:ST_DATE AND ENTRY_DT<:END_DATE";
                        $res = $this->db->prepare($sql);
