...
class MOBILE_API_LOCAL_NOTIFICATION_LOG extends TABLE{
	...
	public function insert($profileid='',$key='',$messageId='',$sent='',$nextPollTime='', $osType='')
	{
        ...
        {
            try{
                $sql = "SELECT count(PROFILEID) count, NOTIFICATION_KEY, SENT, OS_TYPE FROM $this->databaseName.`LOCAL_NOTIFICATION_LOG` WHERE ENTRY_DATE>=:START_DATE AND ENTRY_DATE<=:END_DATE GROUP BY NOTIFICATION_KEY,SENT,OS_TYPE";
                $res = $this->db->prepare($sql);
