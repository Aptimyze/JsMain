...
class MOBILE_API_LOCAL_NOTIFICATION_LOG extends TABLE{
	...
	public function insert($profileid='',$key='',$messageId='',$sent='',$nextPollTime='', $osType='')
	{
		...
		if(!$deviceModel)
			$deviceModel='';
                $sqlInsert = "INSERT INTO $this->databaseName.LOCAL_LOG (`REGISTRATION_ID`,`APP_V`,`OS_V`,`ENTRY_DATE`,`PROFILEID`,`DEVICE_BRAND`,`DEVICE_MODEL`) VALUES (:REGISTRATION_ID,:APP_V,:OS_V,now(),:PROFILEID,:DEVICE_BRAND,:DEVICE_MODEL)";
                $resInsert = $this->db->prepare($sqlInsert);
        ...
        public function updateSentPrev($pid,$notificationKey,$status)
        {
                ...
                try{
                        $sql = "UPDATE $this->databaseName.LOCAL_NOTIFICATION_LOG SET SENT =:SENT,`UPDATE_DATE`=now() WHERE NOTIFICATION_KEY=:NOTIFICATION_KEY AND PROFILEID =:PROFILEID AND SENT!='L' ORDER BY ENTRY_DATE DESC LIMIT 1";
        ...
        public function updateSent($messageId,$status,$osType)
        {
                if(!$messageId || !$status || !$osType)
                     throw new jsException("","pid/notificationKey/status not provided to update status in $this->databaseName.LOCAL_NOTIFICATION_LOG");
                try{
                        $sql = "UPDATE $this->databaseName.LOCAL_NOTIFICATION_LOG SET SENT =:SENT,`UPDATE_DATE`=now() WHERE MESSAGE_ID=:MESSAGE_ID AND OS_TYPE=:OS_TYPE AND SENT!='L'";
                        $res=$this->db->prepare($sql);
        ...
        public function deleteNotification($messageId,$osType)
        {
                if(!$messageId || !$osType)
                     throw new jsException("","pid/notificationKey/status not provided to update status in $this->databaseName.LOCAL_NOTIFICATION_LOG");
                try{
                        $sql = "DELETE FROM $this->databaseName.LOCAL_NOTIFICATION_LOG WHERE MESSAGE_ID=:MESSAGE_ID AND OS_TYPE=:OS_TYPE AND SENT!='L'";
                        $res=$this->db->prepare($sql);
                        $res->bindValue(":MESSAGE_ID",$messageId,constant('PDO::PARAM_'.$this->{'MESSAGE_ID_BIND_TYPE'}));
                        $res->bindValue(":OS_TYPE",$osType,constant('PDO::PARAM_'.$this->{'OS_TYPE_BIND_TYPE'}));
                        $res->execute();
        ...
        {
            try{
                $sql = "SELECT count(distinct PROFILEID) count, NOTIFICATION_KEY, SENT, OS_TYPE FROM $this->databaseName.`LOCAL_NOTIFICATION_LOG` WHERE ENTRY_DATE>=:START_DATE AND ENTRY_DATE<=:END_DATE GROUP BY NOTIFICATION_KEY,SENT,OS_TYPE";
                $res = $this->db->prepare($sql);
	...
	public function deleteRecordDateWise($sdate,$edate)
        {
                try{
                        $sql = "delete FROM $this->databaseName.LOCAL_NOTIFICATION_LOG WHERE ENTRY_DATE>:ST_DATE AND ENTRY_DATE<:END_DATE";
                        $res = $this->db->prepare($sql);
