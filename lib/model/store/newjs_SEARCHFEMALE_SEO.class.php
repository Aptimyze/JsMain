<?php
class NEWJS_SEARCHFEMALE_SEO extends TABLE
{
        ...
        public function getTopProfiles($fieldQuery,$limit)
        {
			...
			try 
			{
				...
				$sql_2="SELECT SQL_CACHE S.PROFILEID FROM newjs.SEARCH_FEMALE AS S WHERE S.HAVEPHOTO='Y' AND S.PHOTO_DISPLAY='A' AND S.PRIVACY IN('','A') AND $fieldQuery ORDER BY S.NTIMES DESC LIMIT :limit";
		...
		public function getNoCountProfiles($fieldQuery,$limit, $notInProfileStr)
        {
			try 
			{
				$sql_2="SELECT SQL_CACHE S.PROFILEID FROM newjs.SEARCH_FEMALE AS S WHERE S.PROFILEID NOT IN ('$notInProfileStr') AND S.PRIVACY IN('','A') AND $fieldQuery ORDER BY S.NTIMES DESC LIMIT :limit";
				$prep=$this->db->prepare($sql_2);
				//$prep->bindValue(":tableName",$tableName,PDO::PARAM_STR);
				//$prep->bindValue("$fieldQuery",$fieldQuery,PDO::PARAM_STR);
				//$prep->bindValue("'$notInProfileStr'",'$notInProfileStr',PDO::PARAM_STR);
