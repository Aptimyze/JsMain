<?php
/*
 * This Class provide functions for PROFILE_VERIFICATION.DOCUMENTS_SCREENING table
 * @author Lavesh Rawat
 * @created March 12, 2014
*/
class PROFILE_VERIFICATION_DOCUMENTS_SCREENING extends TABLE
{
        ...
        **/
        public function insertDocuments($pid,$user)
        {
			...
			$time = date("Y-m-d H:i:s");
                        $sql="REPLACE INTO PROFILE_VERIFICATION.DOCUMENTS_SCREENING(PROFILEID, ALLOTED_TIME, ASSIGNED_TO) VALUES (:PID,:TIME,:USER)";
	...
	* @return array containing alloted alloted user info
        **/
        public function userAllottedProfiles($time,$case,$user='')
        {
                try
                {
                	$sql="SELECT PROFILEID,ALLOTED_TIME,ASSIGNED_TO FROM PROFILE_VERIFICATION.DOCUMENTS_SCREENING WHERE 1 ";
			if($user)
	...
	* This function is used to delete records.
        ...
        */
        public function del($pid,$user="")
        {
                try
                {
                        $sql = "DELETE FROM PROFILE_VERIFICATION.DOCUMENTS_SCREENING WHERE PROFILEID=:PROFILEID";
			if($user)
