...
class PROFILE_VERIFICATION_AADHAR_VERIFICATION extends TABLE
{
    ...
    public function insertAadharDetails($profileId,$username,$date,$aadharId,$requestId)
    {
    	try
    	{
    		$sql = "INSERT IGNORE INTO PROFILE_VERIFICATION.AADHAR_VERIFICATION(PROFILEID,USERNAME,DATE,AADHAR,REQUEST_ID) VALUES (:PROFILEID,:USERNAME,:DATE,:AADHARID,:REQUESTID) ON DUPLICATE KEY UPDATE AADHAR=:AADHARID,REQUEST_ID=:REQUESTID,DATE=:DATE";
    		...
    		$res->bindParam(":AADHARID", $aadharId, PDO::PARAM_STR);
    ...
    public function getAadharDetails($profileId)
    {
    	try
    	{
    		$sql = "SELECT PROFILEID,AADHAR,AADHAR_NO,REQUEST_ID,VERIFY_STATUS from PROFILE_VERIFICATION.AADHAR_VERIFICATION WHERE PROFILEID = :PROFILEID";
    		$res = $this->db->prepare($sql);
    		$res->bindParam(":PROFILEID", $profileId, PDO::PARAM_INT);
    		$res->execute();
    		while($row=$res->fetch(PDO::FETCH_ASSOC))
    		{
    			...
    			$output[$profileId]["AADHAR"] =$row['AADHAR'];
    ...
    public function checkIfAadharVerified($aadharId,$verifyValue,$aadhar='')
    {
