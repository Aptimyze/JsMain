        ...
        try
        {
            ...
            if(is_array($params["SERVICE"]) && $params["SERVICE"])
            {
                if(count($params["SERVICE"])==1)
                    $valuesStr = $valuesStr."(:PROFILEID0,:SERVICE0,:DISC30,:DISC60,:DISC120,:DISCL0)";
                else
                {
                    foreach ($params["SERVICE"] as $key => $service) {
                        $valuesStr = $valuesStr."(:PROFILEID".$key.",:SERVICE".$key.",:DISC3".$key.",:DISC6".$key.",:DISC12".$key.",:DISCL".$key."),"; 
                    }
