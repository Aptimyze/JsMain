...
class ASSISTED_PRODUCT_AP_PROFILE_INFO extends TABLE
{
	...
	*/
	...
	@return an array of profileid 
	*/
    public function getAPProfilesResumed($whereLastLogin = '')
    {
      try {
        if($whereLastLogin)
            $sql = "SELECT I.PROFILEID,J.LAST_LOGIN_DT FROM Assisted_Product.AP_PROFILE_INFO AS I LEFT JOIN Assisted_Product.AP_PROFILE_INFO_LOG AS L ON I.PROFILEID = L.PROFILEID LEFT JOIN newjs.JPROFILE AS J ON I.PROFILEID = J.PROFILEID  WHERE I.STATUS='LIVE' AND I.SEND='Y' AND L.PROFILEID IS NULL AND J.ACTIVATED='Y' and J.activatedkey=1 and DATE(J.LAST_LOGIN_DT) > :LOGIN_DT order by J.LAST_LOGIN_DT DESC";
        else
            $sql = "SELECT I.PROFILEID,J.LAST_LOGIN_DT FROM Assisted_Product.AP_PROFILE_INFO AS I LEFT JOIN Assisted_Product.AP_PROFILE_INFO_LOG AS L ON I.PROFILEID = L.PROFILEID LEFT JOIN newjs.JPROFILE AS J ON I.PROFILEID = J.PROFILEID  WHERE I.STATUS='LIVE' AND I.SEND='Y' AND L.PROFILEID IS NULL AND J.ACTIVATED='Y' and J.activatedkey=1 order by J.LAST_LOGIN_DT DESC";
        $res = $this->db->prepare($sql);
        if($whereLastLogin)
