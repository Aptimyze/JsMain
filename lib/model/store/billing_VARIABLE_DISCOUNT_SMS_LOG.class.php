<?php
class billing_VARIABLE_DISCOUNT_SMS_LOG extends TABLE{
	...
	public function getFrequencyAndTimes($entry_dt)
	{
		try
		{
			$sql="SELECT FREQUENCY, NO_OF_TIMES FROM billing.VARIABLE_DISCOUNT_SMS_LOG WHERE ENTRY_DT=:ENTRY_DT AND STATUS='Y'";
			$prep = $this->db->prepare($sql);
			$prep->bindValue(":ENTRY_DT",$entry_dt,PDO::PARAM_STR);
			$prep->execute();
	...
	public function updateStartTime($entry_dt){
		try
		{	
			$sql="UPDATE billing.VARIABLE_DISCOUNT_SMS_LOG SET START_TIME=CURRENT_TIMESTAMP() WHERE ENTRY_DT=:ENTRY_DT AND STATUS='Y'";	
			$prep = $this->db->prepare($sql);
			$prep->bindValue(":ENTRY_DT",$entry_dt,PDO::PARAM_STR);
			$prep->execute();
		}
	...
	public function updateUptoCount($entry_dt, $upto_count){
		try
		{	
			$sql="UPDATE billing.VARIABLE_DISCOUNT_SMS_LOG SET UPTO_COUNT=:UPTO_COUNT WHERE ENTRY_DT=:ENTRY_DT AND STATUS='Y'";	
			$prep = $this->db->prepare($sql);
			$prep->bindValue(":ENTRY_DT",$entry_dt,PDO::PARAM_STR);
	...
	public function updateFlatCount($entry_dt, $flat_count){
		try
		{	
			$sql="UPDATE billing.VARIABLE_DISCOUNT_SMS_LOG SET FLAT_COUNT=:FLAT_COUNT WHERE ENTRY_DT=:ENTRY_DT AND STATUS='Y'";	
			$prep = $this->db->prepare($sql);
			$prep->bindValue(":ENTRY_DT",$entry_dt,PDO::PARAM_STR);
	...
	public function insertVdSmsSchedule($entry_dt_arr, $no_of_times){
		try
		{
			foreach($entry_dt_arr as $k => $entry_dt) {
				$sql="INSERT INTO billing.`VARIABLE_DISCOUNT_SMS_LOG` VALUES ('', 'Y', :ENTRY_DT, '0000-00-00 00:00:00', '0000-00-00 00:00:00', '0', '0', ".$k.", :NO_OF_TIMES)";	
				$prep = $this->db->prepare($sql);
				$prep->bindValue(":ENTRY_DT",$entry_dt,PDO::PARAM_STR);
	...
	public function isStatusY(){
		try
		{
			$sql="SELECT ID FROM billing.`VARIABLE_DISCOUNT_SMS_LOG` WHERE STATUS='Y'";
			$prep = $this->db->prepare($sql);
			$prep->execute();
