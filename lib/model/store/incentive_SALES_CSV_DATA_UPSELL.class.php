<?php
class incentive_SALES_CSV_DATA_UPSELL extends TABLE
{
	...
	public function insertProfile($profileid,$dialerPriority,$score,$dialerDialStatus,$allotedTo,$vdDiscount,$lastLoginDt,$phone1,$phone2,$havePhoto,$birthDt,$mstatus,$everPaid,$gender,$relation,$leadId,$csvDate,$username,$serviceSelected,$fpEntryDt,$discount,$onlineStatus,$netAmount,$country)
	{
		try
		{
			$sql= "INSERT IGNORE INTO incentive.SALES_CSV_DATA_UPSELL(PROFILEID,PRIORITY,ANALYTIC_SCORE,OLD_PRIORITY,DIAL_STATUS,AGENT,VD_PERCENT,LAST_LOGIN_DATE,PHONE_NO1,PHONE_NO2,PHOTO,DOB,MSTATUS,EVER_PAID,GENDER,POSTEDBY,PHONE_NO3,PHONE_NO4,USERNAME,SERVICE_SELECTED,PAYMENT_ENTRY_DT,DISCOUNT,ONLINE_STATUS,AMOUNT,COUNTRY,LEAD_ID,CSV_ENTRY_DATE) VALUES(:PROFILEID,:PRIORITY,:ANALYTIC_SCORE,:OLD_PRIORITY,:DIAL_STATUS,:AGENT,:VD_PERCENT,:LAST_LOGIN_DATE,:PHONE_NO1,:PHONE_NO2,:PHOTO,:DOB,:MSTATUS,:EVER_PAID,:GENDER,:POSTEDBY,:PHONE_NO3,:PHONE_NO4,:USERNAME,:SERVICE_SELECTED,:FP_ENTRY_DT,:DISCOUNT,:ONLINE_STATUS,:LAST_AMOUNT_TRIED,:COUNTRY,:LEAD_ID,:CSV_ENTRY_DATE)";
        ...
        public function getData($dateSet)
        {
			...
			$dateTime1	=$dateArr[0];
			$dateTime2	=$dateArr[1];
                        $sql="SELECT * FROM incentive.SALES_CSV_DATA_UPSELL WHERE CSV_ENTRY_DATE>=:CSV_ENTRY_DATE1 AND CSV_ENTRY_DATE<:CSV_ENTRY_DATE2 AND DIAL_STATUS>0";
                        $prep=$this->db->prepare($sql);
