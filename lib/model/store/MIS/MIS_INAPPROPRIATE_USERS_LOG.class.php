...
class MIS_INAPPROPRIATE_USERS_LOG extends TABLE
{
  ...
  {
    try {  
      $sql= "SELECT USERNAME,SUM(RELIGION_COUNT) AS RCOUNT,SUM(AGE_COUNT) AS ACOUNT,SUM(MSTATUS_COUNT) AS MCOUNT,SUM(TOTAL_SCORE) AS TCOUNT, `DATE` FROM MIS.INAPPROPRIATE_USERS_LOG WHERE PROFILEID % :DIVISOR = :REMAINDER AND DATE <= :EDATE AND DATE>=:SDATE GROUP BY USERNAME, `DATE`";
