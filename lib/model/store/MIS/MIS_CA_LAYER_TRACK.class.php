...
class MIS_CA_LAYER_TRACK extends TABLE
{
  ...
  public function removeLimitedCAL($limitedCALArray,$limitedCALArrayDuration)
  {
    if(!is_array($limitedCALArray) || !$limitedCALArray)
                        throw new jsException("","Either no CAL is selected or Duration is not specified");


    try {   

          $date = date("Y-m-d H:i:s", strtotime('-'.$limitedCALArrayDuration.' months'));
          $records = count($limitedCALArray);
                 $layerArray = "(";
          for($i=0; $i<$records; $i++){
                $val = $limitedCALArray[$i]; 
                $layerArray = $layerArray.$val;
                if($i != $records-1) 
      ...
      $sql= "DELETE FROM MIS.CA_LAYER_TRACK WHERE `ENTRY_DT` < :DT and `  
LAYERID` in $layerArray";
      $prep=$this->db->prepare($sql);
      $prep->bindValue(":DT",$date,PDO::PARAM_STR);
      $prep->execute();
  ...
  public function removeUnlimitedCAL($unlimitedCALArray,$unlimitedCALArrayDuration)
  {

   if(!is_array($unlimitedCALArray) || !$unlimitedCALArray)
                        throw new jsException("","Either no CAL is selected or Duration is not specified");

    try {   
  $date = date("Y-m-d H:i:s", strtotime('-'.$unlimitedCALArrayDuration.' months'));

      $records = count($unlimitedCALArray); 

                 $layerArray = "(";
          for($i=0; $i<$records; $i++){
                $val = $unlimitedCALArray[$i]; 
                $layerArray = $layerArray.$val;
                if($i != $records-1) 
                $layerArray = $layerArray.",";
                } 
