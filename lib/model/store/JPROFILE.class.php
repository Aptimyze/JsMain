    ...
    public function getRegistrationMisGroupedData($fromDate,$toDate,$month='',$groupType)
    {
    	try
    	{
    		if($groupType != "")
    		{
    			if($month == "")
    			{
    				$sql = "SELECT COUNT(*) AS COUNT,$groupType,EXTRACT(MONTH FROM ENTRY_DT) AS MONTH FROM newjs.JPROFILE J LEFT JOIN newjs.JPROFILE_CONTACT C ON J.PROFILEID=C.PROFILEID WHERE ACTIVATED IN ('Y','H') AND activatedKey ='1' AND ENTRY_DT BETWEEN :FROMDATE AND :TODATE AND (J.MOB_STATUS='Y' OR J.LANDL_STATUS='Y' OR C.ALT_MOB_STATUS='Y') GROUP BY $groupType,MONTH";
    			}
    			else
    			{
    				$sql = "SELECT COUNT(*) AS COUNT, $groupType,EXTRACT(DAY FROM ENTRY_DT) AS DAY FROM newjs.JPROFILE J LEFT JOIN newjs.JPROFILE_CONTACT C ON J.PROFILEID=C.PROFILEID WHERE MOB_STATUS ='Y' AND ACTIVATED IN ('Y','H') AND activatedKey ='1' AND ENTRY_DT BETWEEN :FROMDATE AND :TODATE AND (J.MOB_STATUS='Y' OR J.LANDL_STATUS='Y' OR C.ALT_MOB_STATUS='Y') GROUP BY $groupType,DAY";
    			}
    		}
    		else
    		{
