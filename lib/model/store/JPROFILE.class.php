    ...
    public function getRegistrationMisGroupedData($fromDate,$toDate,$month='',$groupType)
    {
    	try
    	{
    		if($groupType != "")
    		{
    			if($month == "")
    			{
    				$sql = "SELECT COUNT(*) AS COUNT,$groupType,EXTRACT(MONTH FROM ENTRY_DT) AS MONTH FROM newjs.JPROFILE WHERE ENTRY_DT BETWEEN :FROMDATE AND :TODATE AND DATEDIFF(ENTRY_DT,VERIFY_ACTIVATED_DT)<'3' GROUP BY $groupType,MONTH";
    			}
    			else
    			{
    				$sql = "SELECT COUNT(*) AS COUNT, $groupType,EXTRACT(DAY FROM ENTRY_DT) AS DAY FROM newjs.JPROFILE  WHERE  ENTRY_DT BETWEEN :FROMDATE AND :TODATE AND DATEDIFF(ENTRY_DT,VERIFY_ACTIVATED_DT)<'3' GROUP BY $groupType,DAY";
    			}
    		}
    		else
    		{
