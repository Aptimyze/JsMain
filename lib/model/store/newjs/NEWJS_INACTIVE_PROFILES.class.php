...
class NEWJS_INACTIVE_PROFILES extends TABLE
{
	...
	**/
        ...
        public function UpdateStatusIncomplete($profileIdMailSent)
        {
                try
                {
                	//echo "string";
        ...
        public function SelectProfilesInactivated($totalScript,$currentScript)
	        {
	                try
	                {
							$sql = "SELECT PROFILEID,TIME_INTERVAL FROM newjs.INACTIVE_PROFILES WHERE STATUS = 'N' AND PROFILEID%:totalScript=:currentScript";
							$res = $this->db->prepare($sql);
