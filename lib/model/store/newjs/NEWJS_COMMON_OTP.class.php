<?php
class NEWJS_COMMON_OTP extends TABLE 
{
    ...
    {  
		...
		try{
			$sql = "SELECT * FROM newjs.COMMON_OTP WHERE PHONE_WITH_ISD = :PHONE AND PROFILEID=:PROFILEID AND TYPE=:TYPEOTP ORDER BY DATE DESC LIMIT 1";
	...
	public function setNewOTPRecord($profileid,$phone,$otp,$otpSettings)
    {
		...
		try{
			...
			$sql = "INSERT INTO newjs.COMMON_OTP(PROFILEID,PHONE_WITH_ISD,OTP,DATE,TYPE) VALUES (:PROFILEID,:PHONE,:OTP,:DATE,:TYPEOTP);";
			$pdoStatement = $this->db->prepare($sql);
			$pdoStatement->bindValue(":PHONE",$phone,PDO::PARAM_STR);
			$pdoStatement->bindValue(":PROFILEID",$profileid,PDO::PARAM_INT);
    ...
    public function incrementSmsCount($id)
    {
		...
		try{
			$sql = "UPDATE newjs.COMMON_OTP SET SMS_COUNT=SMS_COUNT+1 WHERE `ID`=:ID";
			$pdoStatement = $this->db->prepare($sql);
	 ...
	 public function incrementTrialCount($id)
    {
		...
		try{
			$sql = "UPDATE newjs.COMMON_OTP SET TRIAL_COUNT=TRIAL_COUNT+1 WHERE `ID`=:ID";
			$pdoStatement = $this->db->prepare($sql);
			$pdoStatement->bindValue(":ID",$id,PDO::PARAM_INT);
			$pdoStatement->execute();
    ...
    { 
		...
		try{
			...
			$sql = "UPDATE newjs.COMMON_OTP SET TRIAL_COUNT=0,SMS_COUNT=0,OTP=:OTP,DATE='$now',TYPE=:TYPEOTP WHERE `ID`=:ID";
			$pdoStatement = $this->db->prepare($sql);
			$pdoStatement->bindValue(":ID",$id,PDO::PARAM_INT);
			$pdoStatement->bindValue(":OTP",$otp,PDO::PARAM_STR);
    ...
    {
		if(!$id)
		throw new jsException("","ID is empty in arguements in function incrementSmsCount() of NEWJS_COMMON_OTP class");
			
		try{
			$sql = "DELETE FROM newjs.COMMON_OTP  WHERE `ID`=:ID";
			$pdoStatement = $this->db->prepare($sql);			
			$pdoStatement->bindValue(":ID",$id,PDO::PARAM_INT);
			$pdoStatement->execute();
