		...
		public function getMessageListing($condition,$skipArray)
		{
			try{
				...
				else
				{
					...
					$sql = "SELECT SQL_CACHE SENDER AS PROFILEID, MESSAGE,  'R' AS SR,SEEN,DATE FROM  `MESSAGE_LOG` USE INDEX (RECEIVER) JOIN MESSAGES ON ( MESSAGE_LOG.ID = MESSAGES.ID ) WHERE  `RECEIVER` =:PROFILEID".$sender." AND  `TYPE` IN ('R','I') AND  `IS_MSG` ='Y' UNION ALL SELECT  RECEIVER AS PROFILEID, MESSAGE,  'S' AS SR,SEEN,DATE FROM  `MESSAGE_LOG` USE INDEX (SENDER) JOIN MESSAGES ON ( MESSAGE_LOG.ID = MESSAGES.ID ) WHERE  `SENDER` =:PROFILEID ".$receiver." AND  `TYPE` IN ('R','I') AND  `IS_MSG` ='Y' ORDER BY DATE DESC";
					$res=$this->db->prepare($sql);
