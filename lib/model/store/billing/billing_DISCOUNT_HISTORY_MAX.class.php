...
class billing_DISCOUNT_HISTORY_MAX extends TABLE{
    ...
    public function updateDiscountHistoryMax($paramsArr){
        if(is_array($paramsArr)){
            try{
                $sql = "INSERT INTO billing.DISCOUNT_HISTORY_MAX (PROFILEID, LAST_LOGIN_DATE, MAX_DISCOUNT_DATE, MAX_DISCOUNT) VALUES (:PROFILEID, :LAST_LOGIN_DATE_NEW, :MAX_DISCOUNT_DATE_NEW, :MAX_DISCOUNT_NEW) ON DUPLICATE KEY UPDATE LAST_LOGIN_DATE=:LAST_LOGIN_DATE_NEW, MAX_DISCOUNT=CASE WHEN MAX_DISCOUNT<:MAX_DISCOUNT_NEW THEN :MAX_DISCOUNT_NEW ELSE MAX_DISCOUNT END,MAX_DISCOUNT_DATE=CASE WHEN MAX_DISCOUNT<:MAX_DISCOUNT_NEW THEN :MAX_DISCOUNT_DATE_NEW ELSE MAX_DISCOUNT_DATE END";
                ...
                $prep->bindValue(":MAX_DISCOUNT_DATE_NEW",date('Y-m-d H:i:s'),PDO::PARAM_STR);
