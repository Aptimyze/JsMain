  ...
  public function getDetailsForProposalMail(){
  	try{
        $tomorrowDT = date("Y-m-d",strtotime(' +1 day'));
  		...
  		$sql = "SELECT AGENT_USERNAME,CLIENT_ID,MEMBER_ID,STATUS FROM billing.EXCLUSIVE_FOLLOWUPS WHERE (STATUS LIKE 'F1' AND FOLLOWUP2_DT = :CURRENT_DT) OR (STATUS LIKE 'F2' AND FOLLOWUP3_DT = :CURRENT_DT) OR (STATUS LIKE 'F3' AND FOLLOWUP4_DT = :CURRENT_DT)";
