...
class MOBILE_API_NOTIFICATION_LOG extends TABLE{
	...
	public function getBindString($value,$param,$count,$relation)
	{
        ...
        {
                try{
                        $sql = "SELECT count(PROFILEID) count, NOTIFICATION_KEY, SENT,OS_TYPE FROM MOBILE_API.`NOTIFICATION_LOG` WHERE SEND_DATE>=:START_DATE AND SEND_DATE<=:END_DATE GROUP BY NOTIFICATION_KEY,SENT,OS_TYPE";
                        $res = $this->db->prepare($sql);
        ...
        {
                try{
                        $sql = "insert into test.TEMP_NOTIFICATION_LOG SELECT PROFILEID,NOTIFICATION_KEY from MOBILE_API.NOTIFICATION_LOG WHERE SEND_DATE>=:START_DATE AND SEND_DATE<=:END_DATE";
                        $res = $this->db->prepare($sql);
                        $res->bindValue(":START_DATE",$startDate, PDO::PARAM_STR);
                        $res->bindValue(":END_DATE",$endDate, PDO::PARAM_STR);
                        $res->execute();
