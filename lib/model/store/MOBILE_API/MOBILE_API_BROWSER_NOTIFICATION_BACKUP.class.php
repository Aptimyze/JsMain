    ...
    */
    public function insertBackupNotification($paramsArr){
        try{
            ...
            {
                $sql = "INSERT IGNORE INTO MOBILE_API.BROWSER_NOTIFICATION_BACKUP(ID,REG_ID, NOTIFICATION_KEY, NOTIFICATION_TYPE, MESSAGE, TITLE, ICON, TAG, LANDING_ID, ENTRY_DT,RECEIVED_DATE,PRIORITY,MSG_ID,SENT_TO_QUEUE,SENT_TO_FCM,RECEIVED_STATUS,TTL,RESPONSE,STATUS) VALUES ";
                foreach ($paramsArr as $key => $val) 
                {
                    $insertValues = $insertValues."(:ID$key,:REG_ID$key, :NOTIFICATION_KEY$key, :NOTIFICATION_TYPE$key, :MESSAGE$key, :TITLE$key, :ICON$key, :TAG$key, :LANDING_ID$key, :ENTRY_DT$key, :RECEIVED_DATE$key,:PRIORITY$key,:MSG_ID$key, :SENT_TO_QUEUE$key,:SENT_TO_FCM$key,:RECEIVED_STATUS$key,:TTL$key,:RESPONSE$key,:STATUS$key),";
                }
                ...
                foreach ($paramsArr as $key => $value) 
                {
                  ...
                  $prep->bindValue(":ID$key",$value["ID"],PDO::PARAM_INT);
                    ...
                    $prep->bindValue(":RECEIVED_STATUS$key",$value["RECEIVED_STATUS"],PDO::PARAM_STR);
        ...
        {
                try{
                        $sql ="SELECT count(b.REG_ID) count, b.NOTIFICATION_KEY, b.SENT_TO_QUEUE, b.RECEIVED_STATUS,b_reg.CHANNEL FROM MOBILE_API.`BROWSER_NOTIFICATION_BACKUP` b, MOBILE_API.BROWSER_NOTIFICATION_REGISTRATION b_reg WHERE b.REG_ID=b_reg.REG_ID AND b.ENTRY_DT>=:START_DATE AND b.ENTRY_DT<=:END_DATE GROUP BY b.NOTIFICATION_KEY, b.SENT_TO_QUEUE, b.RECEIVED_STATUS,b_reg.CHANNEL";
