...
class MOBILE_API_DAILY_NOTIFICATION_COUNT_LOG extends TABLE{
	...
	public function insertData($notificationKey,$totalCount, $gcmPush,$gcmAccepted,$pushReceived,$localApiHit,$localDelivered,$localReceived,$active7Days, $active1Days, $totalIosPushed, $totalIosReceived, $entryDate)
	{
		try{
			$sqlInsert ="INSERT INTO MOBILE_API.DAILY_NOTIFICATION_COUNT_LOG(`NOTIFICATION_KEY`,`TOTAL_COUNT`,`PUSHED_TO_GCM`,`ACCEPTED_BY_GCM`,`PUSH_ACKNOWLEDGED`,`LOCAL_API_HIT_BY_DEVICE`,`LOCAL_SENT_TO_DEVICE`,`LOCAL_ACKNOWLEDGED`,`ACTIVE_LOGIN_7DAY`,`ACTIVE_LOGIN_1DAY`,`PUSHED_TO_IOS`,`ACCEPTED_BY_IOS`,`ENTRY_DT`) VALUES (:NOTIFICATION_KEY,:TOTAL_COUNT,:PUSHED_TO_GCM,:ACCEPTED_BY_GCM,:PUSH_ACKNOWLEDGED,:LOCAL_API_HIT_BY_DEVICE,:LOCAL_SENT_TO_DEVICE,:LOCAL_ACKNOWLEDGED,:ACTIVE_LOGIN_7DAY,:ACTIVE_LOGIN_1DAY,:PUSHED_TO_IOS,:ACCEPTED_BY_IOS,:ENTRY_DT)";
        ...
        public function getData($startDate, $endDate, $notificationKey='', $scheduledNotificaionStr='')
        {
		try{
			if($notificationKey)
	     	           	$sql ="SELECT TOTAL_COUNT,PUSHED_TO_GCM,ACCEPTED_BY_GCM,PUSHED_TO_IOS,ACCEPTED_BY_IOS,PUSH_ACKNOWLEDGED,LOCAL_API_HIT_BY_DEVICE,LOCAL_SENT_TO_DEVICE,LOCAL_ACKNOWLEDGED,ACTIVE_LOGIN_7DAY,ACTIVE_LOGIN_1DAY,DAY(ENTRY_DT) DAY FROM MOBILE_API.DAILY_NOTIFICATION_COUNT_LOG WHERE NOTIFICATION_KEY=:NOTIFICATION_KEY AND ENTRY_DT>=:START_DATE AND ENTRY_DT<=:END_DATE GROUP BY DAY(ENTRY_DT)";
			else if($scheduledNotificaionStr)
				$sql ="SELECT SUM(TOTAL_COUNT) TOTAL_COUNT, SUM(PUSHED_TO_GCM) PUSHED_TO_GCM, SUM(ACCEPTED_BY_GCM) ACCEPTED_BY_GCM, SUM(PUSHED_TO_IOS) PUSHED_TO_IOS, SUM(ACCEPTED_BY_IOS) ACCEPTED_BY_IOS, SUM(PUSH_ACKNOWLEDGED) PUSH_ACKNOWLEDGED, SUM(LOCAL_API_HIT_BY_DEVICE) LOCAL_API_HIT_BY_DEVICE, SUM(LOCAL_SENT_TO_DEVICE) LOCAL_SENT_TO_DEVICE, SUM(LOCAL_ACKNOWLEDGED) LOCAL_ACKNOWLEDGED, SUM(ACTIVE_LOGIN_7DAY) ACTIVE_LOGIN_7DAY, SUM(ACTIVE_LOGIN_1DAY) ACTIVE_LOGIN_1DAY, DAY(ENTRY_DT) DAY FROM MOBILE_API.DAILY_NOTIFICATION_COUNT_LOG WHERE NOTIFICATION_KEY IN($scheduledNotificaionStr) AND ENTRY_DT>=:START_DATE AND ENTRY_DT<=:END_DATE GROUP BY DATE(ENTRY_DT)";
			else
				$sql ="SELECT SUM(TOTAL_COUNT) TOTAL_COUNT, SUM(PUSHED_TO_GCM) PUSHED_TO_GCM, SUM(ACCEPTED_BY_GCM) ACCEPTED_BY_GCM, SUM(PUSHED_TO_IOS) PUSHED_TO_IOS, SUM(ACCEPTED_BY_IOS) ACCEPTED_BY_IOS, SUM(PUSH_ACKNOWLEDGED) PUSH_ACKNOWLEDGED, SUM(LOCAL_API_HIT_BY_DEVICE) LOCAL_API_HIT_BY_DEVICE, SUM(LOCAL_SENT_TO_DEVICE) LOCAL_SENT_TO_DEVICE, SUM(LOCAL_ACKNOWLEDGED) LOCAL_ACKNOWLEDGED, SUM(ACTIVE_LOGIN_7DAY) ACTIVE_LOGIN_7DAY, SUM(ACTIVE_LOGIN_1DAY) ACTIVE_LOGIN_1DAY, DAY(ENTRY_DT) DAY FROM MOBILE_API.DAILY_NOTIFICATION_COUNT_LOG WHERE ENTRY_DT>=:START_DATE AND ENTRY_DT<=:END_DATE GROUP BY DATE(ENTRY_DT)";
