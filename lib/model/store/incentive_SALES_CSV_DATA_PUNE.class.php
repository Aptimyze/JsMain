<?php
class incentive_SALES_CSV_DATA_PUNE extends TABLE
{
	...
	public function insertProfile($profileid,$dialerPriority,$score,$dialerDialStatus,$allotedTo,$vdDiscount,$lastLoginDt,$phone1,$phone2,$havePhoto,$birthDt,$mstatus,$everPaid,$gender,$relation,$leadId)
	{
		try
		{
			...
			$sql= "INSERT IGNORE INTO incentive.SALES_CSV_DATA_PUNE(PROFILEID,PRIORITY,ANALYTIC_SCORE,OLD_PRIORITY,DIAL_STATUS,AGENT,VD_PERCENT,LAST_LOGIN_DATE,PHONE_NO1,PHONE_NO2,PHOTO,DOB,MSTATUS,EVER_PAID,GENDER,POSTEDBY,PHONE_NO3,PHONE_NO4,LEAD_ID,CSV_ENTRY_DATE) VALUES(:PROFILEID,:PRIORITY,:ANALYTIC_SCORE,:OLD_PRIORITY,:DIAL_STATUS,:AGENT,:VD_PERCENT,:LAST_LOGIN_DATE,:PHONE_NO1,:PHONE_NO2,:PHOTO,:DOB,:MSTATUS,:EVER_PAID,:GENDER,:POSTEDBY,:PHONE_NO3,:PHONE_NO4,:LEAD_ID,:CSV_ENTRY_DATE)";
	...
	public function removeProfiles($csvEntryDate)
	{
		try
		{
			$sql="DELETE FROM incentive.SALES_CSV_DATA_PUNE WHERE CSV_ENTRY_DATE<:CSV_ENTRY_DATE";
			$prep=$this->db->prepare($sql);
        ...
        public function getData($date)
        {
                try
                {
                        $sql="SELECT * FROM incentive.SALES_CSV_DATA_PUNE WHERE CSV_ENTRY_DATE = :CSV_ENTRY_DATE ORDER BY PRIORITY DESC,ANALYTIC_SCORE DESC,LAST_LOGIN_DATE DESC";
                        $prep=$this->db->prepare($sql);
        ...
        public function getCampaignRecordsCnt()
        {
                ...
                {
			$date =date("Y-m-d");
                        $sql="SELECT count(*) CNT FROM incentive.SALES_CSV_DATA_PUNE WHERE CSV_ENTRY_DATE = :CSV_ENTRY_DATE";
                        $prep=$this->db->prepare($sql);
                        $prep->bindValue(":CSV_ENTRY_DATE",$date,PDO::PARAM_STR);
                        $prep->execute();
