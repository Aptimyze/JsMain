...
class ContactMailer
{
	...
	public static function sendMessageMailer($receiver,$sender,$message)
	{
		...
		$tpl=$emailSender->setProfile($receiver);
		$smartyObj = $tpl->getSmarty();
		$smartyObj->assign("otherProfileId",$sender->getPROFILEID());
		$smartyObj->assign("SENDERNAME",$sender->getUSERNAME());
		$smartyObj->assign("RECEIVERNAME",$receiver->getUSERNAME());
		$smartyObj->assign("CUST_MESSAGE",$message);
		$emailSender->send();
	}
