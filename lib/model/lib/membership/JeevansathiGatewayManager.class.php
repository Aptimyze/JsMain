...
class JeevansathiGatewayManager
{
    ...
    public static function setCCAVENUEParams($apiObj, $apiParams)
    {
        ...
        if ($apiParams->currency == 'DOL') {
            $merchantDataArr = array('merchant_id' => $apiObj->mid, 'amount' => $apiObj->amount, 'order_id' => $apiObj->txnid, 'tid' => $apiObj->checksum, 'merchant_param1' => $apiObj->checksum, 'redirect_url' => $apiObj->returnURL, 'cancel_url' => $apiObj->returnURL, 'language' => 'EN', 'currency' => 'USD', 'billing_name' => $apiObj->firstname, 'billing_address' => $apiObj->address);
            ...
            //$apiObj->ccavenueChecksum = CCAvenueDolManager::getCheckSum($apiObj->mid, $ORDER["ORDERID"], $ORDER["AMOUNT"], $apiObj->key, "USD", $apiObj->returnURL);
        } else {
