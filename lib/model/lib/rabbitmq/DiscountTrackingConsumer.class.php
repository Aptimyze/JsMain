...
*/
class DiscountTrackingConsumer
{
  private $connection;
  private $channel;
  ...
  */
  public function __construct($serverid,$messsageCount)
  {
    ...
    {
      $str="\nRabbitMQ Error in DiscountTrackingConsumer, Connection to rabbitmq broker with host-> ".JsConstants::$rabbitmqConfig[$serverid]['HOST']. " failed: ".$exception->getMessage()."\tLine:".__LINE__;
  ...
  {   
    ...
    try
    {
      $this->channel->basic_consume(MQ::DISC_HISTORY_QUEUE, MQ::CONSUMER, MQ::NO_LOCAL, MQ::NO_ACK,MQ::CONSUMER_EXCLUSIVE , MQ::NO_WAIT, array($this, 'processMessage'));
    }
    catch (Exception $exception) 
    {
      $str="\nRabbitMQ Error in DiscountTrackingConsumer, Unable to consume message from queues : " .$exception->getMessage()."\tLine:".__LINE__;
      RabbitmqHelper::sendAlert($str,"default");
      return;
    }  
    ...
    try
    {
      ...
      switch($process)
      {
        case 'DISCOUNT_HISTORY':  
          $handlerObj->logDiscount($body,$type);  
          break;
      }
      unset($handlerObj);
    }
