<?php
//include("/var/www/svnlive/branches/release13/jsadmin/connect.inc");
class timeFunctions
{
	/**
	...
	**/
	...
	public function get_status_color($submit_time,&$time_diff)
	{	
	...
	**/
	public function time_difference($t1, $t2,&$time_diff)
	{
		$t1=getIST($t1);
		$t2=getIST($t2);
		if($t1==$t2)
	...
	**/
	...
	public static function newtime($t1,$d,$h,$m)
	{
	//changing time t1 to GMT from EST and then addin 5hrs 30 min to convert in IST
		...
		{
			$holidayObj = new HOLIDAY();
			$holidays = $holidayObj->calculateNoOfEntriesOfDate($return_date);
			if($holidays>0)
