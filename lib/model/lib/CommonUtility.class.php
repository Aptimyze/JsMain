...
//echo "<br><br><br>".$urlToHit."?".$postParams;echo "<br><br>\n\n";//die;
  ...
  public static function sendPlusTrackInstantSMS($key,$profileid,$tokenArr=null)
  {
	...
	*/
	public static function logRecentUserEntry($profileId,$gapTimeEntry,$isMobile,$domain,$dateTime1,$dateTime2)
	{
		$allow=1;
			...
			$time=$_COOKIE["LOGUSERENTRY"];
					...
					$allow=1;
                        	else
	                        {
                	                ...
                	                if($timepassed>$gapTimeEntry)
                        	                $allow=1;
	                        }
                	}
			if($allow)
			@setcookie("LOGUSERENTRY",time(),0,"/",$domain);
		}
	...
	public static function loginTracking($profileId,$channel,$websiteVersion,$location="",$isNotApp,$isNewMobileSite)
	{
		if(!$websiteVersion)
		{
			if($isNotApp){
