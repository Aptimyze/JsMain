...
class phpUserAgentStringParser
{
  ...
  protected function doParse($userAgentString)
  {
	...
	$pattern = "#(".$userAgent['operating_system'].")[ ]+([0-9]+(?:\.[0-9]+)?)#";
	if(preg_match($pattern, $userAgent['string'], $match1))
	{
		if(isset($match1[2]))
			$userAgent['operating_system_version'] = $match1[2];
	}
	unset($match1);
      }
    }
  ...
  protected function filterMsie(array &$userAgent)
  {
    ...
    protected function filterAndroid(array &$userAgent) {
        if ('safari' === $userAgent['browser_name'] && strpos($userAgent['string'], 'android ')) {
            ...
            $userAgent['operating_system_version'] = preg_replace('|.+'.$userAgent['operating_system'].' ([0-9]+(?:\.[0-9]+)+).+|', '$1', $userAgent['string']);
     ...
     * Mac version check
     */
    protected function filterMac(array &$userAgent) {
        //if ('mac' === $userAgent['operating_system'] && strpos($userAgent['string'], ' version/')) {
        if ('mac' === $userAgent['operating_system']) {
		$pattern = "#(os)[ ]+([0-9]+(_[0-9]+)?)#";
                if(preg_match($pattern, $userAgent['string'], $match1))
                {
                        if(isset($match1[2]))
                                $userAgent['operating_system_version'] = str_replace("_",".",$match1[2]);
                }
                unset($match1);
	    //$userAgent['operating_system_version'] = preg_replace('|.+\sversion/([0-9]+\.[0-9]+)\s*.*|', '$1', $userAgent['string']);
	}
	if('mac' === $userAgent['operating_system'] && strpos($userAgent['string'], 'crios'))
	{
		$userAgent['browser_name'] = "chrome";
		$userAgent['browser_version'] = preg_replace('|.+\scrios/([0-9]+\.[0-9]+)\s*.*|', '$1', $userAgent['string']);
	}
	/*
	if(!$userAgent['operating_system_version'])
	{
		$pattern = "#(os)[ ]+([0-9]+(?:_\.[0-9]+)?)#";
                if(preg_match($pattern, $userAgent['string'], $match1))
                {
	...
	*/
     ...
     * Linux version check
     */
    protected function filterLinux(array &$userAgent) {
        if ('linux' === $userAgent['operating_system']) {
		$pattern = "#(".$userAgent['operating_system'].")[ ]+([A-Za-z0-9]+(?:\.[0-9]+)?)#";
        	if(preg_match($pattern, $userAgent['string'], $match1))
        	{
                	if(isset($match1[2]))
                        	$userAgent['operating_system_version'] = $match1[2];
