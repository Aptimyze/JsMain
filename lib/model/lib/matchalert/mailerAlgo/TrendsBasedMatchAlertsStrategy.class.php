...
class TrendsBasedMatchAlertsStrategy extends MatchAlertsStrategy
{
	...
	*/
	public function getMatches($notInProfiles = array())
	{
		...
		$SearchUtilityObj =  new SearchUtility;
                ...
                if(is_array($pids)){
                     ...
                     if(!empty($notInProfiles) && $this->logProfile == 1) {
                             $this->logRecords($this->loggedInProfileObj->getPROFILEID(), $notInProfiles, MailerConfigVariables::$strategyReceiversTVsT,$this->limit);
                     }  
                }
                return array("CNT"=>count($pids),"profiles"=>$pids);   
        }
