 ...
 */
class StrictDppBasedMatchAlertsStrategy extends MatchAlertsStrategy {
        private $listLimit;
        private $mailerLimit;
        private $hasTrends;
        private $logicLevel;
        ...
        public function getMatches($matchesSetting='') {
                if($this->hasTrends == "1"){
                        $this->sort = SearchSortTypesEnums::SortByStrictTrends;
                        $this->logicLevel = MailerConfigVariables::$logicLevelStrictTrends;
                }else{
                        $this->sort = SearchSortTypesEnums::SortByStrictNonTrends;
                        $this->logicLevel = MailerConfigVariables::$logicLevelStrictNonTrends;
                }
                $this->searchObj = new MatchAlertsStrictDppProfiles($this->loggedInProfileObj,$this->hasTrends);
                $this->searchObj->getStrictDppCriteria($this->listLimit,$this->sort);
