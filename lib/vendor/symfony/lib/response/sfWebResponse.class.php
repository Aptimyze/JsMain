...
class sfWebResponse extends sfResponse
{
  ...
  public function addMeta($key, $value, $replace = true, $escape = true)
  {
    ...
    if ($escape)
    {
      $value = htmlspecialchars($value, ENT_QUOTES, $this->options['charset'],false);
    }
  ...
  {
    return isset($this->metas['canonical']) ? $this->metas['canonical'] : '';
  }
   ...
   * @param bool    $escape  true, for escaping the title
   */
  public function setCanonical($canonical, $escape = true)
  {
    $this->addMeta('canonical', $canonical, true, $escape);
  }
   ...
   */
  public function getJavascripts($position = self::ALL, $placement="")
  {
    if (self::ALL === $position)
    {
      $javascripts = array();
      foreach ($this->getPositions() as $position)
      {
        ...
        {
	  if(!$placement){if(strstr($file,"@bottom")) $file = substr($file,0,-7);}
