...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    // clear global cache except Minify folder
    if (null === $options['app'] && 'all' == $options['type'])
    {
      $this->getFilesystem()->remove(sfFinder::type('file')->discard('.sf')->in(sfConfig::get('sf_cache_dir'),'minify'));
    }
    // clear global cache 
    if (null === $options['app'] && 'minify' == $options['type'])
    {
