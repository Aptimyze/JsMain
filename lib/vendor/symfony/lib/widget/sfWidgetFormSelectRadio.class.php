...
class sfWidgetFormSelectRadio extends sfWidgetFormChoiceBase
{
  ...
  protected function formatChoices($name, $value, $choices, $attributes)
  {
    ...
    return call_user_func($this->getOption('formatter'), $this, $inputs,$this->getOption('type'));
  }

  public function formatter($widget, $inputs,$option)
  {
    $rows = array();
	if($option=='inline'){
		...
		foreach ($inputs as $input)
			$rows[]="<div class='gridCol gennew fl'>".$input['input']."<label class='lblStyl'>".$input['label']."</label></div>";
	}else
		foreach ($inputs as $input)
		{
