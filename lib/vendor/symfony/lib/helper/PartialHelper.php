...
function get_component($moduleName, $componentName, $vars = array())
{
  ...
  $class = sfConfig::get('mod_'.strtolower($moduleName).'_partial_view_class', 'sfSmarty').'PartialView';
...
function get_partial($templateName, $vars = array())
{
  ...
  if (false !== $sep = strpos($templateName, '/'))
  {
    ...
    if($moduleName=="globalPartial")
    {
            $moduleName="global";
            $custom=1;
            $count++;
            $templateName = str_replace("globalPartial/", "", $templateName);
    }
    else
  ...
  if(!$custom)
        ...
        $actionNameArr = explode("/",$templateName);
        $actionNameArr[count($actionNameArr)-1] = "_".$actionNameArr[count($actionNameArr)-1];
        $actionName = implode("/", $actionNameArr);
