...
function include_metas()
{
  ...
  $not_tags=array("title","canonical");	
  foreach ($context->getResponse()->getMetas() as $name => $content)
  {
    if(!in_array($name,$not_tags))
    {
	   $z=tag('meta', array('name' => $name, 'content' => null === $i18n ? $content : $i18n->__($content)))."\n";
	   echo $z=str_replace("&amp;","&",$z);
    }
  }
}
...
{
  $title = htmlspecialchars_decode(sfContext::getInstance()->getResponse()->getTitle(),ENT_QUOTES);

  echo content_tag('title', $title)."\n";
}
function include_canurl()
{
 $canurl = sfContext::getInstance()->getResponse()->getCanonical();

  echo content_canurl($canurl)."\n";
}
/**
