...
class sfMemcacheCache extends sfCache
{
  ...
  public function initialize($options = array())
  {
    ...
    /********Added by Tanu for integrating older functionality*******/
    $this->setOption("prefix","");//It wont add any prefix in the key while setting data in memcache
    /*********Ends here**********/
    ...
    else
    {
      ...
      else
      {
        ...
        {
          //Commented by Tanu Gupta Ticket #1590
        //  throw new sfInitializationException(sprintf('Unable to connect to the memcache server (%s:%s).', $this->getOption('host', 'localhost'), $this->getOption('port', 11211)));
          sfContext::getInstance()->getLogger()->err("Unable to connect to the memcache server");
        }
      }
    }
  }
  ...
  */
  ...
  public function set($key, $data, $lifetime = null)
  {
    ...
    if (false !== $this->memcache->replace($this->getOption('prefix').$key, $data, false,$lifetime))
    {
      return true;
    }
