...
class cronConsumeUpdateViewLogQueueTask extends sfBaseTask
{
  /**
   * 
  ...
  {
    $this->namespace           = 'cron';
    $this->name                = 'cronConsumeUpdateViewLogQueue';
    $this->briefDescription    = 'Initialises instance of rabbitmq consumer class to retrieve messages on first server';
    $this->detailedDescription = <<<EOF
     The [cronConsumeUpdateViewLogQueue|INFO] calls receiveMessage function of consumer class through its instance to retrieve messages on first server:
     [php symfony cron:cronConsumeUpdateViewLogQueue] 
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    $consumerObj=new updateViewLogConsumer('FIRST_SERVER',0);  //If $serverid='FIRST_SERVER', then 2nd param in Consumer constructor is not taken into account.
