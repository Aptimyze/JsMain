...
class cronExecuteProductMetricLoggingConsumerTask extends sfBaseTask
{
  /**
   * 
  ...
  */
  ...
  {
    $this->namespace           = 'cron';
    $this->name                = 'cronExecuteProductMetricLoggingConsumer';
    ...
    $this->detailedDescription = <<<EOF
     ...
     [php symfony cron:cronExecuteProductMetricLoggingConsumer] 
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
   if (!sfContext::hasInstance())
    ...
    $instancesNum=MessageQueues::PRODUCT_METRIC_QUEUE_CONSUMER_COUNT;
    for($i=1;$i<=$instancesNum;$i++)
    {
      passthru(JsConstants::$php5path." ".MessageQueues::CRON_PRODUCT_METRIC_QUEUE_CONSUMER_STARTCOMMAND." > /dev/null &");
    }	
