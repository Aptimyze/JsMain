...
{  
	...
	if ($submit)
	{
		...
		{	
			echo $mailsql = "INSERT IGNORE INTO jsadmin.AFFILIATE_MAIN (SOURCE , EMAIL , MODE , ENTRYBY , ENTRYTIME) values ('$source', '$email','$mode','$username',NOW())";

	        	//$mailres = mysql_query($mailsql,$db2) or die("$mailsql".mysql_error());

			echo $sql = "INSERT  IGNORE INTO jsadmin.AFFILIATE_DATA (USERNAME , GENDER , MSTATUS, AGE, CASTE, EMAIL , PHONE_MOB , COUNTRY_RES , CITY_RES , CONTACT , ENTRY_DT) values ('$Name','$gender','$maritalstatus','$age','$caste', '$email','$mobileno','$country','$city','$address',NOW())";
