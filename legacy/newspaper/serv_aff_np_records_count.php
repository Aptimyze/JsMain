...
****************************************************************************************************************************/
        ...
        if ($submit)
	{
			...
			{					
				...
				$conv_res = mysql_query($conv_sql,$db) or die("$sql".mysql_error());
				...
				while ($conv_row = mysql_fetch_array($conv_res))
				{
					...
					{	
						...
						$conv_totsubcount[$k][$i]	+= $conv_row["CNT"];
						$conv_totmmcount[$k][$mm]	+= $conv_row["CNT"];//$conv_mmcount[$k][$mm][$i];
						$conv_total[$k]			+= $conv_row["CNT"];

						$reg_totcount[$mm]              += $conv_row["CNT"];
                                               	$mem_totcount[$mm]              += $conv_row["CNT"];//$conv_totmmcount[$k][$mm];
                                                $conv_grandtotal                += $conv_row["CNT"];
						$mem_grandtotal                 += $conv_row["CNT"];//$mem_totcount[$mm]; */
			...
			$smarty->assign("username",$username);
                        ...
                        $smarty->display("serv_aff_np_records_count.htm");
