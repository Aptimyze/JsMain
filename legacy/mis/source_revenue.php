...
include("connect.inc");
/**************************************************************************************************************************
*       FILE NAME        : source revenue mis.php
*       CREATED BY       : 
*       MODIFIED ON      : 
...
if(authenticated($cid))
{
	if($CMDGo && $month!='ALL' && $year!='ALL')
	{
		...
		$sql="SELECT A.PROFILEID,A.SCORE,A.CURRENT_SCORE,A.ENTRY_DT,A.SOURCE,A.GROUPNAME,A.PAID_ALLTIME,A.CONV_RATE,A.COUNTRY_RES,DAY(A.ENTRY_DT) AS day FROM incentive.MAIN_ADMIN_POOL_INTO_SYSTEM_20_DAYS AS A , newjs.JPROFILE AS B WHERE A.PROFILEID=B.PROFILEID AND B.ACTIVATED='Y' AND A.ENTRY_DT BETWEEN '$year-$month-01' AND '$year-$month-31'";
		//$sql="SELECT *,DAY(ENTRY_DT) as day FROM incentive.MAIN_ADMIN_POOL_INTO_SYSTEM_20_DAYS WHERE ENTRY_DT BETWEEN '$year-$month-01' AND '$year-$month-31' ";
		...
		while($row=mysql_fetch_array($res))
		{
			...
			//price shoud change when services prices are increased. 2234,1507
			...
			$revenue[$groups[$row['GROUPNAME']]][$row['day']-1]+=round(($row['CONV_RATE']/100) * $price,2);
			$revenue_total[$groups[$row['GROUPNAME']]]+=round(($row['CONV_RATE']/100) * $price,2);
			$revenue_day_total[$row['day']-1]+=round(($row['CONV_RATE']/100) * $price,2);		
	...
	for($i=2004;$i<=date("Y");$i++)
{
	...
	$smarty->display("source_revenue_mis.htm");
}
else
{
