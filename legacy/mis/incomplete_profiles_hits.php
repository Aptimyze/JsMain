...
if(authenticated($checksum) || $JSIndicator)
{
	if($CMDGo && $download != 'Y')
	{
		...
		else
		{
			...
			$sql1=" SOURCEGP ";
			$sql2=" GROUPNAME ";
			$sql_s="SELECT DISTINCT GROUPNAME FROM MIS.SOURCE WHERE GROUPNAME<>'NONE'";
			$res_s=mysql_query_decide($sql_s,$db) or die("$sql_h".mysql_error_js());
			while($row_s=mysql_fetch_array($res_s))
			{
		...
		if($dt_type=="day")
		{
			...
			if($get_today)
			{
				...
				if($business_flag)
					...
					$sql_mb="SELECT COUNT(distinct j.PROFILEID) as cnt, DAYOFMONTH(j.ENTRY_DT) as dd FROM newjs.JPROFILE j,jsadmin.INCOMPLETE i where j.ENTRY_DT>='$today' AND j.SOURCE like 'af%' AND j.SOURCE not like 'afl%' AND j.PROFILEID=i.PROFILEID";
                        	        $sql_mb.=" AND j.INCOMPLETE='Y' ";
					$sql_mb.=" GROUP BY dd";
				}
        			if(($sourcegp && !$business_flag) || !$sourcegp)
        			{
					$sql_h="SELECT s.$sql2 as src, COUNT(*) as cnt,DAYOFMONTH(Date) as dd FROM MIS.SOURCE s,MIS.HITS h WHERE Date>='$today' AND s.GROUPNAME<>'NONE' ";
					if($sourcegp)
						$sql_h.=" AND s.GROUPNAME='$sourcegp' ";
					$sql_h.=" AND s.SourceID=h.SourceID GROUP BY src,dd";
					//sql_m->src, cnt, dd, j.
					$sql_m="SELECT s.$sql2 as src, COUNT(distinct j.PROFILEID) as cnt,DAYOFMONTH(j.ENTRY_DT) as dd FROM MIS.SOURCE s,newjs.JPROFILE j,jsadmin.INCOMPLETE i WHERE j.ENTRY_DT>='$today' AND s.GROUPNAME<>'NONE' AND j.PROFILEID=i.PROFILEID";
					$sql_m.=" AND j.INCOMPLETE='Y' ";
					if($sourcegp)
			...
			else//for the entire month from start date.
			{
				if($business_flag)
				{
					...
					$sql_mb="SELECT SUM(COUNT) as cnt, DAYOFMONTH(ENTRY_DT) as dd FROM MIS.SOURCE_MEMBERS_INCOMPLETE WHERE ENTRY_DT BETWEEN '$st_date' AND '$end_date' AND SOURCEID like 'af%' AND SOURCEID not like 'afl%' ";
                                        $sql_mb.=" GROUP BY dd";
				//here
				}
				if(($sourcegp && !$business_flag) || !$sourcegp)
				{
					$sql_h="SELECT $sql1 as src, SUM(COUNT) as cnt,DAYOFMONTH(ENTRY_DT) as dd FROM MIS.SOURCE_HITS WHERE ENTRY_DT BETWEEN '$st_date' AND '$end_date' AND SOURCEGP<>'NONE' ";
					if($sourcegp)
						$sql_h.=" AND SOURCEGP='$sourcegp' ";
					$sql_h.="GROUP BY src,dd";
					$sql_m="SELECT $sql1 as src, SUM(COUNT) as cnt,DAYOFMONTH(ENTRY_DT) as dd FROM MIS.SOURCE_MEMBERS_INCOMPLETE WHERE ENTRY_DT BETWEEN '$st_date' AND '$end_date' AND SOURCEGP<>'NONE' ";
					if($sourcegp)
		...
		elseif($dt_type=="mnt")//if data type is month
		{
			...
			if($business_flag)
			{
				 ...
				 $sql_ma="Select SUM(COUNT) as cnt, MONTH(ENTRY_DT) as mm FROM MIS.SOURCE_MEMBERS_INCOMPLETE WHERE ENTRY_DT BETWEEN '$st_date' AND '$end_date' AND SOURCEID like 'af%' AND SOURCEID not like 'afl%' ";
                                $sql_ma.=" GROUP BY mm";
			...
			if(($sourcegp && !$business_flag) || !$sourcegp)
			{
				$sql_h="SELECT $sql1 as src, SUM(COUNT) as cnt,MONTH(ENTRY_DT) as mm FROM MIS.SOURCE_HITS WHERE ENTRY_DT BETWEEN '$st_date' AND '$end_date' AND SOURCEGP<>'NONE' ";
				if($sourcegp)
					$sql_h.=" AND SOURCEGP='$sourcegp' ";
				$sql_h.=" GROUP BY src,mm";
				$sql_m="SELECT $sql1 as src, SUM(COUNT) as cnt,MONTH(ENTRY_DT) as mm FROM MIS.SOURCE_MEMBERS_INCOMPLETE WHERE ENTRY_DT BETWEEN '$st_date' AND '$end_date' AND SOURCEGP<>'NONE' ";
				if($sourcegp)
					$sql_m.=" AND SOURCEGP='$sourcegp' ";
...
/**************************************End of Addition********************************************************************/ 
		...
		if ($prop_mis == 'Y')
		{
			...
			$smarty->assign("incompProfiles",TRUE);
			$smarty->display("source_prop_mis.htm");
		}
		else
			$smarty->display("incomplete_profiles_hits.htm");
		...
		if($dt_type=="day")
		{
			...
			if($get_today)
			{
				...
				if($business_flag)
		        	{
					$sql_hb="SELECT COUNT(*) as cnt, DAYOFMONTH(Date) as dd FROM MIS.HITS where Date>='$today' AND SourceID like 'af%' AND SourceID not like 'afl%' GROUP BY dd";
					$sql_mb="SELECT COUNT(distinct j.PROFILEID) as cnt, DAYOFMONTH(j.ENTRY_DT) as dd FROM newjs.JPROFILE j,jsadmin.INCOMPLETE i where j.ENTRY_DT>='$today' AND SOURCE like 'af%' AND SOURCE not like 'afl%' AND j.PROFILEID=i.PROFILEID";
                        	        $sql_mb.=" AND j.INCOMPLETE='Y' ";
					$sql_mb.=" GROUP BY dd";
				}
        			if(($sourcegp && !$business_flag) || !$sourcegp)
...
***************************************************************************************************************************/
	...
	{	
		...
		*/
		...
		$smarty->assign("checksum",$checksum);
		$smarty->assign("incompProfiles","1");
