...
if(authenticated($cid))
{
	...
	if($submit)
	{
		...
		$sql = "SELECT COUNT( * ) AS COUNT, CONFIRM, DAYOFMONTH( REQ_DT ) AS DAY FROM incentive.PAYMENT_COLLECT WHERE CONFIRM IN ('Y','N') AND REQ_DT BETWEEN '$start_date' AND '$end_date' AND PICKUP_TYPE='CHEQ_REQ_USER'";
		...
		$sql .= " GROUP BY LEFT(REQ_DT,10), CONFIRM";
		$res = mysql_query_decide($sql,$db) or die($sql.mysql_error_js());
		while($row = mysql_fetch_array($res))
		{
			$day = $row['DAY'] - 1;
			...
			if($row['CONFIRM'] == "N")
			{
				$touched_cc['REJECT'][$day] = $row['COUNT'];
				//row total for rejected records..
				$total_cc['REJECT'] += $row['COUNT'];
			}
			elseif($row['CONFIRM'] == "Y")
			{
				$touched_cc['ACCEPT'][$day] = $row['COUNT'];
				//row total for accepted records..
				$total_cc['ACCEPT'] += $row['COUNT'];
			}
		...
		$sql = "SELECT COUNT( * ) AS COUNT, crd.STATUS , DAYOFMONTH( crd.ENTRY_DT ) AS DAY FROM billing.CHEQUE_REQ_DETAILS crd, incentive.PAYMENT_COLLECT pc WHERE crd.REQUEST_ID = pc.ID AND crd.STATUS IN ('DONE', 'CANCEL') AND crd.ENTRY_DT BETWEEN '$start_date' AND '$end_date'";
		...
		$sql .= " GROUP BY LEFT( crd.ENTRY_DT, 10 ) , crd.STATUS";
		...
		while($row = mysql_fetch_array($res))
		{
			...
			{
				$touched_bt['REJECT'][$day] = $row['COUNT'];
				//row total for rejected records..
				$total_bt['REJECT'] += $row['COUNT'];
			}
			elseif($row['STATUS'] == "DONE")
			{
				$touched_bt['ACCEPT'][$day] = $row['COUNT'];
				//row total for accepted records..
				$total_bt['ACCEPT'] += $row['COUNT'];
			}

		}

		//finding the PENDING records for which mail has been sent.
		$sql = "SELECT COUNT( * ) AS COUNT, DAYOFMONTH( crd.ENTRY_DT ) AS DAY FROM billing.CHEQUE_REQ_DETAILS crd, incentive.PAYMENT_COLLECT pc WHERE crd.REQUEST_ID = pc.ID AND crd.ENTRY_DT BETWEEN '$start_date' AND '$end_date' AND crd.MAIL_SENT > 0";
		...
		$sql .= " GROUP BY LEFT( crd.ENTRY_DT, 10 )";

		$res = mysql_query_decide($sql) or die($sql.mysql_error_js());
		while($row = mysql_fetch_array($res))
		{
			$day = $row['DAY'] - 1;
			$touched_bt['MAIL_SENT'][$day] = $row['COUNT'];
			//row total for accepted records..
			$total_bt['MAIL_SENT'] += $row['COUNT'];
		}
	...
	elseif($cc_link_clicked)
	{
		...
		$sql = "SELECT COUNT(*) AS COUNT, REJECT_REASON FROM billing.REJECTED_RECORDS rr, incentive.PAYMENT_COLLECT pc WHERE rr.ORDER_ID = pc.ID AND pc.REQ_DT BETWEEN '$req_date_start' AND '$req_date_end' AND pc.PICKUP_TYPE='CHEQ_REQ_USER'";
	...
	elseif($bt_link_clicked)
	{
		...
		if(mysql_num_rows($res) > 0)
		{
			...
			//query to find count grouped on reject reason's
			$sql = "SELECT COUNT(DISTINCT(REQUEST_ID)) AS COUNT, REJECT_REASON FROM billing.REJECTED_RECORDS rr, billing.CHEQUE_REQ_DETAILS crd, incentive.PAYMENT_COLLECT pc WHERE rr.ORDER_ID = crd.REQUEST_ID AND crd.REQUEST_ID = pc.ID AND crd.ENTRY_DT BETWEEN '$entry_date_start' AND '$entry_date_end'";
			//for a prticular agent
			if($mis_type == "AGENT_WISE")
				$sql .= " AND pc.ACC_REJ_MAIL_BY = '$agent'";
			//for a prticular branch
	...
	$smarty->display("rejected_records_mis.htm");
}
else
{
	$smarty->assign("user",$user);
