...
$sql0="select PROFILEID FROM twowaymatch.TRENDS where REVERSE_COUNT=0 limit 100000";
...
while($row0=mysql_fetch_array($res0))
{
	...
	while($row1=mysql_fetch_array($res_main))
	{
		...
		$receiver_str='';
		$sql_spam="select datediff(now(),SPAM_DATE) as spam_date from jsadmin.SPAMMERS where SPAMMER=$row1[PROFILEID]";
		$res_spam=mysql_query($sql_spam,$db2) or die($sql_spam.mysql_error($db2));
		if($row_spam=mysql_fetch_array($res_spam))
		{
			$spam_date=$row_spam[0];
		}
		else
			$spam_date=-1;

		if($spam_date!=-1)
			$spammer_time=" and datediff(now(),A.`TIME`)<=$spam_date ";
		else
			$spammer_time='';
		...
		mail("sadaf.alam@jeevansathi.com","Reverse Scores calculated for $records Trends table in $time hours ".date('Y-m-d'),date('Y-m-d'));
