...
*/
var tb_pathToImage = "IMG_URL/images/loadingAnimation.gif";
tb_pathToImage = "IMG_URL/images/loader_big.gif";
	...
	try {
		if (typeof document.body.style.maxHeight === "undefined") {//if IE 6
			...
			if (document.getElementById("TB_HideSelect") === null) {//iframe to hide select elements in ie6
				...
				if (url.indexOf('/social/profileLayer/view')!=-1)	//If condition Added by ANAND
					$("#TB_overlay").click(reloadPage);
				else
		...
		if(url.indexOf("TYPE_OF")!=-1 && url.indexOf("TYPE_OF=CI")==-1 )
		{
	   ...
	   var urlType = baseURL.toLowerCase().match(urlString);
		...
		if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp'){//code to show images
			...
			if(url.indexOf('TB_iframe') != -1){// either iframe or ajax window		
				...
				// IF CONDITION MODIFIED BY ANAND FOR PROFILE PIC LAYER 
						...
						$("#TB_window").append("<iframe frameborder='0' hspace='0' vspace = '0' scrolling = 'no' src='"+urlNoQuery[0]+"' id='TB_iframeContent' name='TB_iframeContent"+Math.round(Math.random()*1000)+"' onload='tb_showIframe()' style='width:"+(ajaxContentW)+"px;height:"+(ajaxContentH)+"px;' > </iframe>");
					}else{//iframe modal
					$("#TB_overlay").unbind();
						$("#TB_window").append("<iframe frameborder='0' hspace='0' vspace = '0' scrolling = 'no' src='"+urlNoQuery[0]+"' id='TB_iframeContent' name='TB_iframeContent"+Math.round(Math.random()*1000)+"' onload='tb_showIframe()' style='width:"+(ajaxContentW + 29)+"px;height:"+(ajaxContentH + 17)+"px;' > </iframe>");
					}
			...
			if (url.indexOf('/social/profileLayer')==-1){ // IF CONDITION ADDED BY ANAND FOR PROFILE PIC LAYER
			...
			$("#TB_closeWindowButton").click(tb_remove);
				...
				if(url.indexOf('TB_inline') != -1){	
					...
					data={};
					if(url.indexOf("AjaxContact.php")!=-1 || url.indexOf("single_contact_aj.php")!=-1)
        			        {

						 var queryString = url.replace(/^[^\?]+\??/,'');
			       data = tb_parseQuery( queryString );
						 url=baseURL+"?";
					}
	
					$("#TB_ajaxContent").load(url += "&random=" + (new Date().getTime()),data,function(){//to do a post change this load method
...
function reloadPage()	//Function added by ANAND for reload of viewAllPhotos template when the user closes the cropping Layer
{
	window.location.href= "/social/viewAllPhotos/none";
}
