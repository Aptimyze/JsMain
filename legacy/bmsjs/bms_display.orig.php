...
$dbbms = mysql_connect("localhost","root","Km7Iv80l") or logErrorBms("BMS Site is down for maintenance. Please try after some time.","","ShowErrTemplate");
...
/*	$sql="Select locid from GeoIP.Blocks_Co where ('$ipnum' >= startIpNum and '$ipnum' <= endIpNum)";
  	$sql="Select locid from GeoIP.Blocks  where ('$ipnum' >= startIpNum and '$ipnum' <= endIpNum)";
...
*/
        ...
        if($myrow=mysql_fetch_array($result))
        {
                ...
                $result1=mysql_query($sql1,$dbbms) or logErrorBms("bms_display.php:getIPLocation:1:Could not get Location Id of ip. <br><!--$sql1(".mysql_error($dbbms).")-->:".mysql_errno($dbbms),$sql1,"continue","YES");
		...
		$country = $critarr["country"];
                $city    = $critarr["city"];
                {
                        if($locstr)
                        {
                                ...
                                if($country == 51)
                                        $locstr.="and (b.BannerInCity LIKE '%$city%' or b.BannerInCity='')";
                                if($country == 127)
