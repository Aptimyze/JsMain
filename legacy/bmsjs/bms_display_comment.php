...
$_LogosZone=31;      //Logos Zone where Banner String is used instead of Zone string
...
function getIPLocation()
{
	...
	//print_r($iparr);
//echo	$sql="Select locid from GeoIP.Blocks_IN  Force Index (startIpNum_2) where ('$ipnum' >= startIpNum and '$ipnum' <= endIpNum)";
  	$sql="Select locid from GeoIP.Blocks  Force Index (startIpNum) where ('$ipnum' >= startIpNum and '$ipnum' <= endIpNum)";
	$result=mysql_query($sql,$dbbms) or logErrorBms("bms_display.php:getIPLocation:1:Could not get Location Id of ip. <br><!--$sql(".mysql_error($dbbms).")-->:".mysql_errno($dbbms),$sql,"continue","YES");  
	if($myrow=mysql_fetch_array($result))
	{
		return $locid=$myrow["locid"];
...
{
	global $_COOKIE;		
	//echo "REGION : ".$region." ZONE : ".$zone." LOCATION : ".$location." CATEGORIES : ".$categories;		
	...
	$critarr["location"]=$userarr["CITY_RES"];
...
function display($critarr)
{
	global $dbbms;
    	...
    	if(!$zone || $zone==0)
    	{
     		...
     		$result=mysql_query($sql,$dbbms) or logErrorBms("bms_display.php:display:1:Could not get Zone listings. <br><!--$sql(".mysql_error($dbbms).")-->:".mysql_errno($dbbms),$sql,"continue","YES");
    	}
    	else
    	{
     		$sql="Select SQL_CACHE * from bms2.ZONE where ZoneId in ($zone) and ZoneStatus='active'";
     		$result=mysql_query($sql,$dbbms) or logErrorBms("bms_display.php:display:2:Could not get Zone listings. <br><!--$sql(".mysql_error($dbbms).")-->:".mysql_errno($dbbms),$sql,"continue","YES");
    	}
	if($myrow=mysql_fetch_array($result))
    	{
    		do
...
echo "@@";        	$criterias=explode(",",$zonearr[$currzone]["criterias"]);
print_r($criterias);
echo "@@";
		...
		echo "**********QUERY*************************<br>";
    		...
    		$resfin=mysql_query($query,$dbbms) or logErrorBms("bms_display.php:display:3:Could not retrieve the banners. <br><!--$sql(".mysql_error($dbbms).")-->:".mysql_errno($dbbms),$sql,"continue","YES");
  ...
  function actual_display($finlist,$bannarr,$zonearr,$zone)
  {
  	...
  	$align=$zonearr[$zone]["align"];
    	...
    	if($zone!=$_LogosZone)
    	{
    		if($ispopup!='Y')
    		{
    			$echostr="<Table border=0 cellpadding=2 cellspacing=2>";
    		  	...
    		  	for($i=1;$i<=$maxbans;$i++)
			{
				...
				if($banner)
				{
					...
					if($echostr)
  	    	  			{
  	    	  			//	$echostr.=",$gif#$features#".$bannarr[$banner]["class"];
  	    	  				$echostr.=",$gif#$features#".$class;
  	    	  			}
  	    	  			else
  	    	  			{
//  	    	  				$echostr="$gif#$features#".$banner[$banner]["class"];
  	    	  				$echostr="$gif#$features#".$class;
  	    	  			}
				}
    			}
	    	...
	    	echo "HI";echo $returnzones=$echostr;echo "bye";
  	}
  	else
	{
		for($i=1;$i<=$maxbans;$i++)
...
function createQuery($critarr,$zonecriterias,$zones)
{
	...
	$sql="Select b.*,h.BannerCount from bms2.BANNER b,bms2.BANNERHEAP h where (b.ZoneId IN ($zones)) and b.BannerStatus='live'";
   	...
   	{
		echo "HERE I AM";			
    		$locarr=explode(',',$critarr["location"]);
    		for($i=0;$i<count($locarr);$i++)
    		{
     			if($locstr)
     			{
      				$locstr.=" or b.BannerLocation LIKE '% $locarr[$i] %'";
     			}
     			else
     			{
      				$locstr=" b.BannerLocation LIKE '% $locarr[$i] %'";
     			}
    		}
	...
	if(in_array("INCOME",$zonecriterias))	
   	...
   	{
   		$ctc=$critarr["ctc"]; 
//   		$ctcstr=" (b.BannerCTCMin <= '$ctc' and b.BannerCTCMax >= '$ctc') ";  
		...
		echo "<br>"."ZONEARR :";
   		...
   		for($i=0;$i<count($zonearray);$i++)
   		{
			...
			if($zonearr[$zone]["ispop"]!='Y') 
				$returnzones[$string].="<Img src=\"$_LOGIMPS?banlist=".$bannerlist[$zone]."\" Width=0 height=0>";
	...
	global $dbbms;
   	if($banlist and trim($banlist)!='')
   	{
	   	$sql="Update bms2.BANNERHEAP set BannerServed=(BannerServed+1),BannerCount=(BannerCount+1) where BannerId IN ($banlist)";
    		mysql_query($sql,$dbbms) or logErrorBms("bms_display.php:logimpression:4:Could not log the impression.<br><!--$sql(".mysql_error($dbbms).")-->:".mysql_errno($dbbms),$sql,"continue","YES");
   ...
   function filterOnCriteria($bannarr,$banner,$critarr,$zonecrit)
   {
			...
			if(in_array("LOCATION",$zonecritarr))
			{
				if($bannarr[$banner]["location"] && trim($bannarr[$banner]["location"])!='')
				{
					if($critarr["location"] && trim($critarr["location"])!='')
					{
				 		$temp=explode("#",$bannarr[$banner]["location"]);
						$string=trim($temp[1]);
				 		$string=str_replace(" , ",",",$string);
				 		$string=strtoupper($string);
				 		$strarr=explode(",",$string);
				 		
				 		$crits=explode(",",$critarr["location"]);
				 		for($i=0;$i<count($crits);$i++)
				 		{
				 			if(in_array($crits[$i],$strarr))
				 			{
					 			$resfrom["location"]="True";
					 			if($dueto["location"])
					 			 	$dueto["location"].=",".$crits[$i];
					 			else 
					 			 	$dueto["location"]=$crits[$i]; 
					 		}
				 		}
...
//						if(($critarr["ctc"] >= $bannarr[$banner]["minctc"]) && ($critarr["ctc"] <= $bannarr[$banner]["maxctc"]))
                                                ...
                                                echo "<br>"."TEMP : ";print_r($temp);echo "<br>";                                                $string=trim($temp[1]);
                                                $string=str_replace(" ","",$string);
                                                $strarr=explode(",",$string);
