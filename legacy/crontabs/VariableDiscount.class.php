...
class VariableDiscount 
{
	...
	// get VD Discount
        ...
        public function addVariableDiscount()
        {
			...
			mysql_query_decide($sqlVdDuration,$this->myDb) or die($sqlVdDuration.mysql_error($this->myDb));
                }
        }
	...
	public function fetchVdPoolProfiles()
	{
        ...
        public function filterJprofileCondition($profileid)
        {
                ...
                $resJ =mysql_query_decide($sqlJ,$this->slaveDb) or die($sqlJ.mysql_error($this->slaveDb));
			...
			if(!$subExpiryPeriodExist){
				...
				return;
                	...
                	mysql_query_decide($sqlVd,$this->myDb) or die($sqlVd.mysql_error($this->myDb));
		}else
			$this->removeVdPoolProfiles(array($profileid));
	}	
	...
	public function fetchVdDiscount($profileid,$mTongueArr)
	{
		...
		$sql ="select SCORE,GENDER,MTONGUE from billing.VARIABLE_DISCOUNT_POOL_TECH where PROFILEID='$profileid'";
			...
			$resLook =mysql_query_decide($sqlLook,$this->slaveDb) or die($sqlLook.mysql_error($this->slaveDb));
