<?php 
 ...
 include_once("/usr/local/scripts/DocRoot.php");
	...
	function check_profile($profileid,$extra_params="")
	{
		...
		$sql_offline= "SELECT COUNT(*) AS COUNT FROM newjs.JPROFILE WHERE PROFILEID= '$profileid' AND SOURCE IN ('OFL_PROF','onoffreg')";
	...
	function write_contents_to_file($profileid,$mtongue="",$score="",$failed_payments="",$DPP=0,$extra_params="")
	{
		...
		if($extra_params["SERVICE_CALL"] == 1)
			...
			$sql = "SELECT ENTRY_DT, LAST_LOGIN_DT , USERNAME , PHONE_RES  , DTOFBIRTH  , STD, PHONE_MOB , CITY_RES , COUNTRY_RES , MTONGUE , GENDER , RELATION , HAVEPHOTO , CASTE , CHAR_LENGTH(YOURINFO) + CHAR_LENGTH(FAMILYINFO) + CHAR_LENGTH(SPOUSE) + CHAR_LENGTH(JOB_INFO) + CHAR_LENGTH(SIBLING_INFO) + CHAR_LENGTH(FATHER_INFO) AS PROFILELENGTH FROM newjs.JPROFILE WHERE PROFILEID ='$profileid' AND SUBSCRIPTION='' AND ACTIVATED IN ('Y','H') AND INCOMPLETE='N' AND LAST_LOGIN_DT>=DATE_SUB(CURDATE(), INTERVAL 45 DAY)";

		if(!$failed_payments)
			$sql .= " AND COUNTRY_RES=51";
		...
		if ($row = mysql_fetch_array($res))
		{
			...
			$edt1=explode("-",$ENTRY_DT);
			$edt2=explode(":",$edt1[2]);
			$edt3=explode(" ",$edt2[0]);
			$edt2[0]=$edt3[1];
			$edts=mktime($edt2[0],$edt2[1],$edt2[2],$edt1[1],$edt1[2],$edt1[0]);
			$ENTRY_DT1=date("d/m/y h:m:s",$edts);
			$ENTRY_DT2 = $ENTRY_DT1;
			...
			if((trim($PHONE_RES) || trim($PHONE_MOB)))
			{
				...
				$DOB = $row['DTOFBIRTH'];
				$dobt1=explode("-",$DOB);
				$dobts=mktime(0,0,0,$dobt1[1],$dobt1[2],$dobt1[0]);
				$DOB1=date("d/m/y",$dobts);
				$DOB = $DOB1;
				$posted = $row['RELATION'];
				switch($posted)
				{
				...
				{
					$line="\"$profileid\"".","."\"$PHONE_MOB\"".","."\"$PHONE_RES\"".","."\"$CITY_RES $EVER_PAID1\"".","."\"$PHOTO1 $status\"".","."\"$INITIATE_CNT\"".","."\"$TOTAL_ACCEPTANCE\"".","."\"$RECEIVE_CNT\"".","."\"$TOTAL_ACCEPTANCE\"".","."\"$DOB\"".","."\"$POSTEDBY\"".","."\"$GENDER\"".","."\"$CASTE\"".","."\"$MTONGUE\"".","."\"$PROFILELENGTH\"".","."\"$HAVEPARTNER\"".","."\"$LAST_LOGIN_DT\"".","."\"$score\"".","."\"$ENTRY_DT2\"".","."\"$lead_id\"".","."\"$PHONE_MOB1\"".","."\"$PHONE_RES1\"".","."\"1\"".","."\"\"";
				}
				elseif ($PHONE_MOB && $PHONE_RES =='')
				{
					$line="\"$profileid\"".","."\"$PHONE_MOB\"".",\"\","."\"$CITY_RES $EVER_PAID1\"".","."\"$PHOTO1 $status\"".","."\"$INITIATE_CNT\"".","."\"$TOTAL_ACCEPTANCE\"".","."\"$RECEIVE_CNT\"".","."\"$TOTAL_ACCEPTANCE\"".","."\"$DOB\"".","."\"$POSTEDBY\"".","."\"$GENDER\"".","."\"$CASTE\"".","."\"$MTONGUE\"".","."\"$PROFILELENGTH\"".","."\"$HAVEPARTNER\"".","."\"$LAST_LOGIN_DT\"".","."\"$score\"".","."\"$ENTRY_DT2\"".","."\"$lead_id\"".","."\"$PHONE_MOB1\"".","."\"\"".","."\"1\"".","."\"\"";
				}
				elseif ($PHONE_MOB =='' && $PHONE_RES)
				{
					$line="\"$profileid\"".","."\"$PHONE_RES\"".",\"\","."\"$CITY_RES $EVER_PAID1\"".","."\"$PHOTO1 $status\"".","."\"$INITIATE_CNT\"".","."\"$TOTAL_ACCEPTANCE\"".","."\"$RECEIVE_CNT\"".","."\"$TOTAL_ACCEPTANCE\"".","."\"$DOB\"".","."\"$POSTEDBY\"".","."\"$GENDER\"".","."\"$CASTE\"".","."\"$MTONGUE\"".","."\"$PROFILELENGTH\"".","."\"$HAVEPARTNER\"".","."\"$LAST_LOGIN_DT\"".","."\"$score\"".","."\"$ENTRY_DT2\"".","."\"$lead_id\"".","."\"\"".","."\"$PHONE_RES1\"".","."\"1\"".","."\"\"";
				}
