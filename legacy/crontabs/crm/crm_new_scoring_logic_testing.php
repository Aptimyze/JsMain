<?php 
 ...
 include_once("/usr/local/scripts/DocRoot.php");
	...
	$parameter = "USERNAME,GENDER,MTONGUE,RELATION,COUNTRY_RES,CITY_RES,ENTRY_DT,DRINK,SMOKE,BTYPE,DIET,MANGLIK,HAVEPHOTO,SHOW_HOROSCOPE,AGE,YOURINFO,FATHER_INFO,SIBLING_INFO,JOB_INFO,ACTIVATED,INCOME,SUBSCRIPTION,LAST_LOGIN_DT,SOURCE";
        ...
        {
		$myDb=connect_737();
		$shDbName=getActiveServerName($activeServerId);
                $shDb=$mysqlObj->connect("$shDbName","slave");
		mysql_query('set session wait_timeout=50000',$shDb);
		$sql = "SELECT DISTINCT(PROFILEID) FROM LOGIN_HISTORY WHERE LOGIN_DT>=DATE_SUB(CURDATE(),INTERVAL 30 DAY) and PROFILEID='5912'";
		$res = mysql_query_decide($sql,$shDb) or die($sql.mysql_error($shDb));
		while($row = mysql_fetch_array($res))
		{
			$pid = $row['PROFILEID'];
			$sqlf = "SELECT ALLOTMENT_AVAIL,TIMES_TRIED,SOURCE FROM incentive.MAIN_ADMIN_POOL WHERE PROFILEID=$pid";
                        $resf = mysql_query_decide($sqlf,$myDb) or die($sqlf.mysql_error($myDb));
                        $rowf = mysql_fetch_array($resf);
			if($rowf['ALLOTMENT_AVAIL']=='Y' && $rowf['TIMES_TRIED']<3 && $rowf['SOURCE']!='OFL_PROF' && $rowf['SOURCE']!='onoffreg')
				$pro_arr[] = $pid;
		}
		...
		if(count($first_arr)>0)
		{
			for($j=0;$j<count($first_arr);$j++)
			{
				...
				$sql1 = "select weight,param from scoring.weight where payment_type='F'";
				$res1 = mysql_query_decide($sql1,$myDb) or die($sql1.mysql_error($myDb));
				...
				echo $p_att.",".$p_beh;
				$sql2 = "select * from scoring.pop_users where payment_type='F'";
				...
				echo "*";
				echo $prob_4paidest=round(((($p_att+2*$p_beh)/3)/$sfactor),3);die;
		...
		if(count($repeat_arr)>0)
		{
			for($j=0;$j<count($repeat_arr);$j++)
			{
				...
				$sql1 = "select weight,param from scoring.weight where payment_type='R'";
				$res1 = mysql_query_decide($sql1,$myDb) or die($sql1.mysql_error($myDb));
				while($row1 = @mysql_fetch_array($res1))
				{
					$param=$row1["param"];
				...
				$p_att = exp($attscore)/(1+exp($attscore));
				$p_beh = exp($behscore)/(1+exp($behscore));
				$sql2 = "select * from scoring.pop_users where payment_type='R'";
                                $res2 = mysql_query_decide($sql2,$myDb) or die($sql1.mysql_error($myDb));
                                while($row2 = @mysql_fetch_array($res2))
                                {
                                        $param=$row2["param"];
                                ...
                                $sfactor = round((($pop_tot_users/$pop_paid_users)/($model_pop_tot_users/$model_pop_paid_users)),2);
				echo $sfactor = ($pop_tot_users/$pop_paid_users)/($model_pop_tot_users/$model_pop_paid_users);die;
