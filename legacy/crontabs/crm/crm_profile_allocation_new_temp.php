<?php 
 ...
 include_once("/usr/local/scripts/DocRoot.php");
	...
	$sql="TRUNCATE TABLE incentive.PROFILE_ALLOCATION_TEMP";
	...
	while ($row_branches = mysql_fetch_array($res_branches))
	{
		...
		if($city_str)
		{
			if($city_res !='WB05' && $city_res !='PH00' && $city_res !='AP03' && $city_res !='TN02' && $city_res !='KE03')
			{
					...
					if($row_jp = mysql_fetch_array($res_jp))
					{
						if(check_profile($profileid))
						{
							...
							if($allocate && ($row_jp['PHONE_RES'] || $row_jp['PHONE_MOB']))
							{
								/*if($city_res == 'KA02')
                                                        	{
									//commenting because tahir's id has been deactivated.
									if(($userarr[0]['ALLOTED'] < $MAX_ALLOCATE || $userarr[1]['ALLOTED'] < $MAX_ALLOCATE || $userarr[2]['ALLOTED'] < $MAX_ALLOCATE)&& !profile_allocated($profileid))
                                                                        	...
                                                                        	if(isset($banf))
										 {
											...
											if($user_value !='')
											{
                                	                                                	$sql_ins = "INSERT IGNORE INTO incentive.PROFILE_ALLOCATION_TEMP (PROFILEID, ALLOTED_TO , ALLOT_DT,STATUS,PROFILE_TYPE) VALUES('$profileid','$user_value',now(),'N','$profile_type')";
								...
								else*/if($userarr[$total_executives-1]['ALLOTED'] < $MAX_ALLOCATE && !profile_allocated($profileid))
								{
			...
			//$sql = "SELECT PROFILEID,CITY_RES FROM incentive.MAIN_ADMIN_POOL WHERE CITY_RES IN ($city_str) AND ALLOTMENT_AVAIL ='Y' AND TIMES_TRIED < 3 AND ENTRY_DT < DATE_SUB(CURDATE(),INTERVAL 8 DAY) ORDER BY SCORE DESC";
			$sql = "SELECT PROFILEID,CITY_RES FROM incentive.MAIN_ADMIN_POOL_BUFFER WHERE CITY_RES IN ($city_str) ORDER BY SCORE DESC";
			$res = mysql_query($sql) or die("$sql".mysql_error());
			while($row = mysql_fetch_array($res))
			{
				$profileid = $row['PROFILEID'];
