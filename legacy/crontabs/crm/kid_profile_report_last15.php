...
$yest_dt_ts=time()-15*24*60*60;
$yest_dt= date("Y-m-d",$yest_dt_ts);
$st_dt= $yest_dt." 00:00:00";
$st_dt_frt= date("dMy",$yest_dt_ts);
$st_frmt=strtoupper($st_dt_frt);

$yest_dt_ts=time()-24*60*60;
$yest_dt= date("Y-m-d",$yest_dt_ts);
$end_dt= $yest_dt." 23:59:59";
$end_dt_frt= date("dMy",$yest_dt_ts);
...
if(mysql_num_rows($res))
{
	...
	send_email("analytics@naukri.com"," Keyword profile detail report in csv format for $st_frmt to $end_frmt ","Keyword Profile csv","noreply@jeevansathi.com","puneet.makkar@jeevansathi.com,neha.verma@99acres.com","",$data,"","$yest_frmt.csv");
...
{
	mail("puneet.makkar@jeevansathi.com,neha.verma@99acres.com","Error in keyword profile detail cron csv for last 15 days",mysql_error($db));
}
