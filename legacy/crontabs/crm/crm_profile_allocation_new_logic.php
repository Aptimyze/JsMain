<?php 
 ...
 include_once("/usr/local/scripts/DocRoot.php");
	...
	while ($row_branches = mysql_fetch_array($res_branches))
	{
		...
		if($city_str)
		{
			if($city_res !='WB05' && $city_res !='PH00' && $city_res !='AP03' && $city_res !='TN02' && $city_res !='KE03' && $branch!="MUMBAI" && $branch!="PUNE")
			{
				...
				while($row = mysql_fetch_array($res))
				{
					...
					$sql_jp = "SELECT PHONE_RES,PHONE_MOB,CITY_RES from newjs.JPROFILE WHERE ACTIVATED IN ('Y','H') AND INCOMPLETE='N' AND PROFILEID ='$profileid' AND SUBSCRIPTION='' AND CITY_RES IN ($city_str) AND LAST_LOGIN_DT>=DATE_SUB(CURDATE(), INTERVAL 30 DAY)";
			...
			$sql = "SELECT PROFILEID,CITY_RES FROM incentive.MAIN_ADMIN_POOL WHERE CITY_RES IN ($city_str) AND ALLOTMENT_AVAIL ='Y' AND TIMES_TRIED < 3 AND CUTOFF_DT=now() ORDER BY ANALYTIC_SCORE DESC";
			$res = mysql_query($sql) or die("$sql".mysql_error());
			while($row = mysql_fetch_array($res))
			{
				$profileid = $row['PROFILEID'];
				$city_profile = $row['CITY_RES'];
				$sql_jp = "SELECT PHONE_RES,PHONE_MOB from newjs.JPROFILE WHERE ACTIVATED IN ('Y','H') AND INCOMPLETE='N' AND PROFILEID ='$profileid' AND SUBSCRIPTION='' AND CRM_TEAM='online' AND LAST_LOGIN_DT>=DATE_SUB(CURDATE(), INTERVAL 30 DAY)";
