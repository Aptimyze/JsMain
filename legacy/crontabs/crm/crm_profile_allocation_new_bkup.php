<?php 
 ...
 include_once("/usr/local/scripts/DocRoot.php");
	...
	while ($row_branches = mysql_fetch_array($res_branches))
	{
		...
		if($city_res == 'KA02')
			$sql_cities = "SELECT VALUE FROM incentive.BRANCH_CITY WHERE (PRIORITY='$city_res' OR VALUE='AP03')";
		...
		if($city_str)
		{
			...
			while($row = mysql_fetch_array($res))
			{
				...
				$sql_jp = "SELECT PHONE_RES,PHONE_MOB from newjs.JPROFILE WHERE ACTIVATED IN ('Y','H') AND INCOMPLETE='N' AND PROFILEID ='$profileid' AND SUBSCRIPTION='' AND SOURCE !='onoffreg' AND LAST_LOGIN_DT>=DATE_SUB(CURDATE(), INTERVAL 45 DAY)";
