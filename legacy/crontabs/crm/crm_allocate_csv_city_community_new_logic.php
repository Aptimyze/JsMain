<?php 
 ...
 include_once("/usr/local/scripts/DocRoot.php");
	...
	include("allocate_functions_new_logic.php");
	...
	$header="\"PROFILEID\"".","."\"PHONE_NO1\"".","."\"PHONE_NO2\"".","."\"CITY\"".","."\"PHOTO\"".","."\"CONTACTS_INITIATED\"".","."\"CONTACTS_ACCEPTED\"".","."\"CONTACTS_RECEIVED\"".","."\"ACCEPTANCE_RECEIVED\"".","."\"AGE\"".","."\"POSTEDBY\"".","."\"GENDER\"".","."\"CASTE\"".","."\"COMMUNITY\"".","."\"PROFILELENGTH\"".","."\"DESIRED_PARTNER_PROFILE\"".","."\"LAST_LOGIN_DATE\"".","."\"ANALYTIC_SCORE\"".","."\"PROFILE_AGE\"".","."\"MSTATUS\"".","."\"INCOME\"".","."\"LEAD_ID\"".","."\"PHONE_1\"".","."\"PHONE_2\"".","."\"DIAL_STATUS\"".","."\"PRIORITY\"\n";
        ...
        $filename1 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_south_tech.txt";
        $filename2 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_dncr_tech.txt";
        $filename3 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_pune_tech.txt";
        $filename4 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_west_tech.txt";
        $filename5 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_roi_tech.txt";
        $filename6 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_failed_payments_tech.txt";
	...
	$sql="insert into incentive.TEMP_CSV_PROFILES_TECH (PROFILEID, SCORE, MTONGUE) select PROFILEID, ANALYTIC_SCORE, MTONGUE from incentive.MAIN_ADMIN_POOL WHERE ANALYTIC_SCORE != 0 AND CUTOFF_DT=now() AND ALLOTMENT_AVAIL='Y' AND TIMES_TRIED<3 AND MTONGUE <> '1'";
	...
	$sql_pid = "SELECT PROFILEID, SCORE, MTONGUE from incentive.TEMP_CSV_PROFILES_TECH where 1";
	...
	$profileid_file1 = $SITE_URL."/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_south_tech.txt";
	$profileid_file2 = $SITE_URL."/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_dncr_tech.txt";
	$profileid_file3 = $SITE_URL."/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_pune_tech.txt";
	$profileid_file4 = $SITE_URL."/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_west_tech.txt";
	$profileid_file5 = $SITE_URL."/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_roi_tech.txt";
	$profileid_file6 = $SITE_URL."/crm/csv_files/bulk_csv_crm_data_".date('Y-m-d')."_failed_payments_tech.txt";
	...
	$bcc="vibhor.garg@jeevansathi.com,kasa.shirish@naukri.com";
	$sub="Daily CSV for calling New Logic";
