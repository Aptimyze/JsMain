<?php 
 ...
 include_once("/usr/local/scripts/DocRoot.php");
	...
	$south_arr = array('KA02','TN02','AP03','TN04','TN05','AP01','KE02','TN06','KE03','TN09','KA08','KA09','AP07','TN14','TN15','TN16','TN19','KA','KE','KE04','KE06','KE08','TN07','TN17','TN18','TN20');
	//Delhi-NCR
	...
	$header="\"PROFILEID\"".","."\"PHONE NO.(1)\"".","."\"PHONE NO.(2)\"".","."\"CITY\"".","."\"PHOTO\"".","."\"CONTACTS INITIATED\"".","."\"CONTACTS ACCEPTED\"".","."\"CONTACTS RECEIVED\"".","."\"ACCEPTANCE RECEIVED\"".","."\"DATE OF BIRTH\"".","."\"POSTEDBY\"".","."\"GENDER\"".","."\"CASTE\"".","."\"COMMUNITY\"".","."\"PROFILELENGTH\"".","."\"DESIRED PARTNER PROFILE\"".","."\"LAST LOGIN DATE\"".","."\"SCORE\"".","."\"ENTRY_DATE\"".","."\"EVER_PAID\"\n";
        ...
        $filename1 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/test_csv_data_".date('Y-m-d')."_south.txt";
        $filename2 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/test_csv_data_".date('Y-m-d')."_dncr.txt";
        $filename3 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/test_csv_data_".date('Y-m-d')."_pune.txt";
        $filename4 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/test_csv_data_".date('Y-m-d')."_west.txt";
        $filename5 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/test_csv_data_".date('Y-m-d')."_roi.txt";
        $filename6 = "$_SERVER[DOCUMENT_ROOT]/crm/csv_files/test_csv_data_".date('Y-m-d')."_failed_payments.txt";
	...
	$sql = "SELECT DISTINCT PROFILEID FROM billing.ORDERS WHERE STATUS='' AND ENTRY_DT BETWEEN '2008-08-20 14:30:00' AND '2008-08-21 14:30:30' UNION SELECT DISTINCT PROFILEID FROM billing.PAYMENT_HITS WHERE ENTRY_DT BETWEEN '2008-08-20 14:30:00' AND '2008-08-21 14:30:30' AND PAGE>1";
...
//	$bcc="vibhor.garg@jeevansathi.com, shiv.narayan@jeevansathi.com, sriram.viswanathan@jeevansathi.com";
	$to = "sriram.viswanathan@jeevansathi.com";
