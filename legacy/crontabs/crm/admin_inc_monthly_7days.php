...
$sql="SELECT DATE_ADD(newjs.JPROFILE.ENTRY_DT,INTERVAL 7 DAY) as follow_dt,PROFILEID, CITY_RES,PHONE_RES,PHONE_MOB, incentive.BRANCH_CITY.NEAR_BRANCH from newjs.JPROFILE, incentive.BRANCH_CITY where ENTRY_DT between '2006-03-24 00:00:00' and '2006-03-30 23:59:59' and newjs.JPROFILE.CITY_RES=incentive.BRANCH_CITY.VALUE and incentive.BRANCH_CITY.NEAR_BRANCH NOT IN ('','UP25') order by ENTRY_DT,GENDER,AGE"; 
$result=mysql_query($sql) or die("$sql".mysql_error());//(logError($sql);
...
while($myrow2=mysql_fetch_array($result))
{
	...
	for($i=0;$i<$cnt_proid;$i++)
	{
		...
		if($follow_date=='2006-04-02')
		{
			$follow_date='2006-04-03';
		}
