<?php 
 ...
 include_once("/usr/local/scripts/DocRoot.php");
	...
	function write_contents_to_file($profileid,$mtongue="",$score="",$failed_payments="",$DPP=0,$extra_params="",$nri="")
	{
		global $fp1, $fp2, $fp3, $fp4, $fp5, $fp6, $south_arr, $dncr_arr, $pune_city_arr, $west_city_arr, $nri_arr, $db;
		...
		if(!$failed_payments && $extra_params["SERVICE_CALL"] != 1 && !$nri)
			$sql .= " AND ENTRY_DT < DATE_SUB(CURDATE(), INTERVAL 8 DAY) AND MTONGUE <> '1'";
		elseif($nri)
			$sql .= " AND ENTRY_DT < DATE_SUB(CURDATE(), INTERVAL 1 DAY)";
	...
	function minimize_data()
	{
		...
		$db737=connect_737();
                ...
                while($row=mysql_fetch_assoc($res))
                {
                        ...
                        $sql_prof="select PHONE_MOB,PHONE_RES,STD from newjs.JPROFILE where PROFILEID=$profileid and ACTIVATED IN ('Y','H') AND INCOMPLETE='N' AND SUBSCRIPTION='' AND LAST_LOGIN_DT>=DATE_SUB(CURDATE(), INTERVAL 45 DAY)";
