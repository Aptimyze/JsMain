...
{
 	$update_statement = "UPDATE crawler.crawler_JS_competition_caste_values_mapping SET JS_FIELD_VALUE = ".$new_value[$k]." WHERE JS_FIELD_VALUE = ".$v;
//    	echo $update_statement."<br />";
     	$mysqlObjM->executeQuery($update_statement,$dbM) or $mysqlObjM->logError($update_statement);
}
echo "crawler|crawler_JS_competition_caste_values_mapping|JS_FIELD_VALUE\n";

foreach($old_value as $k=>$v)
{
        $update_statement = "UPDATE crawler.crawler_search_results SET CASTE = ".$new_value[$k]." WHERE CASTE = ".$v;
//      echo $update_statement."<br />";
        $mysqlObjM->executeQuery($update_statement,$dbM) or $mysqlObjM->logError($update_statement);
}
echo "crawler|crawler_search_results|CASTE\n";
