 ...
 */
public class GmailMessagePlugin implements Plugin,PacketInterceptor {

	    /**
     * used to send violatietProfileIdnotifications
     */
     ...
     ArrayList<String> gmailindex=new ArrayList<String>();
     public HashMap<String,String> profilefetch=new HashMap<String,String>();
     ArrayList<String> profileindex=new ArrayList<String>();
     ...
     public ConcurrentHashMap<String,Integer> useronline=new ConcurrentHashMap<String,Integer>();
   ...
   public void interceptPacket(org.xmpp.packet.Packet packet,Session session,boolean incoming,boolean processed)   throws PacketRejectedException
   {
			...
			if(packet instanceof Message  && !processed)
			{
				...
				if(js_user)
				{
					...
					if(msg.equals("time_out"))
					{
						...
						throw new PacketRejectedException("timed out");
						
					}
					if(timeExceed(message,gmailID,true))
					{
						throw new PacketRejectedException("time exceed");
						
					}	
					chatMessage(message,gmailID,nick,profileID);
					throw new PacketRejectedException("send message");
				...
				else
				{
					...
					if(msg.length()>8)
					{
						if((msg.substring(0,6)).equals("@block"))
						{
							...
							if(found)
							if(this.chatAcceptListGet(getProfileId(msgFrom,0)))
					...
					else if(msg.equals("@hide") || msg.equals("'@hide'"))
					{
						...
						throw new PacketRejectedException("@hide");
					...
					if(msg.equals("@show") || msg.equals("'@show'"))
					{
						...
						throw new PacketRejectedException("show");
					...
					if(isReceiver!=null)
					{
						if(msg.equals("yes") || msg.equals("'yes'"))
						{
							...
							if(ret==2 || ret==0)
							{
								throw new PacketRejectedException("@yes");
							}
						}
						//Chat declined
						else if(msg.equals("no") || msg.equals("'no'"))
						{
							...
							if(ret==2 || ret==0)
							{
								throw new PacketRejectedException("@no");
							}
						}
						else if(msg.equals("@end") || 	msg.equals("'@end'"))
						{
							...
							{
								sendEndingMes(message,msgFrom);
								throw new PacketRejectedException("@end");
								
							}
							else
							{
								sendYesNoAgain(message);
								throw new PacketRejectedException("@yes@no");
						...
						else
						{
							...
							{
								sendYesNoAgain(message);
								throw new PacketRejectedException("@yesno@noaccept");
					...
					{
						sendAutoMes(message);
						throw new PacketRejectedException("@auto");
					...
					throw new PacketRejectedException("@sendmessage");
			...
			if(packet instanceof Presence  && !processed)
			{
				...
				if(fromJID!=null && toJID!=null && !toJID.getNode().equals("sathi") && !fromJID.getNode().equals("sathi"))
				{
					...
					if(domain.equals("gmail.com"))
					{
						...
						router.route(pp);
						String profid=getProfileId(prsFrom,1);
						//Log.warn(prsFrom+"--------------"+profid);
						if(profid.equals("-1"))
						{
							Presence pp1=new Presence(Presence.Type.unsubscribe);
							pp1.setTo(originalPresence.getFrom());
							pp1.setFrom(originalPresence.getTo());
							router.route(pp1);
							pp1=null;
							return;
						}
	...
	public boolean timeExceed(Message message,String gmailID,boolean mestosend)
	{
		...
		if(this.timelastchatGet(gmailID)!=null)
		{
			...
			if((currenttime-this.timelastchatGet(gmailID))>1800)
			{
				if(mestosend)
				{
					String gmailProfid=getProfileId(gmailID,0);
					...
					message2.setBody("Chat with "+to.getNode()+" has been disconnected because of inactivity.");
	...
	public boolean BlockUnblock(String msgFrom,String block_user, int what)
	{
		...
		try
		{
			...
			sql="select PROFILEID from newjs.JPROFILE where USERNAME=?  and activatedKey=1 limit 1";
			st = conprof.prepareStatement(sql);
			st.setString(1,block_user);
			rs=st.executeQuery();
			...
			sql="select PROFILEID from newjs.JPROFILE where EMAIL=? and activatedKey=1 limit 1";
			st=conprof.prepareStatement(sql);
			...
			if(userID!=null && destinationUserID!=null)
			{
				...
				st.execute();
				DbConnectionManager.closeConnection(conprof); 
				return true;
			}
			DbConnectionManager.closeConnection(conprof); 
	...
	{
		String gmailProfid=getProfileId(msgFrom,0);
	...
	public  void sendEndingMes(Message message,String msgFrom)
	{
		...
		//message2.setBody("You have ended your chat with "+to.getNode());
	...
	public int checkYesNo(Message message,String msgFrom,int type)
	{
		...
		if(receiver!=null)
		{
			...
			else
			{
				...
				JID to=message.getTo();
				if(type==1)
					message2.setBody("You have approved "+to.getNode()+"'s chat request");
				else
					message2.setBody("You have declined "+to.getNode()+"'s chat request");
	...
	public void setON_OFF(int onoff,String msgFrom,Message message)
	{
		...
		try
		{
			...
			st.execute();
			DbConnectionManager.closeConnection(conpres); 
	...
	public void subunsub(String msgFrom,int sub_unsub)
	{
		...
		try
		{
			...
			String profid=getProfileId(msgFrom,0);
			if(profid!=null)
			{
				st=consub.prepareStatement("insert delayed ignore into bot_jeevansathi.SUB_UNSUB set EMAIL ='"+msgFrom+"',SUB_TAKEN='"+sub_unsub+"',`DATE`=now(),`TYPE`='G'");
				st.execute();
			}
			DbConnectionManager.closeConnection(consub); 
		}
		catch(Exception ex)
		{
...
/*
		...
		try
		{
			...
			if(profid!=null)
			{
				...
				st=conOff.prepareStatement("insert delayed ignore into bot_jeevansathi.user_online_new set USER ='"+profid+"'");
				st.execute();	
				DbConnectionManager.closeConnection(conOff); 
			}
		}
		catch(Exception ex)
		{
...
*/
	...
	public void showOnlineOffline(String msgFrom,int onoff)
	{
		...
		try
		{
			...
			{
				useronline.put(profid,new Integer(0));
			...
			{
				useronline.put(profid,new Integer(1));
				//st=conOff.prepareStatement("insert delayed ignore into bot_jeevansathi.user_online set USER ='"+profid+"'");
				//st.execute();
			}
			
			if(useronline.size()>200)
			{
				//Log.warn("size exceeds 200-->"+useronline.size());
				conOff = DbConnectionManager.getConnection();
				Set set= useronline.keySet();
				Iterator iter = set.iterator();
				
				String on="";
				
				Statement stmt = conOff.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
				
				while(iter.hasNext()){
					
				Object keys=iter.next();
				String currentKey = keys.toString();
				
				onoff=useronline.get(currentKey);
				if(onoff==0)
					on="delete from  bot_jeevansathi.user_online where USER ='"+currentKey+"';";
				else
					on="insert delayed ignore into bot_jeevansathi.user_online set USER ='"+currentKey+"'";
					
					stmt.addBatch(on);
					//useronline.remove(currentKey);
				//	Log.warn(useronline+"");
					iter.remove();
				//	Log.warn(useronline+"");
				}
	...
	public void removeEntriesFromHashMap(String gmailID)
	{
		...
		if(receiver!=null)
		{
			String profId=getProfileId(gmailID,0);
	...
	public void chattingWithWhomClear(String gmailID)
	{
		...
		{
			//Removing entry 
			this.chattingWithWhom.remove(gmailID);
	...
	public String chattingWithWhomGet(String gmailID)
	{
		...
		try
		{
			if(this.chattingWithWhom.containsKey(gmailID))
					return this.chattingWithWhom.get(gmailID);
			...
			singleEntry(this.chattingWithWhom,gmailID,profileid);
			DbConnectionManager.closeConnection(conprof);
	...
	public String checkAlreadyTalking(String gmailid,String thread)
	{
		...
		if(receiver!=null)
		{
			if(!this.chatAcceptListGet(getProfileId(gmailid,0)))
	...
	public String getProfileId(String gmail,int fromPresence)
	{
		String profileid=null;
		PreparedStatement st=null;
		ResultSet rs=null;
		...
		else
		{
			...
			try
			{
				...
				st = conprof.prepareStatement("select PROFILEID  from newjs.JPROFILE where EMAIL='"+gmail+"' and activatedKey=1 and ACTIVATED='Y'");
				//st.setString(1, gmail);
				rs=st.executeQuery();
				if(rs.next()){
				//Log.warn(rs.getString("profileID"));	
					...
					DbConnectionManager.closeConnection(conprof);
					insertEntry(this.gmailfetch,this.gmailindex,profileid,gmail);
					insertEntry(this.profilefetch,this.profileindex,gmail,profileid);
	...
	public String getGmailId(String profileid)
	{
		...
		//Log.warn(this.gmailfetch+" "+profileid);
		//if(false)
		...
		else
		{
			...
			try
			{
				...
				DbConnectionManager.closeConnection(congmail);
				insertEntry(this.gmailfetch,this.gmailindex,profileid,gmail);
				insertEntry(this.profilefetch,this.profileindex,gmail,profileid);
				//this.gmailfetch.put(profileid,gmail);
				//this.profilefetch.put(gmail,profileid);
	...
	public void updateLOGCHATREQUEST(String action,int sen_prof,int rec_prof)
	{
		...
		try
		{
			...
			st.execute();
			DbConnectionManager.closeConnection(conlog); 
		}
		catch(Exception e)
		{
    ...
    public void insertEntry(HashMap<String,String>hashmap,ArrayList<String>hashindex,String key,String value)
	{
		int k=0;
		//while(true)
		//{
		//key=key+(int)(Math.random()*10000);
		//Log.warn(key+" "+value+" "+hashmap+" "+hashindex+" "+hashmap.size());
		//Log.warn(hashmap+" ");
		//Log.warn("------------");
		//Log.warn(hashindex+" ");
		//Log.warn("++++++++++++");
		if(hashmap.size()>4000)
		{
			
			Object ia[] = hashindex.toArray(); 
			String ky=(ia[0]).toString();
			//Log.warn("400 reached ---> "+key);
			hashmap.remove(ky);
			hashindex.remove(ky);
		}
		if(key!=null)
		{
		//Log.warn("not found key "+key+" gmail is "+value);
...
*/
    ...
    public void singleEntry(HashMap<String,String>hashmap,String key,String value)
	{
	    if(hashmap.size()>400)
		{
			hashmap.clear();
