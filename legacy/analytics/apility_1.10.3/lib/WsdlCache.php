  ...
  */
  class APIlityWsdlCache {
    ...
    function isDirectoryValid($dir){
      ...
      if(!is_dir($dir)){
        if (!SILENCE_STEALTH_MODE) trigger_error("<br /><b>APIlity PHP library => Warning: </b>Cache Directory: ".$dir." does not exist or is invalid<br/>", E_USER_WARNING);
        return false;
      }
      // can we write to the directory?
      if(!is_writable($dir) && !chmod($dir, 0666)){
        if (!SILENCE_STEALTH_MODE) trigger_error("<br /><b>APIlity PHP library => Warning: </b>Cache Directory: ".$dir." does not have write privileges<br/>", E_USER_WARNING);
        return false;
      }
      return true;
    }
    ...
    function buildCacheFile() {
      $cacheHandler = fopen($this->cacheFile, "w");
	    ...
	    // content length is not return when using gzip (disabled for now)
      ...
      // check response code
      if($responseCode != 200) {
      	if (!SILENCE_STEALTH_MODE) trigger_error("<br /><b>APIlity PHP library => Warning: </b>Request for file (".$this->url.") was invalid, HTTP code: ".$responseCode."<br/>".curl_error($curlConnection), E_USER_WARNING);
        curl_close($curlConnection);
        return false;
      }
      // check file size
      if(filesize($this->cacheFile) != $contentLength){
        if (!SILENCE_STEALTH_MODE) trigger_error("<br /><b>APIlity PHP library => Warning: </b>Cache file for: ". $this->url ."has an invalid file size<br/>", E_USER_WARNING);
        curl_close($curlConnection);
        return false;
      }

      if (curl_errno($curlConnection)) {
        if (!SILENCE_STEALTH_MODE) trigger_error("<br /><b>APIlity PHP library => Warning: </b>Error while trying to cache wsdl file. The cURL error message is: ".curl_error($curlConnection)."<br/>", E_USER_WARNING);
        curl_close($curlConnection);
        return false;
      }
