...
//		$sql="SELECT PROFILEID, USERNAME, MOD_DT, SUBSCRIPTION from newjs.JPROFILE where MOD_DT > '$lastweek_date' and ACTIVATED='Y' and SCREENING != $sum";
		$sql="SELECT PROFILEID, USERNAME, MOD_DT, SUBSCRIPTION from newjs.JPROFILE where ACTIVATED='Y' and SCREENING != $sum";
		$result=mysql_query_decide($sql);
...
*/                                                                                 
	...
	else
	{
		...
		$sum=setAllFlags();
		$sql="SELECT  newjs.JPROFILE.PROFILEID, newjs.JPROFILE.USERNAME, MOD_DT, SUBSCRIPTION FROM newjs.JPROFILE LEFT JOIN jsadmin.MAIN_ADMIN ON newjs.JPROFILE.PROFILEID = jsadmin.MAIN_ADMIN.PROFILEID WHERE (jsadmin.MAIN_ADMIN.PROFILEID IS NULL) AND (ACTIVATED = 'Y' AND SCREENING != '8191' ) ORDER BY MOD_DT"; 
		$result=mysql_query_decide($sql);
