...
if($data)
{
		...
		if($users != '')
		{
			...
			if($count1==$count2)
			{
				$sql="TRUNCATE TABLE jsadmin.UPSELL_AGENT";
                                $res1=mysql_query_decide($sql) or die("$sql".mysql_error_js());
				$sql="INSERT INTO jsadmin.UPSELL_AGENT VALUES ('$userlist')";
				$res1=mysql_query_decide($sql) or die("$sql".mysql_error_js());
				$smarty->assign("msg","List updated.");
			}
			else
				$smarty->assign("msg","You have entered invalid executive name(s).<br><a href=\"$SITE_URL/jsadmin/upsell_agent_list.php?name=$user&cid=$cid\">Back</a>");
		}
		else
			$smarty->assign("msg","You have entered invalid executive name(s).<br><a href=\"$SITE_URL/jsadmin/upsell_agent_list.php?name=$user&cid=$cid\">Back</a>");
	}		
	...
	$smarty->assign("name",$name);
        ...
        unset($Submit);
        $smarty->display("upsell_agent_list.htm");
}
?>
