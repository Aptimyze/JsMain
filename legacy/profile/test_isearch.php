...
{*/
	...
	for($i=4;$i<$all;$i++)
	{
		$sql="CREATE TEMPORARY TABLE $t SELECT COUNT(*) as cnt, j.$choice[$i] FROM newjs.CONTACTS c, $d j WHERE j.PROFILEID=c.RECEIVER AND c.SENDER='$id' GROUP BY $choice[$i] ORDER BY cnt DESC";
		$res=mysql_query_decide($sql,$db) or logError("Error while 1st insertion ".mysql_error_js(),$sql);

		$sql="INSERT INTO $t SELECT COUNT(*) AS cnt, j.$choice[$i] FROM newjs.CONTACTS c, $d j WHERE j.PROFILEID=c.SENDER AND c.TYPE='A' AND c.RECEIVER='$id' GROUP BY $choice[$i] ORDER BY cnt DESC";
		...
		*/
		...
		else
		{
			...
			{
				$cnt=$temp['cnt'][$ii];
                                $grandtotal+=(($cnt/$totalofcount)*100)/$ii;
			}
			$k--;
					/*if($choice[$i]=="COUNTRY_RES")
					{
						//echo $name." = ";
						//echo $cnt."<br>";
					}
			...
			/*$firstpercent=$temp['cnt'][0];
			$secondpercent=$temp['cnt'][1];
			$two=0;
			if($k>1)
			{	
				$firstpercent=($temp['cnt'][0]/$grandtotal)*100;
	                        $secondpercent=($temp['cnt'][1]/$grandtotal)*100;
				$vivek=($secondpercent+$firstpercent)/$k;
				echo "<br>firstpercent=".$firstpercent."<br>";
				echo "<br>secondpercent=".$secondpercent."<br>";
			}
			else 
			{
				$firstpercent=($temp['cnt'][0]/$grandtotal)*100;
				$vivek=$firstpercent/$k;
			}*/
			if($choice[$i]=="CASTE")
			{
				echo "k=".$k."<br>";
				echo "CASTE=".$grandtotal."<br>";
			}
			else if($choice[$i]=="COUNTRY_RES")
			{
				echo "k=".$k."<br>";
				echo "COUNTRY_RES=".$grandtotal."<br>";
			}
			/*for($j=0;$j<$k;$j++)
			{
...
//		if(is_array($incomev))$income="'".implode(",",$incomev)."'";
		...
		while($m['CASTE'][$i+1]!=0)
		{
			...
			$sql="INSERT INTO search_intel.ISCASTE VALUES('$id','".$m['CASTE'][$i]."') ";
