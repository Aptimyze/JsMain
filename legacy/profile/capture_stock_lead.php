<?php
/******************************************************************************************************************
       Filename        :       capture_stock_lead.php
       Description     :       script to capture the All the fields of the users required for the stock trading 
			       banner & mailer.
...
include("connect.inc");
connect_db();
//Auto login of the user while user will go to the stock trading entry form.
if($Submit)//If user submits the form
{
	...
	if($error == 0)
	{
		...
		$sql="replace into newjs.STOCK_TRADING_MAIL (PROFILEID,USERNAME,NAME,SURNAME,PINCODE,OPTION_SELECT,SOURCE,EMAIL_ID,RES_NO,MOB_NO,AGE,GENDER,CITY,OCCUPATION,INCOME,ENTRY_DATE,CLICK_SOURCE) values ('$profileid','$username','$name','$sname','$pincode','$OPTION_SELECT','$SOURCE','$email','$residence','$mobile','$age','$gender','$city','$occupation','$income','$today','$click_source_label')";
		mysql_query_decide($sql) or die(mysql_error_js());
		$smarty->assign("Done",1);
		$smarty->display("stock_trading_mailer.htm");//To display the Thank you page
	}
	else
	{
		...
		$smarty->display("stock_trading_mailer.htm");	
	}
}   
else   //If coming first time to this page
{
	...
	$smarty->display("stock_trading_mailer.htm");//First display
}
?>
