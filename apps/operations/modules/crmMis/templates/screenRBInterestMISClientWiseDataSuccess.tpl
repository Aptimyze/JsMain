...
<table width=100% align=center border=0 cellspacing=2 cellpadding=5>
    ...
    <tr></tr>
    <tr class=formhead style="background-color:LightSteelBlue">
        <td align=center style="background-color:LightSalmon">Client ID</td>
        ~foreach from=$daysArray key=k item=val`
            <td align=center>~$val`</td>
        ~/foreach`
        <td align=center>TOTAL</td>
    </tr>
    ~foreach from=$clientWiseDetails key=k item=v`
        <tr class=formhead>
            <td align=center>
                ~if $k neq 'TOTAL'`
                <a href="~sfConfig::get('app_site_url')`/operations.php/crmMis/screenRBInterestMISClientInfo?cid=~$cid`&name=~$name`&startDT=~$startDT`&endDT=~$endDT`&clientID=~$k`&pid=~$profileIDMapping[$k]`">
                    ~$k`
                </a>
                ~else`
                    ~$k`
