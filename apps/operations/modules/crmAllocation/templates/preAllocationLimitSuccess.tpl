	  	...
	  	<script language="javascript">
			function popLimit(thisform){
				...
				if(centerName){
					...
					~foreach from=$localityLimitArr item=limitVal key=centerKey`
...
~include_partial('global/header')`
	<form name="form1" action="~sfConfig::get('app_site_url')`/operations.php/crmAllocation/preAllocationLimit" method="post">
	...
	<input type="hidden" name="cid" value="~$cid`">
		...
		<br>
				...
				<select name="locality" onChange="popLimit(this);">
					...
					~foreach from=$localityLimitArr item=limitVal key=localityName`
		...
		<tr>
			<td align="center" width="20%">
				~if $errorLimit`
					<font color="red">Please enter valid integer value</font>
				~/if`
				~if $errorRange`
					<font color="red">Max value allowed is 120</font>
				~/if`
