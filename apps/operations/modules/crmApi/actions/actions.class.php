...
class crmApiActions extends sfActions
{
  ...
  */
  ...
  /*Api to fetch nearest branch details of profile based on city.
    * @inputs: $request
    * @return: api response
    */
    public function executeSmsNearestBranchDetailsV1(sfWebRequest $request){
        ...
        if($profileid){
            ...
            $respObj->setHttpArray(CrmResponseHandlerConfig::$MISSING_PARAM);
            $output = array("response"=>"Parameter missing");
        }
        $respObj->setResponseBody($output);
        $respObj->generateResponse();
        die();
