	...
	$requestUri =$_SERVER["REQUEST_URI"];
		...
		elseif($request->getParameter('module')=="common" && $request->getParameter('action')=="resetPassword")
		{
			...
			if(($request->getParameter('module')=="homepage" && $request->getParameter('action')=="index" )|| ($request->getParameter('module')=="myjs" && $request->getParameter('action')=="jspcPerform" ) || $request->getParameter("blockOldConnection500")){
				...
				if(strstr($_SERVER["REQUEST_URI"],"api/v1/social/getAlbum") || strstr($_SERVER["REQUEST_URI"],"api/v1/social/getMultiUserPhoto") || strstr($requestUri,"api/v1/notification/poll") || strstr($requestUri,"api/v1/search/gunaScore")  || strstr($requestUri,"common/resetStaticKey") || strstr($requestUri,"/register/staticTablesData") || HandlingCommonReqDatabaseId::isMasterMasterDone() || strstr($requestUri,"/api/v1/api/hamburgerDetails") || strstr($requestUri,"/api/v2/common/engagementcount") || strstr($requestUri,'/api/versionupgrade'))
					JsCommon::oldIncludes(false);
				else
