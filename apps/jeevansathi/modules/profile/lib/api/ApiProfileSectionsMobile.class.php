...
class ApiProfileSectionsMobile extends ApiProfileSections{
	...
	public function getApiAstroKundali() {
		...
		$astro[MANGLIK][outerSectionValue]=FieldMap::getFieldLabel("manglik_status_ascii",ord($this->profile->getMANGLIK()));
		$astro[MANGLIK][singleKey]=1;
		$astro[MANGLIK][OnClick][]=$this->getApiFormatArray("MANGLIK","Manglik" , $astro[MANGLIK][outerSectionValue],$this->profile->getMANGLIK(),$this->getApiScreeningField("MANGLIK"),$this->dropdown);
		/*
...
//			$basicArr[basic][OnClick][]=$this->getApiFormatArray("NAME","Bride's Name"  ,$name,"",$this->getApiScreeningField("NAME"),$this->text);
	...
	public function getApiDppReligionAndEth() {
		...
		//Manglik
		$szManglik = $this->getDecorateDPP_Response($jpartnerObj->getPARTNER_MANGLIK());
		$DppBasicArr["Religion"][OnClick][] = $this->getApiFormatArray("P_MANGLIK","Manglik",trim($jpartnerObj->getDecoratedPARTNER_MANGLIK()),$szManglik,$this->getApiScreeningField("PARTNER_MANGLIK"),$this->dropdown,'',1);
		return $DppBasicArr;		
	}
