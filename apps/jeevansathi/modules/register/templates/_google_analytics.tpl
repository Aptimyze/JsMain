...
~if $REGISTRATION`
<script type="text/javascript" src="~$IMG_URL`/minify.php?files=/js/time-tracker.js"></script>
<script type="text/javascript">
	var time_trac_count=0;
	function ga_time_track(){
		var timeTracker = new TimeTracker(); 
		if(window._gat){
		var pageTracker = _gat._getTracker('UA-179986-1'); 
		$(document).ready(function() {
		$('form[name=form1]').find('input, select, textarea, radio').each(function() { 
		...
		timeTracker._track(pageTracker, $(this).attr('name'), 'RegTest');
		});
		});
		});
		}
		else{
			if(time_trac_count++ <20)
