...
class membershipActions extends sfActions
{
    ...
    public function executeJsexclusiveDetail(sfWebRequest $request) {
        ...
        $server_day = date('j M');
        if (date('H') >= 21) $server_day = date('j M', strtotime('+1 day', strtotime($server_day)));
        
        $next_1 = date('j M', strtotime('+1 day', strtotime($server_day)));
        $next_2 = date('j M', strtotime('+2 day', strtotime($server_day)));
        $this->dropDownDayArr = array(
            $server_day,
            $next_1,
            $next_2
        );
        $this->dropDownTimeArr1 = array(
            "9 AM",
            "10 AM",
            "11 AM",
            "12 PM",
    ...
    public function executeMobilePaymentOptions(sfWebRequest $request) {
        ...
        if ($currency == "DOL") {
            ...
            if ($this->profileid) $this->courier_visible = $payHandlerObj->getPickup($this->profileid);
        }
        if ($this->currency == "RS") $this->matriPrice = VariableParams::$matriProfilePriceRS;
        else $this->matriPrice = VariableParams::$matriProfilePriceDOL;
        $userObj->setIpAddress($ipAddress);
        $userObj->setCurrency($this->currency);
        if ($this->profileid != '') {
            $userObj->setMemStatus();
        ...
        if ($request->getPostParameter("mainSubMemId")) {
            ...
            if (!$request->getPostParameter("showAllToPayment")) $vasImpression = substr($vasImpression, 0, 5);
        ...
        if ($mainMemDuration == 1188) $mainMemDuration = 'L';
