...
export default class register extends React.Component {
  ...
  componentDidMount() {
    ...
    if (currentRegPage) {
      ...
      if (currentRegPage < 1) {
        // handle -1
        // console.log('ga',"P", `JSMS_REG_S1`);
        this.GAObject.regTrackGA("P", `JSMS_REG_S1`);
        this.GAObject.regTrackGA("P", `JSMS_REG_S1_R`);
      } else if (num <= 5) {
        // console.log('ga',"P", `JSMS_REG_S${num}` );
        this.GAObject.regTrackGA("P", `JSMS_REG_S${num}`);
        this.GAObject.regTrackGA("P", `JSMS_REG_S${num}_R`);
      }
    ...
    window.onpopstate = () => {
      ...
      if (hamDetails && hamDetails.showRegHamburger) {
        ...
        if (slideNum === -1) {
          ...
          // console.log('ga',"P", `JSMS_REG_S${slideNum + 1}`);
          this.GAObject.regTrackGA("P", `JSMS_REG_S${slideNum + 1}`);
          this.GAObject.regTrackGA("P", `JSMS_REG_S${slideNum + 1}_R`);
  ...
  setPages(currentPage, pagenumber) {
    ...
    // console.log('ga--',"P", `JSMS_REG_S${+pagenumber + 1}`);
    this.GAObject.regTrackGA("P", `JSMS_REG_S${+pagenumber + 1}`);
    this.GAObject.regTrackGA("P", `JSMS_REG_S${+pagenumber + 1}_R`);
    if (currentPage == 'nextPage') {
      ...
      // console.log('ga',"E", "jsms",`regPageNavigations${+pagenumber}_s${+pagenumber + 1}`,`time_${timeDiff}`);
      this.GAObject.regTrackGA("E", "jsms", `regPageNavigations${+pagenumber}_s${+pagenumber + 1}`, `time_${timeDiff}`);
      this.GAObject.regTrackGA("E", "jsms", `regPageNavigation_Rs${+pagenumber}_s${+pagenumber + 1}`, `time_${timeDiff}`);
  ...
  constructAndPushUrl(number, ham) {
    ...
    if(!ham){
      ...
      let screenLandingTracking =`/register/screenLandingTracking?screenNumber=s${+num+1}&regMode=R&curParams=${curParams}`;
      commonApiCall(screenLandingTracking, {}, '', 'POST', '', false).then((response) => {
        // console.log("res",response);

      });
      if(num===0) screenTrack('registration', window.location.pathname);
    }
    let trackParams = getItem('trackParams');
    if(trackParams){
      if (!trackParams.hasOwnProperty('source')) {
    ...
    let completePath = this.props.location ? this.props.location.pathname : '/register/page1';
      ...
      <div id="main-content">
        ...
        <Slider ref={slider => (this.slider = slider)} {...settings}>
          <div className="fw bg4 slickContainer" id="createProfile">
            <CreateProfilePage onIconClick={this.handleCurrentStep}/>
          </div>
