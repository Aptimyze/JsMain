...
$sql="select PROFILEID from SEARCH_MALE where LAST_LOGIN_DT < DATE_SUB(CURDATE(), INTERVAL 2 MONTH)";
//$sql="select PROFILEID from SEARCH_MALE where SORT_DT < DATE_SUB(CURDATE(), INTERVAL 2 MONTH)";
...
$sql="select PROFILEID from SEARCH_FEMALE where LAST_LOGIN_DT < DATE_SUB(CURDATE(), INTERVAL 2 MONTH)";
//$sql="select PROFILEID from SEARCH_FEMALE where SORT_DT < DATE_SUB(CURDATE(), INTERVAL 2 MONTH)";
$result=mysql_query($sql) or logError("Due to a temporary problem your request could not be processed. Please try after a couple of minutes",$sql);
while($myrow=mysql_fetch_array($result))
{
        $deletePidArr[]=$myrow["PROFILEID"];
