...
include($curFilePath."../web/P/connect.inc");
include($curFilePath."../web/P/contact.inc");
...
for($serverId=0;$serverId<$noOfActiveServers;$serverId++)
{
	...
	$sql="(SELECT DISTINCT(SENDER) from newjs.CONTACTS,newjs.PROFILEID_SERVER_MAPPING where TIME between '$date1' AND '$date2' AND PROFILEID=SENDER AND SERVERID='$serverId') union (SELECT DISTINCT(RECEIVER) from newjs.CONTACTS,newjs.PROFILEID_SERVER_MAPPING where TIME between '$date1' AND '$date2' AND PROFILEID=RECEIVER AND SERVERID='$serverId')";
	//test
	//echo	$sql="SELECT DISTINCT(SENDER) from newjs.CONTACTS WHERE CONTACTID<155684 LIMIT 10";
	//test
	$res_main=mysql_query($sql,$myDb)or die(mysql_error($myDb).$sql);
	$counter=0;
	while($row0=mysql_fetch_array($res_main))
	{
		$my_profileid=$row0[0];
		...
		$my_age=$row1['AGE'];
		$my_inc=getSortByIncome($row1["INCOME"]);
...
function Spam_Checker($my_profileid,$username,$db,$parameters,$my_gender,$dbM,$myage,$myincome,$myheight,$inactivityPeriodThreshold)
{
	...
	if($receiverIdsI)
	{
		...
		while($row4=mysql_fetch_array($res4))
		{
			...
			$income[]=getTrendsBucket($myincome,getSortByIncome($income_var));
	...
	if($needInitialValue==1 && ($i_val_date!='0000-00-00'))
	{
		$again=(date('U', strtotime(date("Y-m-d"))) - date('U', strtotime($i_val_date))) / 86400;
		if($again>30)
			$i_val_date='';
	}
