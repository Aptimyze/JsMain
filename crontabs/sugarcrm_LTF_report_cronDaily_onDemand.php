...
mysql_query("set session wait_timeout=2000",$db);
	//************************************    Condition after submit state  ***************************************
		...
		$regDurationSet1 ='2016-09-01';
		$regDurationSet2 ='2016-09-01';
		$lastDuration1   ='2016-09-01';
		$lastDuration2   ='2016-09-01';

		// Activated Profiles
                $sql_act ="SELECT distinct(PROFILEID),SCREENED_TIME from jsadmin.SCREENING_LOG WHERE SCREENED_TIME>='$lastDuration1 00:00:00' AND SCREENED_TIME<='$lastDuration2 23:59:59' ORDER BY ID ASC";
		...
		// Photo condition
		$sql_photo ="select distinct(PROFILEID),SUBMITED_TIME from jsadmin.MAIN_ADMIN_LOG where SCREENING_TYPE='P' AND STATUS LIKE '%APPROVED%' AND STATUS NOT LIKE '%EDITIING%' AND SUBMITED_TIME>='$lastDuration1 00:00:00' AND SUBMITED_TIME<='$lastDuration2 23:59:59' ORDER BY ID ASC";
		...
		mail("manoj.rana@naukri.com","LTF Daily Cron - PHOTO", "$duration2 # $duration3");
                ...
                $sql ="select EXECUTIVE,PROFILEID,DATE,count(*) as CNT from MIS.LTF WHERE TYPE IN('EOI','REG') GROUP BY PROFILEID HAVING CNT<'2' AND DATE>'$regDurationSet1 00:00:00' AND DATE<='$regDurationSet2 23:59:59' ORDER BY DATE ASC";
                ...
                mail("manoj.rana@naukri.com","LTF Daily Cron - FIRST EOI SENT", "$duration3 # $duration4");
die('die');
		...
		while($row =mysql_fetch_array($sql_res))
		{
			...
			if($addr_row=mysql_fetch_array($addr_res))
			{
                ...
                mail("manoj.rana@naukri.com","LTF Daily Cron - EMAIL_ADDRESS", "$duration4 # $duration5");

                // Phone Verification Block
                $sql_j ="SELECT PROFILEID,MOB_STATUS,LANDL_STATUS from newjs.JPROFILE where MOD_DT>='$lastDuration1 00:00:00' AND MOD_DT<='$lastDuration2 00:00:00' AND activatedKey=1";
		...
		mail("manoj.rana@naukri.com","LTF Daily Cron - PHONE", "$duration5 # $duration6");
