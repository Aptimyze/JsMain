...
$lastIndexOn = $memcacheObj->get("SOLR_LAST_INDEXED_ON");
if (!$lastIndexOn) {
        $url = JsConstants::$solrServerUrls[0] . "/admin/luke?fl=text&numTerms=1&wt=phps";
        $lastIndexData = unserialize(CommonUtility::sendCurlGetRequest($url));
        $lastIndexOn = str_replace(array("T", "Z"), array(" ", ""), $lastIndexData["index"]["lastModified"]);
}
...
if ($type == "DELTA" || $type == "FULL") {
        $timeNow = date("Y-m-d H:i:s",strtotime("now"));
        $memcacheObj->set("SOLR_LAST_INDEXED_ON", $timeNow, 18000); // set for 5 hour
}
