...
$sql ="SELECT COUNT(*) cnt,A.SERVICEID,B.CENTER FROM billing.SERVICE_STATUS AS A JOIN billing.PURCHASES AS B ON A.BILLID=B.BILLID WHERE B.ENTRY_DT BETWEEN '$st_date' AND '$end_date' AND B.STATUS='DONE' GROUP BY A.SERVICEID,CENTER";
...
if(count($dataSet)>0){
	foreach($dataSet as $key=>$branchVal){
		foreach($branchVal as $key1=>$sidVal){
			...
			$sql_insert="insert into MIS.SERVICE_DETAILS(`ENTRY_DT`,`COUNT`,`SERVICE`,`BRANCH`,`FREE_COUNT`,`PAID_COUNT`) VALUES('$entryDate','$newCnt','$key1','$key','$freeCnt','$paidCnt')";
