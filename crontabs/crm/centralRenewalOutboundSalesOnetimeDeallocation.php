...
$processName 	='HOTFIX-JSC-1917';
$deAllocatedBy	='SYSTEM';

$usernameArr =array('ashish.oza','ashwini.kadam','poonam.yadav','rahul.sharan','ruchita.gupta','rutuja.duduskar','sandeep.sharma','siraj.khan','sudeepta.m','suman.shukla','dhaval.challa','pradnya.salvi','shabnam.khan','vijay.prajapati','vikas.kambale','vyankatesh.u');
$usernameStr ="'".implode("','",$usernameArr)."'";
$sql ="SELECT USERNAME from jsadmin.PSWRDS where USERNAME IN($usernameStr)";
$res = mysql_query($sql) or logError($sql);
while($row = mysql_fetch_array($res))
{
	$agentName	=trim($row['USERNAME']);
