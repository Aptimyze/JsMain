...
mysql_query('set session wait_timeout=1000000,interactive_timeout=100000,net_read_timeout=100000',$shDb3);
...
for($t=0;$t<count($modelType_arr);$t++)
{
	...
	$sql = "SELECT DISTINCT(PROFILEID) FROM test.ANALYTIC_SCORE_POOL WHERE MODEL='$modelType' AND SCORE IS NULL AND PROFILEID%6=5";
        ...
        if(count($modelArr)>0)
        {
				...
				if($modelType=="P")
					...
					$shard = ($profileid%3)+1;
                                        ...
                                        $scorevars = new Scoring($profileid,$myDb,$shDb3,$parameter,$modelType,$shard);
