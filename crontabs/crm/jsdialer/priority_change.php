...
if(count($camp_array)>0)
{
	for($i=0;$i<count($camp_array);$i++)
	{
		...
		if($campaign_name == 'JS_RENEWAL')
			...
			$squery1 = "SELECT easycode,old_priority,PROFILEID,AGENT,VD_PERCENT,SCORE FROM easy.dbo.ct_$campaign_name where Lead_Id IN ('noida$suffix','mumbai$suffix','delhi$suffix','noidaNew$suffix') AND PROFILEID IN ('$pro_str') AND Dial_Status!=0";
