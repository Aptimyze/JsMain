(function (win) {
    ...
    function getNotification(title, options) {
        ...
        if (win.Notification) { /* Safari 6, Chrome (23+) */
            notification =  new win.Notification(title, {
                ...
                icon: isString(options.icon) ? "/images/jspc/CRM/"+options.icon : "/images/jspc/CRM/"+options.icon.x32,
                ...
                */
                tag: options.data.tag || emptyString,
                data:options.data || emptyString
            });
           
            notification.onclick = function(e) { 
                window.open(notification.data.ONCLICKLINK,'targetWindow','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=800, height=800');
            };
            //setTimeout(notification.close.bind(notification), settings.autoClose);
    ...
    function createNotification(title, options) {
         ...
         */
        if (isSupported && isString(title) && (options && (isString(options.icon) || isObject(options.icon)))) {
            if(permissionLevel() === PERMISSION_GRANTED)
                notification = getNotification(title, options);
            else
            {
                requestPermission(function() {notification = getNotification(title, options);});
            }
        }
