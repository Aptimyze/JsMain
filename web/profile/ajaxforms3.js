...
function get2ndPage()
{
	if(flag_2nd)
	{
		...
		launch("/profile/ip_new_2.php?cfrom=page1","get",2);
		flag_2nd=false;
	}
	return;
}
...
function get3rdPage()
{
	...
	{
		//var someDelay=20000;
                //setTimeout('testing1()', someDelay);
		launch("/profile/ip_new_3.php","get",3);
		flag_3rd=false;
	}	
}
function Submit3()
...
function get4thPage()
{
	...
	{
		//var someDelay=20000;
                //setTimeout('testing2()', someDelay);
		launch("/profile/ip_new_4.php?gender="+gender+"&maritalstatus="+mstatus,"get",4);
		flag_4th=false;
	}
}
function Submit4()
...
{
	systemP();
	if((browser=="MSIE" && version<6) || document.form1.ajax_google.value=="1")
		return true;
...
function launch(url, method,getpage)
{
...
function parseInfo()
{
		...
		if((gp==2 && f_2) || (gp==3 && f_3) || (gp==4 && f_4))
		{
			document.getElementById('my_div').innerHTML = answer;
			answer="Loading";
		}
		if(f_2)f_2=false;
		else if(f_3)f_3=false;
		else if(f_4)f_4=false;
        }
}
function makeObject()
{
...
function checkemailn(field)
{
	...
	once=false;}
	var result = false;
	if((/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/.test(emailadd)))
        {
                result=true;
        }
	if(result == false)
