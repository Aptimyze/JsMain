...
include_once(JsConstants::$docRoot."/commonFiles/comfunc.inc");
	include_once($path."/billing/comfunc_sums.php");
...
$announce_to_email = "aman.sharma@jeevansathi.com";
$DOL_CONV_RATE=41;
...
function getProfileDetails($profileid)
{
	...
	if(strstr($myrow["SUBSCRIPTION"],"F") && ($numrows > 0))
	$data["ACTIVE"] = "YES";
	else
	$data["ACTIVE"] = "NO";
	...
	if($myrow["COUNTRY_RES"]=="51")
                $city_res=label_select("CITY_INDIA",$myrow["CITY_RES"]);
        elseif($myrow["COUNTRY_RES"]=="128")
                $city_res=label_select("CITY_USA",$myrow["CITY_RES"]);
        else
                $city_res[0]="";
...
//			$serve_for[]=strtoupper(substr($serve_paid[$i],0,2));		
	...
	if(count($serve_for)>0)
		$serve_for_str[]=implode(",",$serve_for);
	if(count($service_addon_id_arr)>0)
		$serve_for_str[]=implode(",",$service_addon_id_arr);
...
function newOrder($profileid,$paymode,$curtype,$amount,$service_str,$service_main,$discount,$setactivate,$gateway='',$discount_type='')
{
//	echo $profileid."-".$paymode."-".$curtype."-".$amount."-".$service_str."-".$service_main."-".$discount."-".$setactivate;
	...
	if($profileid>0)
	{
		...
		if($gateway!="PAYPAL")
		{
			if($curtype=="DOL" && $gateway!="PAYSEAL")
				$data["AMOUNT"] =round(($data["AMOUNT"]* $DOL_CONV_RATE),2); //convert USD value into INR value 
		}
		...
		$sql1 = "insert into billing.ORDERS (PROFILEID, USERNAME, ORDERID, PAYMODE, SERVICEMAIN, CURTYPE,SERVEFOR, AMOUNT, ENTRY_DT, EXPIRY_DT, BILL_ADDRESS, PINCODE, BILL_COUNTRY, BILL_PHONE, BILL_EMAIL, IPADD,ADDON_SERVICEID,DISCOUNT,SET_ACTIVATE,GATEWAY, DISCOUNT_TYPE) values ('$profileid', '" . addslashes($data[USERNAME]) . "', '$ORDERID', '$paymode', '$service_main','$curtype','$servefor', '$data[AMOUNT]', NOW(), DATE_ADD('$activate_on',INTERVAL ".$service_detail["DURATION"]." MONTH), '".addslashes(stripslashes($data[CONTACT]))."', '".addslashes(stripslashes($data[PINCODE]))."', '".addslashes(stripslashes($data[COUNTRY]))."', '$data[PHONE]', '$data[EMAIL]','$ip','$addon_serviceid','$discount','$setactivate','$gateway','$discount_type')";
...
function start_service($orderid)
{
	...
	if($myrow["CURTYPE"] == 'DOL')
	{
		...
		{
			$type1 = "DOL";//this is allowed value in payment_details table
	...
	if(strstr($subscription,'F') || strstr($subscription,'H') || strstr($subscription,'K'))
        {
                $sqlcon="SELECT SCREENING FROM newjs.JPROFILE WHERE PROFILIED='$myrow[PROFILEID]'";
                ...
                if(strstr($subscription,'F'))
                {
                        if(!isFlagSet("PHONERES",$rowcon["SCREENING"]) || !isFlagSet("PHONEMOB",$rowcon["SCREENING"]) || !isFlagSet("CONTACT",$rowcon["SCREENING"]) || !isFlagSet("MESSENGER",$rowcon["SCREENING"]) || !isFlagSet("EMAIL",$rowcon["SCREENING"]) || !isFlagSet("PARENTS_CONTACT",$rowcon["SCREENING"]))
                        {
                                ...
                                evalue_privacy($myrow["PROFILEID"]);
                }
                else
                {
                        if(!isFlagSet("CITY_BIRTH",$rowcon["SCREENING"]) || !isFlagSet("NAKSHATRA",$rowcon["SCREENING"]))
                                $verify_service="N";
                }
        }
	...
	/*if($myrow['DISCOUNT']>0)	
		$discount_type = 1;
	if((($myrow["DISCOUNT"]/$amount)*100)> 12)
		$discount_type = 2;
	*/
...
/*	$sql_ser="SELECT SQL_CACHE PACKAGE,COMPID,PACKID from billing.SERVICES where SERVICEID='$serviceid'";
	$res_ser=mysql_query_decide($sql_ser) or logError($error_msg,$sql_ser,"ShowErrTemplate");
	$myrow_ser=mysql_fetch_array($res_ser);

	if($myrow_ser['PACKAGE']=="Y")
	{
		$packid=$myrow_ser['PACKID'];
		$sql_comp="SELECT SQL_CACHE COMPID from billing.PACK_COMPONENTS where PACKID='$packid'";
                $res_comp=mysql_query_decide($sql_comp) or logError($error_msg,$sql_comp,"ShowErrTemplate");
		while($myrow_comp=mysql_fetch_array($res_comp))
		{
			$compid[] = $myrow_comp["COMPID"];
		}
	}
	else
		$compid[] = $myrow_ser["COMPID"];

	$i = 0;$insert_query=array();
	while($compid[$i])
	{
		$sql_comp="SELECT SQL_CACHE DURATION from billing.COMPONENTS where COMPID='$compid[$i]'";
		$myrow_comp=mysql_fetch_array(mysql_query_decide($sql_comp)) or logError($error_msg,$sql_comp,"ShowErrTemplate");
		
		if($myrow['SET_ACTIVATE'] != 'Y')	
			$insert_query[] = "('$billid','$myrow[PROFILEID]','$serviceid','$compid[$i]','Y',now(),'','ONLINE',ADDDATE(now(), INTERVAL $myrow_comp[DURATION] MONTH))";
		else
		{
			$renew_status=getRenewStatus($myrow['PROFILEID']);		
			$activate_on=$renew_status['EXPIRY_DT'];
			$insert_query[] = "('$billid','$myrow[PROFILEID]','$serviceid','$compid[$i]','N','','$activate_on','ONLINE',ADDDATE('$activate_on', INTERVAL $myrow_comp[DURATION] MONTH))";
		}
		$i++;
	}
*/
	...
	if($myrow['SET_ACTIVATE'] != 'Y')
		$insert_query[] = "('$billid','$myrow[PROFILEID]','$serviceid','$service_main_components','Y',now(),'','ONLINE',ADDDATE(now(), INTERVAL $service_main_duration MONTH))";
	else
	{
		$renew_status=getRenewStatus($myrow['PROFILEID']);
		$activate_on=$renew_status['EXPIRY_DT'];
	...
	if($v_code)
		mark_voucher_code($myrow['PROFILEID'],$row_vc['CODE'],"","SUCCESSFUL",$billid);
	...
	//Added By lavesh to check suspected ip address.
      	...
      	include_once($path."/profile/suspected_ip.php");
       	...
       	if($suspected_check)
        	send_email('vikas@jeevansathi.com',$myrow['PROFILEID'],"Payment Profileid of suspected email-id","payment@jeevansathi.com");
	...
	//send_email($myrow["BILL_EMAIL"],$msg,$subject,'','','aman.sharma@jeevansathi.com,alok@jeevansathi.com',$bill,$myrow_comp['DURATION']);
	//send_email($myrow["BILL_EMAIL"],$msg,$subject,'','','payments@jeevansathi.com',$bill,$myrow_comp['DURATION']);
        ...
        send_rtf_email($myrow["BILL_EMAIL"],$msg,$subject,'','','payments@jeevansathi.com',$bill);
...
function getServiceName($service)
{
	...
	$res=mysql_query_decide($sql) or die("$sql".mysql_error_js());//logError();
...
function user_start_paying($pid)
{
	...
	$myDb=$mysql->connect("$myDbName");
        ...
        if($row1_1 = $mysql->fetchArray($res1_1))
                $LOGIN_CNT=$LOGIN_CNT+$row1_1['TOTAL_COUNT'];
	...
	unset($contactResult);
        ...
        /*$sql9="select count(*) as cnt from MIS.SEARCHQUERY where PROFILEID='$pid'";
        $res9=mysql_query_decide($sql9) or  logError($error_msg,$sql6,"ShowErrTemplate",$announce_to_email);
        if($row9=mysql_fetch_row($res9))
                $SEARCHES=$row9[0];
        else
                $SEARCHES=0;*/
        ...
        if($row10=mysql_fetch_row($res10))
                $PAGE_VIEWS=$row10[0];
        else
                $PAGE_VIEWS=0;
...
function payment_thanks_things_to_do($profileid, $set_activate)
{
	 ...
	 //Sharding Concept added by Vibhor Garg on table JPARTNER
         ...
         if($jpartnerObj->isPartnerProfileExist($myDb,$mysqlObj,$profileid))
                $smarty->assign("HAVEPARTNER","1");

	//Sharding Concept added by Vibhor Garg on table JPARTNER

        $sql_c = "SELECT COUNT(*) AS CNT FROM newjs.CONTACTS WHERE RECEIVER='$profileid' AND TYPE='A'";
        $res_c = mysql_query_decide($sql_c) or logError($error_msg,$sql_c,"ShowErrTemplate",$announce_to_email);
        $row_c = mysql_fetch_array($res_c);


        if($row_c['CNT'] > 0)
                $smarty->assign("RECEIVED_A",$row_c['CNT']);

        $sql_sst = "SELECT";
        if($set_activate != 'Y')
                $sql_sst .= " ACTIVATED_ON as ST_DATE";
        else
                $sql_sst .= " ACTIVATE_ON as ST_DATE";
...
function promo_mailer($pid)
{
	...
	while($j>0)
	{
		...
		if($row1["GENDER"] || $row1["SLABS"] || $row1["AVAILABLE_IN"])
		$client1=array("clientid"=>$row1["CLIENTID"],"conditions"=>1);
		else
		$client1=array("clientid"=>$row1["CLIENTID"],"conditions"=>0);
		$row1=mysql_fetch_assoc($res1);
		if($row1["GENDER"] || $row1["AVAILABLE_IN"] || $row1["SLABS"])
		$client2=array("clientid"=>$row1["CLIENTID"],"conditions"=>1);
		else
		$client2=array("clientid"=>$row1["CLIENTID"],"conditions"=>0);
		$row1=mysql_fetch_assoc($res1);
		if($row1["GENDER"] || $row1["SLABS"] || $row1["AVAILABLE_IN"])
		$client3=array("clientid"=>$row1["CLIENTID"],"conditions"=>1);
		else
		$client3=array("clientid"=>$row1["CLIENTID"],"conditions"=>0);
