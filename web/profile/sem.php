...
{
	$sql= "SELECT YOURINFO,FAMILYINFO,SPOUSE,JOB_INFO,SIBLING_INFO,FATHER_INFO,AGE,GENDER,RELATION,HAVEPHOTO,MTONGUE,CITY_RES,COUNTRY_RES,DRINK,SMOKE,BTYPE,MANGLIK,DIET,CITY_BIRTH,BTIME,SHOW_HOROSCOPE FROM newjs.JPROFILE where  activatedKey=1 and PROFILEID = '$profileid'";
	...
	while($row=mysql_fetch_array($res))
	{
		...
		$manglik = $row['MANGLIK'];
		$diet = $row['DIET'];
		$horo = $row['SHOW_HOROSCOPE'];
		$btime = $row['BTIME'];
		$birth_city = $row['CITY_BIRTH'];
		$city_res = $row['CITY_RES'];
		$city = substr("$city_res", 0, 2);  
		$country = $row['COUNTRY_RES'];
		$family = $row['FAMILYINFO'];
		$spouse = $row['SPOUSE'];
		$job = $row['JOB_INFO'];
		$sibling = $row['SIBLING_INFO'];
		$father = $row['FATHER_INFO'];
		...
		function s5($city,$mtongue)
		{	
			$czone = cityZone($city);
			$comnzone = comnZone($mtongue);
