...
function mlog_shards($ids,$who)
{
	global $mysql,$myDb;
	 if($ids)
	 {
		$sql="select $who from MESSAGE_LOG where ID in('$ids')";
		$res=$mysql->ExecuteQuery($sql,$myDb) or logError("Due to a temporary problem your request could not be processed. Please try after a couple of minutes",$sql_delete,"ShowErrTemplate");
		while($row=mysql_fetch_array($res))
		{
			$who_pro=$row[$who];
			
			$myDbName=getProfileDatabaseConnectionName($who_pro,'',$mysql);
			$shard_db[$myDbName]=$mysql->connect("$myDbName");

		}
	 }
	return $shard_db;
}
function update_mlog_onshard($sql_delete,$shard_db)
{
	global $mysql;
	if(is_array($shard_db))
		foreach($shard_db as $key=>$val)
		{
			$mysql->ExecuteQuery($sql_delete,$val) or logError("Due to a temporary problem your request could not be processed. Please try after a couple of minutes",$sql_delete,"ShowErrTemplate");
		}
}
...
function delete_mes($send_delete,$folderid,$del_arr,$nudge_arr)
{
	...
	//if not sent item
		...
		{      
			...
			if($del_arr)
			{
				$shard_db=mlog_shards($del_arr,'SENDER');
				...
				update_mlog_onshard($sql_delete,$shard_db);
			}
			if($nudge_arr)
		...
		else
		{
			if($del_arr)
			{
				$shard_db=mlog_shards($del_arr,'RECEIVER');
...
function mark_mes_unread($folderid,$del_arr,$nudge_arr)
{
	...
	if($del_arr)
	{
		...
		update_mlog_onshard($sql,$shard_db);
	}
