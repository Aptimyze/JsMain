...
{
	$sql = "SELECT PROFILEID FROM $searchtable WHERE  COUNTRY_RES='51'";
		...
		if(!$sms_unsubscribe)
		{
			...
			if ($mobile && $source!='ofl_prof')
			{
				...
				if(($rec_is_correct1  || $rec_is_correct2) && (!$checkmobile))
				{
					...
					$contactResult=getResultSet("SENDER",'','',$receiversIn,'',$typeIn,'',$timeClause);
					...
					$contactResult=getResultSet("RECEIVER",$pid,'','','',"'A'",'',$timeClause);
                                        if(is_array($contactResult))
                                        {
						foreach($contactResult as $key=>$value)
                                                {
					...
					if($contactids!='')
					{
						...
						$result=mysql_query_decide($sql,$db) or die($sql.mysql_error($db)) ;
                				$i=0;
						...
						//End
							...
							$message.="since $msg_dt. Logon 2 jeevansathi.com&#013;Ad&#013;Jab rishta mil jaye, kuch meetha ho jaye";
							if(count($msgarray)<200)
								$msgarray[]=array("TO"=>$mobile,"MSG"=>$message,"PID"=>$pid);
							
							if(count($msgarray)==200)
							{
								$db=connect_db();
								$valid_rec=send_message($msgarray,$from,$gsm,$table,"Y");
								unset($msgarray);
								$db=connect_slave();
							}
							//$db=connect_db();
