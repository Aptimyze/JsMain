...
function getSunshine($dtOfBirth)
{
	...
	if($monthFormat =='March')
		$order ="DESC";
	$sql="select * FROM newjs.SUNSHINE_LOOKUP where TO_MONTH='$monthFormat' OR FROM_MONTH='$monthFormat' order by ID $order";
	$result= mysql_query_decide($sql) or logError("Due to a temporary problem your request could not be processed. Please try after a couple of minutes",$sql,"ShowErrTemplate");
	$i = 0;
	while($row = mysql_fetch_array($result))
        {
...
function getCompatibility($userSunshine,$userGender,$viewedUserSunshine,$viewedUserGender)
{
	...
	$sql="select `DESC` FROM newjs.SUNSHINE_COMPAT WHERE `MAN`='$sunshineM' AND `WOMAN`='$sunshineF'";
	$result= mysql_query_decide($sql) or logError("Due to a temporary problem your request could not be processed. Please try after a couple of minutes",$sql,"ShowErrTemplate");
