...
class InstantSMS
{
    ...
    private $smsTypeIgnoreTimeRange = array("DETAIL_CONFIRM", "FORGOT_PASSWORD", "PAYMENT_MEMBERSHIP", "VIEWED_CONTACT_SMS", "FIELD_VISIT_SCHEDULE", "OTP", "MEM_TFN_SMS");
    ...
    public function __construct($smsKey, $profileid, $varArray = array(), $otherProfileId = "")
    {
    ...
    private function isWhitelistedProfile()
    {
        ...
        switch ($this->smsKey) {
            ...
            case "REGISTER_KYC":
                ...
                return false;
            case "AGENT_KYC":
                if ($this->profileDetails["SANAME"] && $this->profileDetails["SAPHONE"]) {
                    return true;
                }
    ...
    private function getMessage()
    {
        ...
        if ($this->profileDetails["SUBSCRIPTION"] != "") {
            $SUBSCRIPTION = "P";
        } elseif ($this->profileDetails['STATE'] != FTOStateTypes::NEVER_EXPOSED) {
            $SUBSCRIPTION = $this->profileDetails['SUBSTATE'];
        } else {
            $SUBSCRIPTION = "F";
        }

        $GENDER = "A";
        if ($this->profileDetails["GENDER"] == "M") {
            $GENDER = "M";
        } else {
            $GENDER = "F";
        }
    ...
    private function getActualMessage($message)
    {
        ...
        for ($i = 0; $i < $mLength; $i++) {
            ...
            if ($message[$i] != '{' && $message[$i] != '}' && !$startToken) {
