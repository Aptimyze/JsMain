...
function RosterUser(jid,subscription,groups,name) {
  ...
  this.chatAuth = null; //This has been added be me for implementing chat authorization part	
  // methods
  this.canChat = null;
  this.nickName =null;
  this.profileId= null;
  ...
  this.popout=null;
  this.userName=null;
  this.msgHistory=new Array();//for message history of the session
...
function RosterUpdateGroups() {
  ...
  for (var i=0; i<oldGroups.length; i++) {
    ...
    if (remove && this.rosterW) {
     ...
     // alert("groupEl is >>>>"+groupEl);
      if(groupHeaderEl && groupHeaderEl != null){
