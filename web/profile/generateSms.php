...
$profileReceiver= "336";
$sql1 = "SELECT EMAIL, jp.PROFILEID, INCOMPLETE, GENDER, USERNAME, SUBSCRIPTION, PHONE_MOB, PASSWORD, CASTE, DTOFBIRTH, MSTATUS, MTONGUE, CITY_RES, WEIGHT, AGE, HEIGHT, EDU_LEVEL_NEW, INCOME, ENTRY_DT,MOB_STATUS, LANDL_STATUS, LAST_LOGIN_DT, HAVEPHOTO, OCCUPATION, COUNTRY_RES, GET_SMS, SOURCE, ACTIVATED, COMPANY_NAME, FAMILY_INCOME, OWN_HOUSE,jt.NTIMES,fs.SUBSTATE AS FTO_SUB_STATE,fto.FTO_ENTRY_DATE,fto.FTO_EXPIRY_DATE FROM newjs.JPROFILE jp LEFT JOIN newjs.JP_NTIMES jt ON (jt.PROFILEID = jp.PROFILEID) LEFT JOIN FTO.FTO_CURRENT_STATE fto ON (fto.PROFILEID = jp.PROFILEID) LEFT JOIN FTO.FTO_STATES fs ON (fto.STATE_ID = fs.STATE_ID) WHERE jp.PROFILEID IN ('".$profileSender."','".$profileReceiver."');";
