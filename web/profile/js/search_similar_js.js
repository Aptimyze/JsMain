...
function show_details(contact_id,con_profilechecksum,force_focus,eoi_label,par_span_label,to_do)
{
	...
	if(to_do=='callnow_layer'){
		after_func = 'append_callnow';
		only_layer='&ONLY_LAYER=1';
		callnow_layer=1;
	}else{
		...
		to_do='view_contact';
		after_func = 'append_contact_det';
	}
	var send_url="?to_do="+to_do+"&show_contacts=&index=0&rand=444&ajax_error=2&from_search=1&profilechecksum="+con_profilechecksum+"&close_func=close_all_con_layer()&checksum="+prof_checksum+"&profile_url="+profile_id+"&prev_cache="+Math.round(Math.random()*1000)+only_layer;
        send_ajax_request('invoke_contact_engine.php'+send_url,'con_show_loader',after_func);
...
function append_callnow(){
        if(act_contact_id)
        {
			...
			$.colorbox({href:url});
                ...
                {
                        dID(act_contact_id).innerHTML="<div class='lyr' style='display:block'><div class='lyr_tp_cur'></div><div class='call' id='call_directly'>"+result+"</div><div class='lyr_btm_cur'></div>";
                }
...
function check_checkbox(checkbox_id,sno)
{
	...
	if(typeof(SHOW_UNVERIFIED_LAYER)!='undefined' && sno!=-1)
        if(SHOW_UNVERIFIED_LAYER)
        {
    //            hide_exp_layer();
                $.colorbox({href:'/profile/myjs_verify_phoneno.php?sourcePage=EOI&flag=1'});
                return false;
        }
	var is_chb_chk=0;
...
function express_interest()
{
	...
	if(typeof(SHOW_UNVERIFIED_LAYER)!='undefined')
	if(SHOW_UNVERIFIED_LAYER)
	{
		hide_exp_layer();
		$.colorbox({href:'/profile/myjs_verify_phoneno.php?sourcePage=EOI&flag=1'});
