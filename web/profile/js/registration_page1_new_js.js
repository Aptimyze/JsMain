...
var basicInfo_section_arr = new Array("gender","day","month","year","mstatus","has_children","height","country_residence","city_residence","country_code","mobile","showmobile");
...
//				display_rest_of_page();
	...
	'#mstatus_female' : function(element){
				element.onblur = function(){
					mstatus_details(this);
					validate(this);
				}
	...
	'#add_phone' : function(element){
			element.onclick= function(){
				document.getElementById('phone_show').style.display='inline';
				document.getElementById('add_phone').style.display='none';
				document.getElementById('remove_phone').style.display='inline';
        $('#li_add_phone').removeClass('bot_bdr');
			}
	},
	'#remove_phone' : function(element){
			element.onclick= function(){
				document.getElementById('phone_show').style.display='none';
				document.getElementById('add_phone').style.display='inline';
				document.getElementById('remove_phone').style.display='none';
				document.getElementById('phone').value='';
        $('#li_add_phone').addClass('bot_bdr');
			}
	},
...
function change_tab_labels()
{
	...
	{
		label='self';
		dID("personal_heading").innerHTML="Your Personal Details";
		dID("social_heading").innerHTML="Your Social Details";
		dID("contact_heading").innerHTML="Your Contact Details";
		dID("day_month_year_submit_err_label").innerHTML="Your Date of Birth";
	}
	else if(selected_val=="2")
	{
		label='son';
		action = 'male_select';
		dID("personal_heading").innerHTML="Son's Personal Details";
		dID("social_heading").innerHTML="Son's Social Details";
		dID("contact_heading").innerHTML="Son's Contact Details";
		dID("day_month_year_submit_err_label").innerHTML="Son's Date of Birth";
	}
	else if(selected_val=="2D")
	{
		label='daughter';
		action = 'female_select';
		dID("personal_heading").innerHTML="Daughter's Personal Details";
		dID("social_heading").innerHTML="Daughter's Social Details";
		dID("contact_heading").innerHTML="Daughter's Contact Details";
		dID("day_month_year_submit_err_label").innerHTML="Daughter's Date of Birth";
	}
	...
	{
		label='friend';
		dID("personal_heading").innerHTML="Profile Personal Details";
		dID("social_heading").innerHTML="Profile Social Details";
		dID("contact_heading").innerHTML="Profile Contact Details";
		dID("day_month_year_submit_err_label").innerHTML="Date of Birth";
	}
	...
	else if(selected_val=="6")
	{
		label='brother';
		action = 'male_select';
		dID("personal_heading").innerHTML="Brother's Personal Details";
		dID("social_heading").innerHTML="Brother's Social Details";
		dID("contact_heading").innerHTML="Brother's Contact Details";
		dID("day_month_year_submit_err_label").innerHTML="Brother's Date of Birth";
	}
	else if(selected_val=="6D")
	{
		label='sister';
		action = 'female_select';
		dID("personal_heading").innerHTML="Sister's Personal Details";
		dID("social_heading").innerHTML="Sister's Social Details";
		dID("contact_heading").innerHTML="Sister's Contact Details";
		dID("day_month_year_submit_err_label").innerHTML="Sister's Date of Birth";
	}
...
function gender_display_selection()
{
	...
	if(action=='male_select')
	{
		...
		dID("mstatus").style.display='inline';
		dID("mstatus_female").style.display='none';
	...
	else if(action=='female_select')
	{
		...
		dID("mstatus").style.display='none';
		dID("mstatus_female").style.display='inline';
		...
		if(religion_value)
		{
			...
			*/
			...
			if(dID("mtongue").value)
				caste_dropdown_array.push("<select class=\"sel_lng fl\" size=\"1\" name=\"caste\" id=\"caste\" onchange=\"get_caste_using_caste_mapping(this); show_hide_partner(this);\" onfocus=\"change_div_class(this);\" onblur=\"validate(this);\">");
				else
				caste_dropdown_array.push("<select class=\"sel_lng fl\" size=\"1\" name=\"caste\" id=\"caste\" onchange=\"check_mtongue()\" onfocus=\"change_div_class(this);\" onblur=\"validate(this);\">");
			for(var i=-1;i<i1;i++)
			{
				if(i==-1)
				{
...
function validate(obj,to_validate_field)
{
	...
	if(docF.mobile==null)
		var validate_fields = new Array("email","password","relationship","gender","day","month","year","mstatus","has_children","height","country_residence","city_residence","mtongue","religion","caste","termsandconditions");
	else
		var validate_fields = new Array("email","password","relationship","gender","day","month","year","mstatus","has_children","height","country_residence","city_residence","mobile","mtongue","religion","caste","termsandconditions");
	...
	for(var i=0;i<i1;i++)
	{
		...
		else if(field_name == "day" || field_name == "month" || field_name == "year")
		{
			...
			{
				var month=docF.month.value;
				var day=docF.day.value;
				var year=docF.year.value;
				...
				var no_of_days=daysInMonth(month,year);
				...
				if(day>no_of_days)
				{
					...
					dID("dob_error2").style.display = 'none';
					dID("dob_error3").style.display = 'none';
					dID("dob_error4").style.display = 'inline';
				}
				else if(gender_val_selected == "M" && age < 21)
				{
					error_fields[err_i] = "day_month_year_submit_err";
					err_i++;
					...
					dID("dob_error3").style.display = 'none';
					dID("dob_error4").style.display = 'none';
				}
		...
		else if(field_name == "mstatus")
		{
			...
			var mstatus_value=get_mstatus_value();	
		...
		else if(field_name == "has_children")
		{
			if(!mstatus_selected_value){
				mstatus_selected_value = get_mstatus_value();
			}

			if(typeof(mstatus_selected_value) == "undefined")
			{
				var mstatus_arr = document.getElementById("mstatus").value;
				if(mstatus_arr){
					mstatus_selected_value=mstatus_arr;
					break;
				}
			...
			if(mstatus_selected_value && mstatus_selected_value != "N")
			{
				...
				var has_children_arr = document.getElementById("has_children").value;
				if(has_children_arr)
		...
		else if(field_name == "state_code")
		{
			var state_code_value=document.getElementById('state_code').value;
			var filter  = /^[0-9]+$/;
			if(country_val[1] == "51")
			{
				if(!filter.test(state_code_value))
				{
					error_fields[err_i] = field_name + "_submit_err";
					err_i++;
				
					dID("state_code_submit_err").style.display = 'inline';
				}
				else
				{
					dID("state_code_submit_err").style.display = 'none';
		...
		{
			dID("mstatus_error2").style.display = 'none';
			var mstatus_selected_value=get_mstatus_value();
...
function mstatus_details(obj,required_id,action)
{
	...
	if(mstatus_selected_val!='N' && mstatus_selected_val !='')
  	{
...
function populate_city(city_residence_selected)
{
	if(docF.country_residence.value != "")
	{
		...
		pop_city_array.push("<select class=\"sel_lng fl\" size=\"1\" name=\"city_residence\" id=\"city_residence\" onchange=\"fetch_code('CITY',this.value);\" onfocus=\"change_div_class(this);\" onblur=\"validate(this);\">");
...
/*ANKIT*/	if(code_for == "COUNTRY")
	...
	{
	      dID("verify_message_mobile").style.display='block';
	}
	else
	{
  ...
  if (docF.country_code.value == '+91') {
	      dID("verify_message_phone").style.display='block';
  }
  else {
...
function populate_yob()
{
	...
	if(gender_val=='F')
	     var mstatus_selected_val = document.getElementById("mstatus_female").value;
	else
	     var mstatus_selected_val = document.getElementById("mstatus").value;
...
function ajax_leadi(type)
{
		if(type=='T' )
		{
			if(docF.mtongue.value && docF.email.value){
				var value = "email_val=" + encodeURI( docF.email.value ) + "&mobile=" + encodeURI( docF.mobile.value )+ "&mtongue=" + encodeURI( docF.mtongue.value )+"&type=" + encodeURI(type);
			}
		}
		if(type=='M'){
			if(docF.mobile.value && docF.email.value)
		var value = "email_val=" + encodeURI( docF.email.value ) + "&mobile=" + encodeURI( docF.mobile.value )+ "&source=" + encodeURI(docF.tieup_source.value)+"&type=" + encodeURI(type);
		}
		if(value){
	    var req = createNewXmlHttpObject();
        var site_url = docF.site_url.value;
        var to_post=site_url+"/profile/reg_lead_capture.php?action=lead_capture&"+value;
