...
var loggedInPid;
var startTimeL2=0;
var common_error='<span style="color:#FF0000"><img src="IMG_URL/profile/images/iconError_16x16.gif"><b>An error has occurred! We will be correcting this problem at the earliest. Kindly check back later.</b></span>';
...
function appPromoPopup() {

//ios and android promotion starts       added by Palash Chordia

if ($("#IOSPromo")) {
  if (!getCookie('appPromo').length) {
  
             
  tempHeight=$("#IOSPromo").height();
   $("#IOSPromo").css('bottom',(-1)*tempHeight+'px');
   if ((getCookie('chatbar')=='yes') && $(window).width()<1270) _bottom='33px'; else _bottom='0px';
   $("#IOSPromo").css('display','block');
  $("#IOSPromo").animate({
    bottom: _bottom
    }, 500
    );
  ...
  $("#AppPromoCls").click(function() {

$("#IOSPromo").animate({
    bottom: "-="+$("#IOSPromo").offset().top
    }, 500, function () {
      $("#IOSPromo").css('display','none');
    }
    );
var tempDate = new Date();
  tempDate.setTime(tempDate.getTime()+(24*60*60*1000)); 
  document.cookie='appPromo=1; expires='+tempDate.toString();
...
//ios and android promotion ends
...
var show_anchor=1;
function show_layer(to_show,to_hide,to_change,to_roll,from_actual,extra_hide,extra_roll)
{
	if(from_actual)
		show_anchor=0;
	else
	...
	if(dID(to_hide))
	{
        ...
        if(dID(extra_hide))
        {
                dID(extra_hide).style.display='none';
                if(dID(extra_roll))
                {
                        dID(extra_roll).className="cont_close_tab";
                }
        }
...
function writeafterSave(pid)
{
	...
	str='<span style="visibility:hidden;"><a class="fr" href="#" onclick="save_search_options(\'show\','+pid+');return false;"> My saved searches </a><i class="fr blue_arw ml_17"></i></span><IFRAME id="iframeshim"  src="" style="display: inline; left: -60px; top: 32px; z-index: 2; position: absolute; width: 190px; height: 78px;filter: progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0);" frameBorder="0" scrolling="no"></IFRAME><span style="position:absolute;margin-left:357px; z-index:1000;margin-top:-5px;">';
        str+='<div class="lf" style="width:188px;"><div class="m_saved_topbg"><div class="t12 b" style="padding:4px 0 0 55px;"><img src="IMG_URL/profile/images/dw_blk_arrow.gif"> My saved searches</div></div><div class="m_saved_hrbg" style="padding:17px 15px; width:150px; line-height:20px">';
...
{
	var close_save1="<a class='fr' href='#' onclick=\"save_search_options('show',"+pid+");return false;\"> My saved searches </a><i class=\"fr blue_arw ml_17\"></i>";
	...
	var data = 0;
        ...
        if (req.status == 200)
        {
			...
			if(got_response[0]=="logoutSearch")
				$.colorbox({href:"/static/registrationLayer?pageSource=search"});
...
var close_searchbyid='<a class="fr" href=# onclick="search_by_id(\'show\');return false;"> Search by profile id</a><i class="fr blue_arw ml_17"></i>';
...
function red_view_similar(var_data)
{
	if(var_data)
	{
			...
			if(typeof(MES)!='undefined' &&  draft_id!="")
			{
				...
				if(typeof(MES[draft_id])!='undefined'){
...
function show_callnow_layer(profilechecksum,index,contact_accept)
{
	if(contact_accept==1){
	        send_ajax_request("invoke_contact_engine.php?profilechecksum="+profilechecksum+"&index="+index+"&to_do=callnow_layer&ONLY_LAYER=1","call_direct_loader","show_direct_contact");
		loader_Id="contact_accept_layer";
	}
	else{
	        send_ajax_request("invoke_contact_engine.php?profilechecksum="+profilechecksum+"&index="+index+"&to_do=callnow_layer&ONLY_LAYER=1","call_direct_loader","show_direct_contact");
	}
}
function show_contact(both_users,profilechecksum,from_search,index)
{
        
        send_ajax_request("call_directly.php?&ajax_error=2&profilechecksum="+profilechecksum+"&show_con=1&both_users="+both_users+"&from_search="+from_search+"&index="+index,"call_direct_loader","show_direct_contact");
}
var previous_data="";
...
$("#homepageLogin").attr('action',loginUrl);
//$("#iframe_login").attr('src',SITE_URL);
return true;
}
...
function onFrameMessageReceived(message)
{
	if(message.origin === SSL_SITE_URL)
	{
			if(!window.addEventListener)
			{
				...
				if(pinkLoginRegistrationLayer && (emailCheck!==-1 || message.data.indexOf("invalidAuth")!==-1))
				{
					stop_loader_login();
					if(emailCheck!==-1)
					{
				...
				else if(emailCheck!==-1 || message.data.indexOf("invalidAuth")!==-1)
				{
			...
			else
			{
				...
				if(pinkLoginRegistrationLayer && (emailCheck!==-1 || message.data.body.indexOf("invalidAuth")!==-1))
				{
					stop_loader_login();
					if(emailCheck!==-1)
					{
				...
				else if(emailCheck!==-1 ||  message.data.body.indexOf("invalidAuth")!==-1 )
				{
					$("#error_mess").html(cur_err_msg);
					$("#homepageLogin").attr('action', "");
					if(emailCheck!==-1)
