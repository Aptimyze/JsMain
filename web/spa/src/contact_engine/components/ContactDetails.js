...
export default class ContactDetails extends React.Component{
  ...
  constructor(props){
    ...
    this.state.tupleDim = {width:'100%','height': window.innerHeight};
    this.state.smsBtnClicked = false;
...
render(){
  let showSMS='';
  if((this.props.actionDetails.contact1 && this.props.actionDetails.contact1.value != 'blur') ||
    (this.props.actionDetails.contact2 && this.props.actionDetails.contact2.value != 'blur') ||
    (this.props.actionDetails.contact3 && this.props.actionDetails.contact3.value != 'blur') ||
    (this.props.actionDetails.contact4 && this.props.actionDetails.contact4.value != 'blur') ||
    (this.props.actionDetails.contact5 && this.props.actionDetails.contact5.value != 'blur') ||
    (this.props.actionDetails.contact6 && this.props.actionDetails.contact6.value != 'blur')
  ){
    let msg = '';
    if(this.props.selfIsd && this.props.selfIsd=='91'){
      msg = "Send details as SMS";
    }
      if(this.state.smsBtnClicked){
        msg = "Details sent as SMS";
      }
      showSMS = <div className={"f16 pt20 "} style={{textAlign :"center" }} onClick={()=>{
        if(this.state.smsBtnClicked == false){
          this.setState({smsBtnClicked : true});
          this.sendContactInSMS(this.props.profilechecksum);
        }
      }} >{msg}</div>;

  }

return (<div id="viewContactLayer"  className="posfix ce-bg ce_top1 ce_z101" style={this.state.tupleDim}>
...
{this.state.errorMessage.length > 0 ? <TopError timeToHide={6000} message={this.state.errorMessage}></TopError> : null}
{showSMS}
...
componentDidMount(){
  ...
  this.setState({
   cdOHeight:(window.innerHeight - parseInt(sum)  )
 });
...
sendContactInSMS(checksum){
  let ContactInSMSUrl = "/phone/SMSContactsToMobile";
  console.log("checksum", checksum);
  commonApiCall(ContactInSMSUrl+"?profileChecksum="+checksum, {}, '', 'POST').then((response) => {
    if(response.responseStatusCode == 0){
      this.setState({errorMessage : "Details sent to "+response.mobile});
    }
  });
}
          ...
          <div className="memOverlay app_clrw" style={{paddingBottom:'50px'}}>
              <div className="txtc">
                  ...
                  <ul style={{paddingLeft:'40px'}} className=" memList f13 fontlig">
                      {this.state.sH1 && <li className="tick pad21" id="subheading1">{this.state.sH1}</li>}
                      {this.state.sH2 && <li className="tick pad21" id="subheading2">{this.state.sH2}</li>}
                      {this.state.sH3 && <li className="tick pad21" id="subheading3">{this.state.sH3}</li>}
                  </ul>
      ...
      nevMindStyle = {display:'block'};
        ...
        if(actiondetails.footerbutton!=null){
          ...
          if(actiondetails.newerrmsglabel)
          {
            ...
            var lowestOfferDiv = (
                          <div className="f16 fontlig" id="LowestOffer" >
                            <span className="dispibl" dangerouslySetInnerHTML={{__html: "Lowest Membership starts @"}} ></span>
                            <span className="dispibl">
                              <del id="oldPrice" className={oPShow}>{oPrice}</del>
                            </span>
                            <span id="currency" dangerouslySetInnerHTML={{ __html: actiondetails.membershipoffercurrency}}></span>
                            <span id="newPrice">
                              {actiondetails.discountedprice}
                            </span>
                          </div>);
            }
