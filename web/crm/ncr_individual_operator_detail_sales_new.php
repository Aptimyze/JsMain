...
$name = preg_replace('/[^A-Za-z0-9\. -]/', '', $_COOKIE["CRM_NOTIFICATION_AGENT"]);
...
if(authenticated($cid))
{
	...
	//code added to show call source for inbound users.
	...
	if($operator_name != 'all'){
		...
		$sql ="SELECT PROFILEID, AMOUNT, ENTRY_DT AS PAID_DT,ALLOTED_TO, MODE, ALLOT_TIME FROM incentive.MONTHLY_INCENTIVE_ELIGIBILITY WHERE ENTRY_DT >='$st_date' AND ENTRY_DT<='$end_date' AND ALLOTED_TO IN($allotstr) ORDER BY ENTRY_DT DESC";
	}
	$db = connect_db();
		...
		//$billid=$row['BILLID'].",".$row['RECEIPTID'];
			...
			//		if($row1['cnt']>0)
					//{
						...
						//end of - code to show call source for inbound users.
					//}
				}
			//}
		//}
...
/********************************Code added by Aman for Excel format****************************************************/
		...
		header("Content-Disposition: attachment; filename=Sales_Credit_Details_".$opsname."_".$yy."_".$mm.".xls");
