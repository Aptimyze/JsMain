...
//		$smarty->assign("showlink","Y");
	...
	/*	$tdy=substr(get_date_format(Date('Y-m-d')),0,11);
		...
		$sql="INSERT into incentive.INVOICE_TRACK(SENT_TO,TIME,SENT_BY) values ('$idstring',now(),'$name')";
		mysql_query_decide($sql) or die("$sql".mysql_error_js());

                      
		$msg="Mail has been sent successfully";	
       		$msg .= "<a href='mail_for_skypak.php?name=$name&cid=$cid'>Back</a>";
	...
	else
	{
		...
		if($showall=="Y")
			$sql="SELECT count(*) from incentive.PAYMENT_COLLECT, billing.SERVICES,incentive.BRANCH_CITY where CONFIRM='Y' and AR_GIVEN='' and DISPLAY <> 'N' and PAYMENT_COLLECT.SERVICE=SERVICES.SERVICEID and PAYMENT_COLLECT.CITY=BRANCH_CITY.VALUE";
		else
			$sql="SELECT count(*) from incentive.PAYMENT_COLLECT, billing.SERVICES,incentive.BRANCH_CITY where CONFIRM='Y' and AR_GIVEN='' and DISPLAY <> 'N' and PAYMENT_COLLECT.CITY IN('$ar') and PAYMENT_COLLECT.SERVICE=SERVICES.SERVICEID and PAYMENT_COLLECT.CITY=BRANCH_CITY.VALUE";
                $result=mysql_query_decide($sql,$db) or die(mysql_error_js());
                $myrow = mysql_fetch_row($result);
                $TOTALREC = $myrow[0];

		if($showall=="Y")
			$sql="SELECT incentive.PAYMENT_COLLECT.ID,PROFILEID,USERNAME,PAYMENT_COLLECT.NAME,EMAIL,PHONE_RES,PHONE_MOB,ENTRY_DT,ADDON_SERVICEID,SERVICES.NAME as SERVICE,ADDRESS,COMMENTS,PREF_TIME,COURIER_TYPE,BRANCH_CITY.LABEL as CITY,PIN,ENTRYBY from incentive.PAYMENT_COLLECT, billing.SERVICES,incentive.BRANCH_CITY where CONFIRM='Y' and AR_GIVEN='' and DISPLAY <> 'N' and PAYMENT_COLLECT.SERVICE=SERVICES.SERVICEID and PAYMENT_COLLECT.CITY=BRANCH_CITY.VALUE ORDER BY incentive.PAYMENT_COLLECT.ID LIMIT $j,$PAGELEN";
		else
			$sql="SELECT incentive.PAYMENT_COLLECT.ID,PROFILEID,USERNAME,PAYMENT_COLLECT.NAME,EMAIL,PHONE_RES,PHONE_MOB,ENTRY_DT,ADDON_SERVICEID,SERVICES.NAME as SERVICE,ADDRESS,COMMENTS,PREF_TIME,COURIER_TYPE,BRANCH_CITY.LABEL as CITY,PIN,ENTRYBY from incentive.PAYMENT_COLLECT, billing.SERVICES,incentive.BRANCH_CITY where CONFIRM='Y' and AR_GIVEN='' and DISPLAY <> 'N' and PAYMENT_COLLECT.CITY IN ('$ar') and PAYMENT_COLLECT.SERVICE=SERVICES.SERVICEID and PAYMENT_COLLECT.CITY=BRANCH_CITY.VALUE ORDER BY incentive.PAYMENT_COLLECT.ID LIMIT $j,$PAGELEN";
		$result=mysql_query_decide($sql) or die(mysql_error_js());
		$index=0;
		while($myrow=mysql_fetch_array($result))
		{
//			$address=$myrow["ADDRESS"]." ".$myrow["CITY"]."-".$myrow["PIN"];
			...
			$index++;
                ...
                if( $j )
                        ...
                        $cPage = 1;
                pagelink($PAGELEN,$TOTALREC,$cPage,$LINKNO,$cid,"mail_for_skypak.php",'','','',$showall);
		...
		$smarty->assign("cid",$cid);
		$smarty->display("mail_for_skypak.htm");
	}
}
else
{
