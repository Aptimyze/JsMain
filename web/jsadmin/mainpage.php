...
if(isset($data))//successful login
{
	...
	if(JsConstants::$whichMachine == 'prod' && JsConstants::$siteUrl == 'http://crm.jeevansathi.com'){
        ...
        if(in_array('BU',$priv) || in_array('BA',$priv) || in_array('SE',$priv)){ //billing entry operator
	...
	if(in_array('MR',$priv))
		...
		{
			$linkarr[]="<a href=\"$SITE_URL/jsadmin/go_to_large_file.php?name=$user&cid=$cid\">Configure Large File</a>";			$linkarr[]="<a href=\"$SITE_URL/crm/show_IM.php?cid=$cid\">Show/Hide Incentive Multiplier</a>";
		}
