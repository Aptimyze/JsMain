...
if($data)
{
		...
		if(count($profileid_arr))
		{
			...
			if($count)
				$smarty->assign("msg","$count renewal campaign profile(s) transferred.<br><a href=\"$SITE_URL/jsadmin/transfer_ren_profiles.php?name=$user&cid=$cid\">Next</a>");
			else
				$smarty->assign("msg","The profile(s) alloted are not of renewal campaign.<br><a href=\"$SITE_URL/jsadmin/transfer_ren_profiles.php?name=$user&cid=$cid\">Next</a>");
		}
		else
			$smarty->assign("msg","No renewal campaign profile alloted to $transfer_of on $alloted_dt.<br><a href=\"$SITE_URL/jsadmin/transfer_ren_profiles.php?name=$user&cid=$cid\">Back</a>");
	}		
	$name=getname($cid);
	$smarty->assign("name",$name);
        $smarty->assign("cid",$cid);
	//$all_sql= "SELECT USER FROM jsadmin.RENEWAL_AGENT";
	$all_sql= "SELECT USERNAME FROM jsadmin.PSWRDS WHERE PRIVILAGE LIKE '%ExcRnw%' AND ACTIVE='Y'";
	...
	for($i=0;$i<7;$i++)
	{
        ...
        unset($Submit);
        $smarty->display("transfer_ren_profiles.htm");
}
