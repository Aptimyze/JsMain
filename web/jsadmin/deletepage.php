...
if(authenticated($cid))
{
	...
	if($c>0)
        {
                ...
                if($reasons=="unproper")
                        $reason_ar[] = "Improper";
		if($reason=="InvalidProfile")
			$reason_ar[] = "Invalid Profile";
		if($reasons=="Other")
		{
                	if(trim($other) != '')
                        	$reason_ar[] = $other;
        ...
        else
	{
		...
		if($flag_deletion && $send_mail)	
		{
			deletemail($pid, $flags, $other,$entryby);
		}
		if($name)
		{
			$user=$name;
...
function deletemail($pid,$flags,$other,$entryby="")
{
		...
		$msg="<br>Dear $username,<br><br>Your profile with username $username has been deleted because :<br><br>";
		
		for($i=0,$k=1;$i<count($flags);$i++,$k++)
		{
			if($flags[$i]==1)
				$msg .= "$k. You are underage according to our policy.<br>";
			if($flags[$i]==2)
				$msg .= "$k. Your profile is not filled properly.<br>"; 
			if($flags[$i]==3)
				$msg .= "$k. Your username is not correct according to our policy.<br>";
			if($flags[$i]==4)
				$msg.= "$k. $other.<br>";
		}
		if($flags[$i] == 2 || $flags[$i] == 3)
		$msg .= "<br>You are requested to input a new profile with a different username (you will need to input a different e-mail id in that profile). <br><br>We regret the inconvenience caused to you.";

		if($flags[$i]==3)
		$msg .= "<br><br><b>Please note :</b> input of profile with username as Email-ID or Phone No. is not permitted on the site. The same has been specifically mentioned in the Profile Input Form.";

		$msg .= "<br><br>Regards,<br>Jeevansathi.com Team";
