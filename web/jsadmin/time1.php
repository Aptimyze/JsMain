...
{	
	$second_time=2;//time is in hours
	$third_time=4;//time is in hours
...
//changing time t1 to GMT from EST and then addin 5hrs 30 min to convert in IST
        ...
        //$t1=strftime("%Y-%m-%d %H:%M",JSstrToTime("$t1"));
        ...
        {
                $timeadjust=0;
                $t1=strftime("%Y-%m-%d $office_hour1:00:00",JSstrToTime("$t1 + 1 days"));
        }
        elseif($t1_hour<$office_hour1)
        {
                $timeadjust=0;
                $t1=strftime("%Y-%m-%d $office_hour1:00:00",JSstrToTime("$t1"));
        }
