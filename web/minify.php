...
if($IMG_URL)
	...
	define ('IMG_URL',sfConfig::get('app_img_url'));
...
class Minify {
  ...
  public static function handleRequest() {
    ...
    //Fetches realname from script
    if($type=="text/css")
	    $files=convert_files_name($files,"css");
    else
	    $files=convert_files_name($files,"js");
  ...
  public function browserCache() {
	...
	header("Expires: " . gmdate('D, d M Y H:i:s', time()+(3600*24*20*365)) . " GMT");
        exit;
      }
    }
    ...
    if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
      if ($lastModified <= JSstrToTime($_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
        header("Last-Modified: $lastModifiedGMT", true, 304);
	header("Expires: " . gmdate('D, d M Y H:i:s', time()+(3600*24*20*365)) . " GMT");
        exit;
      }
  ...
  public function combine($minify = true) {
    ...
    foreach($this->files as $file) {
      if ($this->type === self::TYPE_CSS && MINIFY_REWRITE_CSS_URLS) {
        ...
        $combined[] = self::rewriteCSSUrls(str_replace("IMG_URL",IMG_URL,file_get_contents($file)),
            dirname($file));
      }
      else {
        $combined[] = str_replace("IMG_URL",IMG_URL,file_get_contents($file));
      }
    }
