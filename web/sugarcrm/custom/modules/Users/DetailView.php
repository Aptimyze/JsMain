...
$displayValue=$app_list_strings['employee_type_list'][$focus->employee_type_c];
if($displayValue=='Please select')
	$displayValue='';
$sugar_smarty->assign("EMPLOYEE_TYPE_C", $displayValue);
$displayValue="";
if($focus->employee_multi_mtongue_c)
{
	$employeeMultiMtongueArray=explode(",",$focus->employee_multi_mtongue_c);
	foreach($employeeMultiMtongueArray as $key=>$value)
	{
		if($app_list_strings['Mtongue'][$value]!='Please select')
			$displayValue.=$app_list_strings['Mtongue'][$value].",";
	}
	$displayValue=trim($displayValue,",");
}
$sugar_smarty->assign("EMPLOYEE_MULTI_MTONGUE_C", $displayValue);
$displayValue="";
if($focus->employee_mutli_zone_c)
{
        $employeeMultiZoneArray=explode(",",$focus->employee_mutli_zone_c);
        foreach($employeeMultiZoneArray as $key=>$value)
        {
                if($app_list_strings['zone_list'][$value]!='Please select')
                        $displayValue.=$app_list_strings['zone_list'][$value].",";
        }
        $displayValue=trim($displayValue,",");
}
$sugar_smarty->assign("EMPLOYEE_MUTLI_ZONE_C", $displayValue);
if($focus->employee_operation_mode_c)
	$sugar_smarty->assign("EMPLOYEE_OPERATION_MODE_C",$app_list_strings['employee_operation_mode_list'][$focus->employee_operation_mode_c]);
