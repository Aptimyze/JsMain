    ...
    <div class="yui-content">
        <div>
            ...
            <table width="100%" border="0" cellspacing="1" cellpadding="0" class="edit view">
				...
				<!-- employee form changes added by Sadaf-->
			    <tr>
				<td width="17%" scope="row"><slot>{$MOD.LBL_EMPLOYEE_TYPE}:</slot></td>
				<td width="33%" scope="row"><slot>{$EMPLOYEE_TYPE_OPTIONS}</slot></td>
				<td width="17%" scope="row"><slot>{$MOD.LBL_EMPLOYEE_MULTI_MTONGUE}</slot></td>
                                <td width="33%" scope="row" ><slot>{$EMPLOYEE_MULTI_MTONGUE_OPTIONS}</slot></td>
			    </tr>
			    <tr>
                                <td width="17%" scope="row"><slot>{$MOD.LBL_EMPLOYEE_MUTLI_ZONE_C}:</slot></td>
                                <td width="33%" scope="row"><slot>{$EMPLOYEE_MUTLI_ZONE_OPTIONS}</slot></td>
                                <td width="17%" scope="row"><slot>{$MOD.LBL_EMPLOYEE_MAX_CAP}</slot></td>
                                <td width="33%" scope="row" ><slot><input id="employee_max_cap_c" name="employee_max_cap_c" type="text" size="20" maxlength='25' value='{$EMPLOYEE_MAX_CAP}' onBlur="checkMaxCap('employee_max_cap_c');"></slot></td>
                            </tr>
			    <tr>
				<td width="17%" scope="row"><slot>{$MOD.LBL_EMPLOYEE_OPERATION_MODE_C}:</slot></td>
                                <td width="33%" scope="row"><slot>{$EMPLOYEE_OPERATION_MODE_OPTIONS}</slot></td>
			    </tr>
...
function checkMaxCap(element)
{
	...
	if(val)
	{
		if(!isNumeric(val))
		{
			alert("Max cap can only be numeric!");
