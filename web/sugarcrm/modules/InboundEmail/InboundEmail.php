			...
			while($a = $this->db->fetchByAssoc($r)) {
				...
				foreach($a as $k => $v) {
					...
					switch($k) {
						...
						case "senddate":
							//$overview->date = JSstrToTime(date('r', JSstrToTime($v)));
							$overview->date = $v;
						break;
...
//			_pp($UIDLs);_pp($cacheUIDLs);
	...
	function search($ieId, $subject='', $from='', $to='', $body='', $dateFrom='', $dateTo='') {
		...
		$criteria .= (!empty($dateFrom)) ? ' SINCE "'.date('d-M-Y', JSstrToTime($dateFrom)).'"' : "";
		$criteria .= (!empty($dateTo)) ? ' BEFORE "'.date('d-M-Y', JSstrToTime($dateTo)).'"' : "";
		//$criteria .= (!empty($from)) ? ' FROM "'.$from.'"' : "";
					...
					while($a = $bean->db->fetchByAssoc($r)) {
						...
						foreach($a as $k => $v) {
							...
							switch($k) {
								...
								case "senddate":
									$overview->date = date('r', JSstrToTime($v));
								break;
	...
	function getUnixHeaderDate($headerDate) {
		...
		$unixHeaderDate = JSstrToTime($headerDate);
		...
		if(JSstrToTime('Jan 1, 2001') > $unixHeaderDate) {
			$unixHeaderDate = JSstrToTime('now');
		}
...
//		$unixHeaderDate = JSstrToTime($gmt0dateTime);
//
	...
	function setAutoreplyStatus($addr) {
		$this->db->query(	'INSERT INTO inbound_email_autoreply (id, deleted, date_entered, date_modified, autoreplied_to, ie_id) VALUES (
							\''.create_guid().'\',
							0,
							\''.gmdate('Y-m-d H:i:s', JSstrToTime('now')).'\',
							\''.gmdate('Y-m-d H:i:s', JSstrToTime('now')).'\',
							\''.$addr.'\',
		                    \''.$this->id.'\') ', true);
	}
	...
	function getAutoreplyStatus($from) {
		...
		$q_clean = 'UPDATE inbound_email_autoreply SET deleted = 1 WHERE date_entered < \''.gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime('now -24 hours')).'\'';
	...
	function getNewMessageIds() {
		...
		if($storedOptions['only_since']) {// POP3 does not support Unseen flags
			if(!isset($storedOptions['only_since_last']) && !empty($storedOptions['only_since_last'])) {
				...
				$date = date('r', JSstrToTime($a['last_run']));
			} else {
	...
	function displayOneEmail($uid, $mbox, $isMsgNo=false) {
		...
		foreach($fetchedAttributes as $k) {
			if ($k == 'date_start') {
				...
				//$souEmail[$k] = $timedate->to_display_date_time(date('r', JSstrToTime($this->email->$k . " " . $this->email->time_start)));
