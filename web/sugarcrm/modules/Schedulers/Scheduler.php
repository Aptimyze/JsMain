...
class Scheduler extends SugarBean {
	...
	function flushDeadJobs() {
		...
		$now = JSstrToTime(gmdate('Y-m-d H:i:s', JSstrToTime('now'))); // this garbage to make sure we're getting comprable data to the DB output
			...
			while($a = $this->db->fetchByAssoc($r)) {
				...
				if($r2 != null) {
					...
					if($a2 != null) {
						...
						$execTime = JSstrToTime($a2['execute_time']);
	...
	function fireQualified() {
		...
		$now = gmdate('Y-m-d H:i', JSstrToTime('now'));
...
//			}
	...
	function deriveDBDateTimes($focus) {
		...
		if($mons == '*') {
			...
			$startMon = date('m', JSstrToTime($focus->date_time_start));
		...
		if($dates == '*') {
			...
			$startDate = date('d', JSstrToTime($focus->date_time_start));
		...
		//$currentHour = date('G', JSstrToTime('00:00'));
		if($hrs == '*') {
		...
		// derive minutes
		//$currentMin = date('i');
		$currentMin = date('i', JSstrToTime($this->date_time_start));
		...
		if($mins == '*') {
			...
			$startMin = date('i',JSstrToTime($focus->date_time_start));
		...
		if(empty($focus->time_from)  && empty($focus->time_to) ) {
			$timeFromTs = 0;
			$timeToTs = JSstrToTime('+1 day');
		} else {
			$timeFromTs = JSstrToTime($focus->time_from);	// these are now GMT (timestamps are all GMT)
			$timeToTs	= JSstrToTime($focus->time_to);	// see above
			if($timeFromTs > $timeToTs) { // we've crossed into the next day
				$timeToTs = JSstrToTime('+1 day '. $focus->time_to);	// also in GMT
			}
		}
		$timeToTs++;
		...
		if(empty($focus->last_run)) {
			...
			$lastRunTs = JSstrToTime($focus->last_run . ' UTC');
		}
...
//		_ppd($minName);
		...
		foreach($hrName as $kHr=>$hr) {
			...
			foreach($minName as $kMin=>$min) {
				if($hourSeen == 25) {
					$theDate = date('Y-m-d', JSstrToTime('+1 day'));
				} else {
					$theDate = date('Y-m-d');
				}

				$tsGmt = JSstrToTime($theDate.' '.str_pad($hr,2,'0',STR_PAD_LEFT).":".str_pad($min,2,'0',STR_PAD_LEFT).":00"); // this is LOCAL
		...
		if($focus->catch_up == 1) {
			if($focus->last_run == null) {
				// always "catch-up"
				$validJobTime[] = gmdate('Y-m-d H:i', JSstrToTime('now'));
			} else {
		...
		switch($type) {
			...
			case 1: // hours
				...
				if(preg_match('/\*\//', $value)) { // every [SOME INTERVAL] hours
					...
					return date($datef['time'], JSstrToTime($value.':'.str_pad($mins, 2, '0', STR_PAD_LEFT)));
				}
			case 2: // day of month
				if(preg_match('/\*/', $value)) {
					return $value;
				} else {
					return date('jS', JSstrToTime('December '.$value));
				}

			case 3: // months
				return date('F', JSstrToTime('2005-'.$value.'-01'));
			case 4: // days of week
				return $days[$value];
			default:
	...
	function cleanJobLog() {
		$this->db->query('DELETE FROM schedulers_times WHERE date_entered < '.db_convert('\''.gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime('-24 hours')).'\'', 'datetime').'');
	}
