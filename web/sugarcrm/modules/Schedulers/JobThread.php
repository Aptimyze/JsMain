...
/*if($_SERVER['SERVER_ADDR'] != FetchClientIP()) { // make sure this script only gets executed locally
...
if(!empty($_REQUEST['job_id'])) {
	...
	$job = new Job();
	$job->runtime = gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime('now'));
