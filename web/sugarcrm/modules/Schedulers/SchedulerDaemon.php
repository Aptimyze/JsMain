...
class SchedulerDaemon extends Scheduler {
	...
	function checkPendingJobs() {
		...
		$fireTimeMinus = gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime('now -1 min'));
		$fireTimePlus = gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime('now +1 min'));
	...
	function deriveDBDateTimes($focus) {
		...
		if($mons == '*') {
			...
			$startMon = date(JSstrToTime('m',$focus->date_time_start));
		...
		// derive hours part
		//$startHour = date('G', JSstrToTime($focus->date_time_start));
		//$currentHour = ($startHour < 1) ? 23 : date('G', JSstrToTime($focus->date_time_start));
		$currentHour = date('G');
		if($hrs == '*') {
		...
		if(empty($focus->last_run)) {
			...
			$lastRunTs = JSstrToTime($focus->last_run);
		}
		...
		$timeStartTs = JSstrToTime($dateTimeStart);
		if(!empty($focus->date_time_end) && !$focus->date_time_end == '2021-01-01 07:59:00') { // do the same for date_time_end if not empty
			...
			$dateTimeEnd = date('Y-m-d H:i:s', JSstrToTime('+1 day'));
//			$dateTimeEnd = '2020-12-31 23:59:59'; // if empty, set it to something ridiculous
		}
		$timeEndTs = JSstrToTime($dateTimeEnd); // GMT end timestamp if necessary
		$timeEndTs++;
