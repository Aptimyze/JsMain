...
function template_display_my_feed(&$feed_row) {
	...
	if(method_exists($currChannel,'getLastBuildDate')) {
		...
		if(!empty($last_build_date)){
			...
			$last_build_date = Date($user_preference_format['date'].' '.$user_preference_format['time'] , JSstrToTime($last_build_date));
		}
	}
...
eoq;
	...
	if(!isset($topitem)) {
		...
		for($j = 0; $j < $totalItems; $j++) {
			...
			if(!empty($item_date)){
				$user_preference_format = $current_user->getUserDateTimePreferences();
				$item_date = Date($user_preference_format['date'].' '.$user_preference_format['time'] , JSstrToTime($item_date));
			}
			$out .= <<<eoq
