...
class ImportDuplicateCheck
{
    ...
    {
	//Using common de-duplication module for import
	include_once("include/utils/Jsde_duplicate.php");
        $JSDuplicateCheck=new Duplicate();
        if($this->_focus->email1)
        {
                $dup=$JSDuplicateCheck->isDuplicateEmail($this->_focus->email1);
		if($dup)
			return true;
        }
	if($this->_focus->enquirer_email_id_c)
        {
                $dup=$JSDuplicateCheck->isDuplicateEmail($this->_focus->enquirer_email_id_c);
                if($dup)
                        return true;
        }
	if($this->_focus->phone_home && $this->_focus->std_c)
        {
                $dup=$JSDuplicateCheck->isDuplicatePhone($this->_focus->std_c,$this->_focus->phone_home);
                if($dup)
                        return true;
        }
	if($this->_focus->enquirer_landline_c && $this->_focus->std_enquirer_c)
        {
                $dup=$JSDuplicateCheck->isDuplicatePhone($this->_focus->std_enquirer_c,$this->_focus->enquirer_landline_c);
                if($dup)
                        return true;
        }
	if($this->_focus->phone_mobile)
        {
                $dup=$JSDuplicateCheck->isDuplicateMobile($this->_focus->phone_mobile);
                if($dup)
                        return true;
        }
	if($this->_focus->enquirer_mobile_no_c)
        {
                $dup=$JSDuplicateCheck->isDuplicateMobile($this->_focus->enquirer_mobile_no_c);
                if($dup)
                        return true;
        }
		...
		//Code added to ignore converted leads -Neha
		$index_fields['converted']="0";
