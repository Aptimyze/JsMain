...
class Lead extends Person {
    ...
    function create_export_query(&$order_by, &$where, $relate_link_join='')
    {
		...
		if($custom_join)
			...
			//Campaign name added in query by Sadaf as reqd
                         $query = "SELECT
                                leads.*,campaigns.name campaign_name,email_addresses.email_address email1,
			...
			$query .= "			LEFT JOIN users
                                ON leads.assigned_user_id=users.id ";
				$query.= " LEFT JOIN campaigns ON leads.campaign_id=campaigns.id ";
    ...
    function converted_lead($leadid, $contactid, $accountid, $opportunityid){
		...
		$lead->status='Converted'; // commented by Neha
		$lead->status='6';	// Added as dropdown value changed to 6 - Neha
		$lead->save();
    }
...
//carrys forward custom lead fields to contacts, accounts, opportunities during Lead Conversion
    ...
    public function saveuname($lead,$uname)
{
	$query="UPDATE leads_cstm set username_c='$uname' where leads_cstm.id_c=$lead ";
       $this->db->query($query,true,"Error entring lead: ");
	//mail("neha.verma@jeevansathi.com,nehaverma.dce@gmail.com","Username updated",$query);
	
}
    public function getLeadProfileID()
	{
		if($this->jsprofileid_c)
		{
			$sql="SELECT PROFILEID FROM newjs.JPROFILE WHERE USERNAME=\"$this->jsprofileid_c\"";
