...
if ($appointment_filter == 'last this_month') {
	$next_month		= strftime("%B %Y", JSstrToTime("+1 month"));
	$first_day		= strftime("%d %B %Y", JSstrToTime("1 $next_month"));
	$appt_filter	= strftime("%d %B %Y", JSstrToTime("-1 day", JSstrToTime($first_day)));
} elseif ($appointment_filter == 'last next_month') {
	$next_month		= strftime("%B %Y", JSstrToTime("+2 month"));
	$first_day		= strftime("%d %B %Y", JSstrToTime("1 $next_month"));
	$appt_filter	= strftime("%d %B %Y", JSstrToTime("-1 day", JSstrToTime($first_day)));
...
$gm_later	= gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime($appt_filter));
