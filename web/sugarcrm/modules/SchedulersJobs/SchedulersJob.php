...
class SchedulersJob extends SugarBean {
	...
	function handleDateFormat($time, $user=null) {
		...
		$dbTime = gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime($time));
	...
	function finishJob() {
		...
		if($this->scheduler->last_run == gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime('Jan 01 2000 00:00:00'))) {
			$this->scheduler->last_run = $this->handleDateFormat('now');
