...
EOQ;
   ...
   function bean_implements($interface) {
	...
	function load_user($user_password) {
		...
		if($_SESSION['loginattempts'] > 5) {
			$GLOBALS['log']->fatal('SECURITY: '.$this->user_name.' has attempted to login '.$_SESSION['loginattempts'].' times from IP address: '.FetchClientIP().'.');
		}
	...
	*/
	...
	function is_exe($user_id='')
	{
		...
		if($this->check_role_membership('Executive',$user_id))
			return true;
		else
			return false;
	}
	...
	{
		global $current_user;
		if($current_user->user_name=='ops1' || $current_user->user_name=='ops2' || $current_user->user_name=='ops3' || $current_user->user_name=='ops4' || $current_user->user_name=='ops5')
			return true;
		else
			return false;
	}
	function is_manager($user_id='')
	{
		...
		if($this->check_role_membership('Manager',$user_id))
			return true;
		else
			return false;
	}
	function is_reg_exe()
	{
		global $current_user;
                ...
                if($this->check_role_membership('Registration Executive',$user_id))
                        return true;
                else
                        return false;
	}

	//Function added by Sadaf to return if user is on dialer or not
	function is_dialer_exe()
	{
		global $current_user;
		
		$operation_mode=$current_user->employee_operation_mode_c;
		if($operation_mode==1)
			return true;
		else
			return false;
