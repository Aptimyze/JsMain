	...
	if($rowsDblCheck < 1) {
		...
		foreach($tables as $table_name =>$table) {
			...
			for($year = $startyear; $year <= $maxyear; $year++) {
				...
				if($range) {
					if( JSstrToTime($range['start']) < JSstrToTime($range['end'])) {
			...
			<td class=\"tabDetailViewDF2\"><slot>
				<table cellpadding='0' cellspacing='0' border='0'>
					<tr>
						...
						<td class=\"tabDetailViewDF2\"><slot>
							".$timedate->to_display_time(date($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime('now')), true, false)."
						</td>
					<tr>
					</tr>
						<td class=\"tabDetailViewDF2\"><slot>
...
if(!empty($_POST['upgrade'])){
	...
	$qDst = "INSERT INTO versions VALUES ('".create_guid()."', 0, '".gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime('now'))."', '".gmdate($GLOBALS['timedate']->get_db_date_time_format(), JSstrToTime('now'))."', '".$current_user->id."', '".$current_user->id."', 'DST Fix', '3.5.1b', '3.5.1b')";
	$qRes = $db->query($qDst);
