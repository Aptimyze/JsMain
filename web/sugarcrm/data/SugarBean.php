		    	...
		    	switch($type){
		    		case 'date':
		    			if(!empty($value['display_default'])){
							...
							$td = new TemplateDate();
                            $timeValue = ($value['display_default'] == 'first of next month') ? $timeValue = JSstrToTime( "+1 month" , JSstrToTime( date("F")."1") ) : JSstrToTime($value['display_default']) ;
		    		...
		    		case 'datetimecombo':
		    			if(!empty($value['display_default'])){
					    	...
					    	if(!empty($dtAry[0]) ){
					    		$dateValue = ($dtAry[0] == 'first of next month') ? $timeValue = JSstrToTime( "+1 month" , JSstrToTime( date("F")."1") ) : JSstrToTime($dtAry[0]) ;
					    		$dateValue = date($GLOBALS['timedate']->dbDayFormat, $dateValue);
					    	}else{
					    		$dateValue='';
					    	}
					    	if(!empty($dtAry[1])){
					    		$timeValue = date($GLOBALS['timedate']->dbTimeFormat, JSstrToTime($dtAry[1]));
					    	}else{
					    		$timeValue = '';
					    	}
...
//    	require_once('data/Link.php');
	...
	*/
	function save($check_notify = FALSE)
	{
	    ...
	    elseif($_FILES[$field_name])
		{
			$file=$_FILES[$field_name];
			$deleteName="delete_".$field_name;
			if(is_array($file) && $file["size"] && !$this->$deleteName)
			{
				if($file["size"]>2097152)
				{
					$fileMaxSizeError[]=$field_name;
				}
				else
				{
					$fp=fopen($file["tmp_name"],"rb");
					$contentVar=fread($fp,$file["size"]);
					if($contentVar)
					{
						$leadFiles[]=array("field_name"=>$field_name,"file_name"=>$file["name"],"file_type"=>$file["type"],"file_content"=>addslashes($contentVar),"file_size"=>$file["size"]);
					}
				}
			}
			elseif($this->$deleteName)
			{
				$delArray[]=$field_name;
			}
			if(!$file["size"])
			{
				if($this->$field_name=='http://')
					$this->$field_name='';
			}
			unset($contentVar);
			unset($file);
		...
		//Added by Sadaf to default last name to any one contact detail
		if(!$this->last_name)
		{
			if($this->enquirer_email_id_c)
				$this->last_name=$this->enquirer_email_id_c;
			elseif($_POST["Leads0emailAddress0"])
				$this->last_name=$_POST["Leads0emailAddress0"];
			elseif($this->enquirer_mobile_no_c)
				$this->last_name=$this->enquirer_mobile_no_c;
			elseif($this->phone_mobile)
				$this->last_name=$this->phone_mobile;
			elseif($this->enquirer_landline_c)
				$this->last_name=$this->enquirer_landline_c;
			elseif($this->phone_home)
				$this->last_name=$this->phone_home;
			elseif($this->p_o_box_no_c)
				$this->last_name=$this->p_o_box_no_c;
		}
		if(!$this->assigned_user_id)
			$this->assigned_user_id=$current_user->id;
		//Added by Sadaf to update lead follow up count
		if($this->status=='12')
			$this->reg_followup_count_c+=1;
		//Added by Sadaf for truncating zeros before saving
                if($this->std_c)
                {
                        $this->std_c=ltrim($this->std_c,"0");
                        $this->std_c="0".$this->std_c;
                }
                if($this->std_enquirer_c)
                {
                        $this->std_enquirer_c=ltrim($this->std_enquirer_c,"0");
                        $this->std_enquirer_c="0".$this->std_enquirer_c;
                }
		//Added by Sadaf to link username to lead and send mail for retrieval of deleted profile in case of status "Already Registered"
		if($this->status=='46' && ($this->disposition_c=='21' || $this->disposition_c=='20' || $this->disposition_c=='25'))
		{
			if($this->jsprofileid_c!=$this->status_comments_c)
			{
				$this->jsprofileid_c=trim($this->status_comments_c);
				if($this->disposition_c=='20')
				{
					$email=$sugar_config["mail_deleted_profile_retrieval"];
					$subject="Deleted profile retrieval : $this->jsprofileid_c";
					$body="Please retrieve deleted profile : $this->jsprofileid_c for lead : $this->last_name";
					mail($email,$subject,$body);
				}
				/*If disposition is Registered Incomplete with another id, checking if leads exist with that other id in status Registered Incomplete and assigning all such leads to the current user and if present in partitions, moving to active partition*/
				if($this->disposition_c=='25' && !empty($this->current_user->id))
				{
					include("custom/crons/housekeepingConfig.php");
					$updateSql="UPDATE sugarcrm.leads JOIN sugarcrm.leads_cstm ON id=id_c SET assigned_user_id='$this->current_user->id',date_modified=NOW(),modified_by='$this->current_user->id' WHERE jsprofileid_c='$this->jsprofileid_c' AND status='24' AND disposition_c='23'";
					//$GLOBALS['log']->info("Updating lead info : $updateSql");
					$this->db->query($updateSql,true);
					global $partitionsArray;
					if(is_array($partitionsArray) && count($partitionsArray))
					{
						foreach($partitionsArray as $partition)
						{
							...
							$findSql="SELECT id FROM $leadsTable JOIN $leadsCstmTable ON id=id_c WHERE jsprofileid_c='$this->jsprofileid_c' AND status='24' AND disposition_c='23'";
							$GLOBALS['log']->info("Finding lead partition : ".$findSql);
							$findRes=$this->db->query($findSql);
							if($this->db->getRowCount($findRes))
							{
								while($findRow=$this->db->fetchByAssoc($findRes))
								{
									$updateSql="UPDATE $leadsTable JOIN $leadsCstmTable ON id=id_c SET assigned_user_id='$this->current_user->id',date_modified=NOW(),modified_by='$this->current_user->id' WHERE jsprofileid_c='$this->jsprofileid_c'";
									$GLOBALS['log']->info("Updating lead info : $updateSql");
				                                        $this->db->query($updateSql,true);
									moveLead($findRow['id'],$partition,'active');
								}
							}
						}
					}					
		...
		//Added by Sadaf to delete profile if status is saved as Registered - Retrieve profile after consent
		if($this->status=='26' && $this->disposition_c=='3' && $this->jsprofileid_c!='')
		{
			$leadProfileID=$this->getLeadProfileID();
			if($leadProfileID)
			{
		...
		//Added by Sadaf to save marriage bureau numbers to negative profile list
		if($this->status=='45' && $this->disposition_c=='32')
		{
			$insert=0;
			...
			if($insert)
			{
				$matchedBy=trim($matchedBy,",");
				$type="Marriage Bureau";
				$insertSql="INSERT INTO incentive.NEGATIVE_PROFILE_LIST(TYPE,EMAIL,MOBILE,ISD,STD_CODE,LANDLINE,ADDRESS,MATCHED_BY,ENTRY_DT) VALUES('$type','$_POST[Leads0emailAddress0]','$this->phone_mobile','$this->isd_c','$this->std_c','$this->phone_home','$this->primary_address_postalcode','$matchedBy',NOW())";
				$GLOBALS['log']->info("Query to insert into negative list : $insertSql");
				$this->db->query($insertSql,true);
				if(($this->std_c && $this->phone_home) || $this->phone_mobile)
				{
					$phoneStr='';
					if($this->phone_mobile)
						$phoneStr =" ('$this->phone_mobile')";
					if($this->phone_home)
					{
						$phone_with_std =$this->std_c.$this->phone_home;
						if($this->phone_home && $this->phone_mobile)
							$phoneStr .=",";
						$phoneStr .=" ('$phone_with_std')";
					}
					$insertSql ="INSERT IGNORE INTO newjs.ABUSIVE_PHONE (PHONE_WITH_STD) VALUES".$phoneStr;
					$GLOBALS['log']->info("Query to insert into abusive list : $insertSql");
					$this->db->query($insertSql,true);
				}
				require_once("../profile/connect_db.php");
				...
				if($this->phone_mobile)
					updateNegativeProfiles('PHONE_MOB','MOBILE',$this->phone_mobile,'',$db,$dataSetArr);
				if($this->phone_home && $this->std_c)
					updateNegativeProfiles('PHONE_WITH_STD','LANDLINE',$this->phone_home,$this->std_c,$db,$dataSetArr);
			}
			...
			if($this->std_enquirer_c && $this->enquirer_landline_c)
			{
				$matchedBy.='LANDLINE,';
				$insert=1;
			}
			if($this->enquirer_mobile_no_c)
			{
				$matchedBy.='MOBILE,';
				$insert=1;
			}
			if($insert)
			{
				$matchedBy=trim($matchedBy,",");
				$type="Marriage Bureau";
				$insertSql="INSERT INTO incentive.NEGATIVE_PROFILE_LIST(TYPE,EMAIL,MOBILE,ISD,STD_CODE,LANDLINE,MATCHED_BY,ENTRY_DT) VALUES('$type','$this->enquirer_email_id_c','$this->enquirer_mobile_no_c','$this->isd_enquirer_c','$this->std_enquirer_c','$this->enquirer_landline_c','$matchedBy',NOW())";
				$GLOBALS['log']->info("Query to insert into negative list : $insertSql");
				$this->db->query($insertSql,true);
				if(($this->std_enquirer_c && $this->enquirer_landline_c) || $this->enquirer_mobile_no_c)
				{
					$phoneStr='';
					if($this->enquirer_mobile_no_c)
						$phoneStr =" ('$this->enquirer_mobile_no_c')";
					if($this->enquirer_landline_c)
					{
						$phone_with_std =$this->std_enquirer_c.$this->enquirer_landline_c;
						if($this->enquirer_landline_c && $this->enquirer_mobile_no_c)
							$phoneStr .=",";
						$phoneStr .=" ('$phone_with_std')";
					}
					$insertSql ="INSERT IGNORE INTO newjs.ABUSIVE_PHONE (PHONE_WITH_STD) VALUES".$phoneStr;
				...
				$db=connect_db();
				$dataSetArr =array("TYPE"=>"Marriage Bureau","MATCHED_BY"=>"$matchedBy");
				if($this->enquirer_email_id_c)
					updateNegativeProfiles('EMAIL','EMAIL',$this->enquirer_email_id_c,'',$db,$dataSetArr);
				if($this->enquirer_mobile_no_c)
					updateNegativeProfiles('PHONE_MOB','MOBILE',$this->enquirer_mobile_no_c,'',$db,$dataSetArr);
				if($this->enquirer_landline_c && $this->std_enquirer_c)
					updateNegativeProfiles('PHONE_WITH_STD','LANDLINE',$this->enquirer_landline_c,$this->std_enquirer_c,$db,$dataSetArr);
			}
		}
		...
		if ($this->db->dbType == 'mysql')
		{
			...
			$firstPass = 0;
			//Added by Neha
                        if(isset($this->campaign_id) && $current_user->is_ops())
                        {
                                $camp_sql="SELECT assigned_user_id FROM campaigns WHERE id='".$this->db->quote($this->campaign_id)."' ";
                                $camp_res = $this->db->query($camp_sql);
                                $camp_row = $this->db->fetchByAssoc($camp_res);
                                $camp_user = $camp_row['assigned_user_id'];
                                if($camp_user!='')
                                        $this->assigned_user_id=$camp_user;
                        }
			foreach($this->field_defs as $field=>$value)
			{
			...
			if(is_array($leadFiles) && $this->id)
			{
				$fileQuery="REPLACE INTO sugarcrm.lead_files(lead_id,field_name,file_name,file_type,file_content,file_size) VALUES ";
				foreach($leadFiles as $leadFile)
				{
					$valueString="('$this->id','$leadFile[field_name]','$leadFile[file_name]','$leadFile[file_type]','$leadFile[file_content]','$leadFile[file_size]')";
					$fileQuery.=$valueString.",";
					$fieldNameArr[]=$leadFile["field_name"];
				}
				$fileQuery=substr($fileQuery,0,strlen($fileQuery)-1);
				$GLOBALS['log']->info("Lead file entries: $fileQuery");
				$this->db->query($fileQuery,true);
				$idQuery="SELECT file_id,field_name FROM sugarcrm.lead_files WHERE lead_id='$this->id' AND field_name IN('".implode("','",$fieldNameArr)."')";
				$idRes=$this->db->query($idQuery,true);
				if($this->db->getRowCount($idRes))
				{
					$idUpdateQuery="UPDATE sugarcrm.leads_cstm SET ";
					while($idRow=$this->db->fetchByAssoc($idRes))
					{
						$idUpdateQuery.=" $idRow[field_name]=\"".$sugar_config["site_url"]."/include/utils/JS_download.php?file_id=$idRow[file_id]&lead_id=$this->id\",";
					}
					$idUpdateQuery=substr($idUpdateQuery,0,strlen($idUpdateQuery)-1);
					$idUpdateQuery.=" WHERE id_c='$this->id'";
					$this->db->query($idUpdateQuery,true);
				}
			}
			if(is_array($delArray))
			{
				$delQuery="DELETE FROM sugarcrm.lead_files WHERE lead_id='$this->id' AND field_name IN('".implode("','",$delArray)."')";
                                $GLOBALS['log']->info("Deleting file entries: $delQuery");
                                $this->db->query($delQuery,true);
				$delUpdateQuery="UPDATE sugarcrm.leads_cstm SET ";
				foreach($delArray as $del)
				{
					$delUpdateArr[]="$del='',delete_$del=''";
				}
				$delUpdateQuery.=implode(",",$delUpdateArr);
				$delUpdateQuery.=" WHERE id_c='$this->id'";
                                $GLOBALS['log']->info("Updating deleted file entries: $delUpdateQuery");
                                $this->db->query($delUpdateQuery,true);
			}
		}
	...
	function mark_deleted($id)
	{
		...
		if ( isset($this->field_defs['modified_user_id']) ) 
		{
			...
			if($_REQUEST['module']=='Leads')
				$query = "UPDATE $this->table_name set deleted=1 , date_modified = '$date_modified', modified_user_id = '$this->modified_user_id', status='32' where id='$id'";
			else
		...
		{
			if($_REQUEST['module']=='Leads')
				$query = "UPDATE $this->table_name set deleted=1 , date_modified = '$date_modified', status='32' where id='$id'";
			else
			...
			if($_REQUEST['module']=='Leads')
			{
				$query = "UPDATE sugarcrm.leads_cstm set disposition_c='28' where id_c='$id'";
				$this->db->query($query, true,"Error marking record deleted in leads_cstm ");
			}
...
//					$this->format_field($value);
    ...
    */
	...
	function isOwner($user_id)
	{
		...
		{
			//Modified by Neha to implement hierarcy
                        global $current_user;
			if($this->assigned_user_id == $user_id) return true;
			elseif($current_user->is_manager())
                        {
                                $query = "SELECT id from users  where reports_to_id='$user_id' and deleted=0";
                                $result = $this->db->query($query, true, "Error while retrieving executives");
                                while($row = $this->db->fetchByAssoc($result))
                                {
                                        if($row['id']==$this->assigned_user_id)
                                                return true;
                                }
                        }
                        // End of code by Neha
