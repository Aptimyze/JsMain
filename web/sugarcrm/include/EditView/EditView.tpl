...
var stat='';
var dispositionVal='';
var dispositionLabel='';
var statusDispositionArr=new Array;
var dispositionArr=new Array;
var statIndex=0;
{{foreach from=$dispositionFullArray key=statusValue item=dispositionArray}}
	stat={{$statusValue}};
	statIndex=0;
	//alert(stat);
	//dispositionArr=new Array;
	statusDispositionArr[stat]=new Array;
	{{foreach from=$dispositionArray key=dispositionValue item=dispositionText}}
	dispositionArr=new Array;
	dispositionVal={{$dispositionValue}};
	dispositionLabel="{{$dispositionText}}";
	dispositionArr[dispositionVal]=dispositionLabel;
	statusDispositionArr[stat][statIndex++]=dispositionArr;
	{{/foreach}}
	//statusDispositionArr[stat]=dispositionArr;
{{/foreach}}
{{else}}
var statusDispositionArr=new Array;
statusDispositionArr[0]=0;
{{/if}}
</script>
...
<script type="text/javascript">
{{if $returnModule eq 'Leads'}}
$("#college_name_c").autocomplete("{{$HOST_NAME}}/profile/autoSug?type=collg",{ldelim}maxItemsToShow:10,field:'#anu'{rdelim});
$("#school_name_c").autocomplete("{{$HOST_NAME}}/profile/autoSug?type=school",{ldelim}maxItemsToShow:10,field:'#gau'{rdelim});
$("#current_employer_c").autocomplete("{{$HOST_NAME}}/profile/autoSug?type=org",{ldelim}maxItemsToShow:10,field:'#organ'{rdelim});
{{/if}}
</script>
