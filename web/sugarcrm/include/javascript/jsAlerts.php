...
EOQ;
	...
	function addActivities(){
		...
		while($row = $db->fetchByAssoc($result)) {
			...
			$timeStart = JSstrToTime($row['date_start']);
			...
			$this->addAlert($app_strings['MSG_JS_ALERT_MTG_REMINDER_MEETING'], $meetingName, 
				...
				$description. 
				$instructions, 
				$timeStart - JSstrToTime($dateTimeNow), 
				$url
			);
		}
		...
		while($row = $db->fetchByAssoc($result)){
			...
			$this->addAlert($app_strings['MSG_JS_ALERT_MTG_REMINDER_CALL'], $row['name'], $app_strings['MSG_JS_ALERT_MTG_REMINDER_TIME'].$timedate->to_display_date_time($row['date_start']) , $app_strings['MSG_JS_ALERT_MTG_REMINDER_DESC'].$row['description']. $app_strings['MSG_JS_ALERT_MTG_REMINDER_MSG'] , $timeStart - JSstrToTime($dateTimeNow), 'index.php?action=DetailView&module=Calls&record=' . $row['id']);
		}
	}
