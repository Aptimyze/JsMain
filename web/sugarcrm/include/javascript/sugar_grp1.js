...
function add_error_style(formname,input,txt){try{inputHandle=document.forms[formname][input];style=get_current_bgcolor(inputHandle);if(txt.substring(txt.length-1)==':')
...
{
	var phone_home=trim(document.getElementById('phone_home').value);
	var phone_mobile=trim(document.getElementById('phone_mobile').value);
 	var enquirer_landline=trim(document.getElementById('enquirer_landline_c').value);
 	var pobox_no=trim(document.getElementById('p_o_box_no_c').value);
	var enquirer_mobile=trim(document.getElementById('enquirer_mobile_no_c').value);
	var lead_email=trim(document.getElementById('Leads0emailAddress0').value);
	var enquirer_email=trim(document.getElementById('enquirer_email_id_c').value);
	var std=trim(document.getElementById('std_c').value);
	var isd=trim(document.getElementById('isd_c').value);
	var enq_std=trim(document.getElementById('std_enquirer_c').value);
	var enq_isd=trim(document.getElementById('isd_enquirer_c').value);
	var err_email=document.getElementById('err').innerHTML;
	var err_enq_email=document.getElementById('result4').innerHTML;
	var err_ll=document.getElementById('result').innerHTML;
	var err_mob=document.getElementById('result1').innerHTML;
	var err_enq_ll=document.getElementById('result2').innerHTML;
	var err_enq_mob=document.getElementById('result3').innerHTML;
	var school_name=trim(document.getElementById('school_name_c').value);
	var college_name=trim(document.getElementById('college_name_c').value);
	var current_employer=trim(document.getElementById('current_employer_c').value);
	var about_the_profile=trim(document.getElementById('about_the_profile_c').value);
	var modeElement=document.getElementById('response_delivery_mode_c');
	var mode=modeElement.options[modeElement.selectedIndex].label;
	var dateOfBirth=document.getElementById('date_birth_c').value;
	var noOfBrothers=document.getElementById('no_of_brothers_c').value;
	var noOfBrothersM=document.getElementById('no_of_brothers_married_c').value;
	var noOfSisters=document.getElementById('no_of_sisters_c').value;
        var noOfSistersM=document.getElementById('no_of_sisters_married_c').value;
	var gender=document.getElementsByName('gender_c');
	var genderValue;
	...
	{
		alert("Gender is mandatory field")
		return false;
	}
	else
	{
		if(dateOfBirth)
		{
			if(dateOfBirth.match(/^\d{2}\/\d{2}\/\d{4}$/))
			{
				...
				var today=new Date();
				var daydiff=Math.floor((today.getTime()-dob.getTime())/(1000*60*60*24));
				if(genderValue=='M' && daydiff<7665)
				{
					alert("Minimum age for Males is 21");
					return false;
				}
				if(genderValue=='F' && daydiff<6570)
				{
					alert("Minimum age for Females is 18");	
					return false;
				}					
			}
			else
	...
	//if(trim(document.getElementById('phone_home').value)=='' && trim(document.getElementById('Leads0emailAddress0').value)=='' && trim(document.getElementById('enquirer_mobile_no_c').value)=='' && trim(document.getElementById('enquirer_landline_c').value)=='' && trim(document.getElementById('p_o_box_no_c').value)=='' && document.getElementById('enquirer_email_id_c').value=='')
	if(!phone_home && !phone_mobile && !enquirer_landline && !pobox_no && !enquirer_mobile && !lead_email && !enquirer_email)
        {
 	       alert('Atleast 1 contact detail is required');
               return false;
        }
	if(phone_mobile!='')
	{
		if(!isNumeric(phone_mobile)) 
		{
		...
		{
			alert('Mobile should be 10 digit number');
				return false;
		}
	}
	if(phone_home)
	{
		...
		{
			alert('Landline should be numeric');
			return false;
		}
	...
	if(enquirer_mobile!='')
        {
                if(!isNumeric(enquirer_mobile))
                {
                        alert('Enquirer mobile should be numeric');
                        return false;
                }
                if(enquirer_mobile.length!='10')
                {
                        alert('Enquirer mobile should be 10 digit number');
                                return false;
                }
        }
        if(enquirer_landline)
        {
                if(!isNumeric(enquirer_landline))
                {
                        alert('Enquirer landline should be numeric');
                        return false;
                }
	...
	if(std)
        {
                if(!isNumeric(std))
                {
                        alert('Std should be numeric');
                        return false;
                }
		if(!phone_home)
		{
			alert("Please enter landline for given std");
			return false;	
		}
        }
	if(isd)
		...
		if(!phone_home)
                {
                        document.getElementById('isd_c').value='';
                }
		else
		{
			var isd_char=isd.substr(0,1);
			var isd_val=isd.substr(1,isd.length);
			if(!isNumeric(isd_val) || (isd_char!='+' && !isNumeric(isd_char)))
			{
				alert('Isd should be numeric preceeded by +');
				return false;
			}
			if(isd_char=='+')
			{
				document.getElementById('isd_c').value=isd_val;
			}
		}
        }
	if(enq_std)
        {
                if(!isNumeric(enq_std))
                {
                        alert('Std should be numeric');
                        return false;
                }
	...
	if(enq_isd)
		...
		if(!enquirer_landline)
                {
			document.getElementById('isd_enquirer_c').value='';
                }
		else
		{
			var enq_isd_char=enq_isd.substr(0,1);
	                var enq_isd_val=enq_isd.substr(1,enq_isd.length);
			if(!isNumeric(enq_isd_val) || (enq_isd_char!='+' && !isNumeric(enq_isd_char)))
                	{
                        	alert('Isd should be numeric and can be preceeded by +');
	                        return false;
        	        }
			if(enq_isd_char=='+')
                	{
                        	document.getElementById('isd_enquirer_c').value=enq_isd_val;
                	}
		}
        }
	if(document.getElementById('primary_address_postalcode'))
	{
		var pin=document.getElementById('primary_address_postalcode').value;
		if(pin !='' && pin.length!='6')
		{
	...
	if(school_name.length && school_name.length<3)
	{
		alert("School Name cannot be lesser than 3 characters");
		return false;
	}
	if(college_name.length && college_name.length<3)
        {
                alert("College Name cannot be lesser than 3 characters");
                return false;
        }
	if(current_employer.length && current_employer.length<3)
        {
                alert("Current Employer name cannot be lesser than 3 characters");
                return false;
        }
	if(about_the_profile.length && about_the_profile.length<100)
        {
	...
	if(mode=='Both')
	{
		if((lead_email=='' && enquirer_email=='') || ((document.getElementById('primary_address_street').value=='' || document.getElementById('primary_address_postalcode').value=='') && pobox_no==''))
                {       
                        alert("Email address and P.O Box No. or Address with pin code needed for response delivery mode - 'Both'");
                        return false;
                }       
	}
	else
	{
		if(mode=='Email')
		{
			if(lead_email=='' && enquirer_email=='')
			{
				alert("Lead or enquirer email id required for response delivery mode = 'Email'");
				return false;
			}
		}
		else
		if(mode=='Courier')
		{
			...
			{
				alert("P.O.Box No. or Address with pin code is required for response delivery mode - 'Courier'");
				return false;
			}
	...
	for(id=1;id<=6;id++)
	{
		var comp_id_field=document.getElementById("competition_id_"+id+"_c");
		var comp_id_site=document.getElementById("competition_site_"+id+"_c");
		if(comp_id_field && comp_id_site)
		{
			var comp_id_field_val=trim(comp_id_field.value);
			var comp_id_site_val=comp_id_site.value;
			if(comp_id_field_val!='')
			{
				if(comp_id_site_val==0)
				{
					alert("Site value missing for competition Id "+id);
					return false;
				}
			}
		}
	}
	var mstatus=document.getElementById('marital_status_c');
	if(mstatus)
	{
		var mstatus_val=mstatus.value;
		if(mstatus_val=='0' || mstatus_val=='N')
		{	
			var children=document.getElementById('have_children_c');
			if(children)
			{
				var children_val=children.checked;
				if(children_val)
				{
					alert("Have children field cannot be checked for a non married status");
					return false;
				}
			}
		}
	...
	if(noOfBrothers)
	{
		if(noOfBrothersM>noOfBrothers)
		{
			alert("No. of brothers married cannot be greater than no. of brothers");
			return false;
		}
	}
	if(noOfSisters)
        {
                if(noOfSistersM>noOfSisters)
		{
                	alert("No. of sisters married cannot be greater than no. of sisters");
	                return false;
		}
        }
	if(err_email.length>1 || err_enq_email.length>1 || err_ll.length>1 || err_mob.length>1 || err_enq_ll.length>1 || err_enq_mob.length>1)
	{
		alert("Errors exist!");
		return false;
	}
...
//End by Neha
//Function added by Sadaf to implement checks on campaign form
function add_checks_campaign()
{
	var forbiddenArray=new Array("matrimony","shaadi\.com","simplymarry\.com","agarwal2agarwal","maheshwari2maheshwari");
	var name=document.getElementById("name").value;
	if(name.match(/shaadi\.com/gi) || name.match(/matrimony/gi) || name.match(/simplymarry\.com/gi) || name.match(/agarwal2agarwal/gi) || name.match(/maheshwari2maheshwari/gi))
	{
		alert("Campaign name contains forbidden keyword");
		return false;
	}
}       
...
return'';}
function unformatNumberNoParse(n,num_grp_sep,dec_sep){if(typeof num_grp_sep=='undefined'||typeof dec_sep=='undefined')return n;n=n?n.toString():'';if(n.length>0){n=n.replace(num_grp_sep,'').replace(dec_sep,'.');if(typeof CurrencySymbols!='undefined'){for(var idx in CurrencySymbols){n=n.replace(CurrencySymbols[idx],'');}}
return n;}
return'';}
...
return false;}// End of File include/javascript/sugar_3.js
