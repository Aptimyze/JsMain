 ...
 class SearchForm extends EditView{
  ...
  function displaySavedSearchSelect(){
	...
	function generateSearchWhere($add_custom_fields = false, $module='') {
        global $timedate;
	global $app_list_strings; // Added by Neha
		...
		//Code added by Neha
		if(in_array($field,$app_list_strings['skip_value']))
			$skip_value=1;
		else
			$skip_value=0;
		// end by Neha
                ...
                if(is_array($parms['value'])) {
                    ...
                    // always construct the where clause for multiselects using the 'like' form to handle combinations of multiple $vals and multiple $parms
                     if(/*$GLOBALS['db']->dbType != 'mysql' &&*/ !empty($this->seed->field_name_map[$field]['isMultiSelect']) && $this->seed->field_name_map[$field]['isMultiSelect']) {
	                    ...
	                    foreach($parms['value'] as $key => $val) {
				// Added by Neha to search nothing when 'Please select' choosen
				if($skip_value and $val=='0')
                                        continue;
	                        if($val != ' ' and $val != '') {
	                            if (!empty($field_value)) {
	                                $field_value .= ',';
                ...
                if($field_value != '') {
                    ...
                    foreach ($parms['db_field'] as $db_field) {
                        ...
                        if($type == 'datetime' || $type == 'datetimecombo') {//bug 22564, date type field may also have this problem. we may add a date type here.
                            ...
                            $temp_offset = JSstrToTime($timedate->swap_formats($timedate->to_display_date_time($field_value." 00:00:00"),$timedate->get_date_time_format(),$timedate->get_db_date_time_format())) - JSstrToTime($field_value." 00:00:00");
                            $start_datetime = date("Y-m-d H:i:s", JSstrToTime($field_value." 00:00:00") - $temp_offset);
                            $end_datetime = date("Y-m-d H:i:s", JSstrToTime($field_value." 23:59:59") - $temp_offset);
