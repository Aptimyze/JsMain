...
function findAllTouchedFiles( $the_dir, $the_array, $date_modified, $filter=''){
    ...
    while (false !== ($f = $d->read())) {
        ...
        if( is_dir( "$the_dir/$f" ) ){
        	...
        	$file_date = date('Y-m-d H:i:s', JSstrToTime(date('Y-m-d H:i:s', filemtime("$the_dir/$f"))) - date('Z'));

        	if(JSstrToTime($file_date) > JSstrToTime($date_modified) && (empty($filter) || preg_match($filter, $f))){
        		 array_push( $the_array, "$the_dir/$f");
        	}
        }
    }
