...
class TimeDate {
	 ...
	 */
	function getDateFromRules($year, $startMonth, $startDate, $weekday, $startTime ){
		if($weekday < 0)return date( 'Y-m-d H:i:s', JSstrToTime("$year-$startMonth-$startDate") + $startTime);
		...
		return date( 'Y-m-d H:i:s', JSstrToTime( "$startWeek week $dayname", JSstrToTime( "$month 1, $year" ) ) + $startTime );
	...
	function inDST($date, $zone){
		...
		$datestamp = JSstrToTime($date);
		$startstamp = JSstrToTime($dstRange['start']);
		$endstamp = JSstrToTime($dstRange['end']);
...
***************************************************************/
	...
	function handle_offset($date, $format, $to = true, $user = null, $usetimezone = null) {
		...
		$result = date($format, JSstrToTime($date) + $deltaServerUser * 3600 + ($ue + $deltaServerGMT) * $zone);
	...
	function handle_offset_depricated($date, $format, $to = true) {
		...
		return date($format, JSstrToTime($date) + $this->get_hour_offset($to) * 60 * 60 + $zone);
	}
	...
	function handleOffsetMax($date, $format, $to = true) {
		...
		/* this ridiculousness b/c PHP returns current time when passing "today"
			or "tomorrow" as JSstrToTime() args */
		$dateNoTime = date('Y-m-d', JSstrToTime($date));
	...
	function convert_to_gmt_datetime($olddatetime) {
		if (!empty($olddatetime)) {
			return date('Y-m-d H:i:s', JSstrToTime($olddatetime) - date('Z'));
		}
	}
