...
*/
include_once('android_tablets_list.php');
function mobile_device_detect($user_agent,$iphone="true",$ipad="true",$android="true",$opera="true",$blackberry="true",$palm="true",$windows="true",$mobileredirect=false,$desktopredirect=false){
  ...
  switch(true){ // using a switch against the following statements which could return true is more efficient than the previous method of using if statements
	...
	case (preg_match('/tab|pad|kindle|htc_flyer_p51|kfjwi|kfot|kftt|kobo touch|homemanager|hpwos|tf101|x11;linux armv6l/i',$user_agent));
		$mobile_browser=true;
		$status='tablet';
		break;
	...
	//Js changes- In android, it can be a tablet browser Status will be Android Tablet is it is tablet else simple android
		preg_match('/.*;(.*)\).*Appl/i',$user_agent,$matches);
		$android_device=trim($matches[1]);
		if($android_device){
			if(@in_array($android_device,$android_tablets))
				$status='tablet';	
			else
      			$status = 'Android';
		}
    ...
    exit;
		...
		if($mobile_browser==''){
			...
			if($status== 'tablet')
				...
				$is_tablet="false";
			return array('mobileBrowser'=>$mobile_browser,'is_tablet'=>$is_tablet); // is a mobile so we are returning an array ['0'] is true ['1'] is the $status value
		}
	}
