...
if(authenticated($cid))
{
	...
	if($CMDGo)
	{
		...
		//$sql="SELECT b.BILLID,j.COUNTRY_RES,j.CITY_RES,SUM(if(b.TYPE='DOL',b.AMOUNT*b.DOL_CONV_RATE,b.AMOUNT)) AS AMOUNT,DAYOFMONTH(b.ENTRY_DT) as dd FROM billing.PAYMENT_DETAIL b, newjs.JPROFILE j,billing.PURCHASES c WHERE b.ENTRY_DT BETWEEN '$st_date' AND '$end_date' AND b.STATUS = 'DONE' AND b.PROFILEID=j.PROFILEID AND b.BILLID = c.BILLID AND c.MEMBERSHIP = 'Y'  AND c.STATUS='DONE' GROUP BY dd,j.COUNTRY_RES,j.CITY_RES";
		$sql="SELECT b.BILLID,j.COUNTRY_RES,j.CITY_RES,SUM(if(b.TYPE='DOL',b.AMOUNT*b.DOL_CONV_RATE,b.AMOUNT)) AS AMOUNT,DAYOFMONTH(b.ENTRY_DT) as dd FROM billing.PAYMENT_DETAIL b, newjs.JPROFILE j WHERE b.ENTRY_DT BETWEEN '$st_date' AND '$end_date' AND b.STATUS = 'DONE' AND b.PROFILEID=j.PROFILEID GROUP BY dd,j.COUNTRY_RES,j.CITY_RES";
		...
		if($row=mysql_fetch_array($res))
		{
			...
			while($row=mysql_fetch_array($res))
			{
				$billid=$row['BILLID'];
		                unset($valid_id);
                		$valid_id = check_validity_n($billid);
				$lavesh1++;	
                		if($valid_id==1)
                		{
						$aman[]=$billid;
...
//echo $haha;
		...
		$smarty->display("crm_revenue_zonewise_new.htm");
	}
	else
	{
		$user=getname($cid);
...
function check_validity_n($bid)
{
	global $db;
        $sql_check="SELECT MEMBERSHIP FROM billing.PURCHASES WHERE BILLID='$bid' AND STATUS='DONE'";
        $res_check=mysql_query($sql_check,$db) or logError($sql_check);
        $row_check=mysql_fetch_array($res_check);
        if($row_check["MEMBERSHIP"]!='Y')
                return 0;
        else
                return 1;
}
