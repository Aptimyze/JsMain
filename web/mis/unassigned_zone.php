...
if(isset($data))
{
	...
	{
		if($val=="new")
		$sql = "SELECT MOD_DT FROM $table J LEFT JOIN newjs.JPROFILE_CONTACT C ON J.PROFILEID=C.PROFILEID WHERE ACTIVATED='N' AND INCOMPLETE='N' AND (J.MOB_STATUS='Y' OR J.LANDL_STATUS='Y' OR C.ALT_MOB_STATUS='Y')";
	elseif($val=="edit")
		$sql="SELECT MOD_DT FROM $table USE INDEX(SCREENING) LEFT JOIN jsadmin.MAIN_ADMIN ON jsadmin.MAIN_ADMIN.PROFILEID = $table.PROFILEID WHERE (jsadmin.MAIN_ADMIN.PROFILEID IS NULL) AND (ACTIVATED = 'Y' AND SCREENING < '1099511627775')";
	}
