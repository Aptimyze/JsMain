	...
	if($CMDGo)
	{
		...
		if($row=mysql_fetch_array($res))
		{
			do
			{
				...
				$sql="SELECT ALLOTED_TO FROM incentive.MAIN_ADMIN_APR WHERE PROFILEID='$profileid'";
		...
		$smarty->display("crm_users_revenue_new_apr.htm");
	}
	else
	{
		$user=getname($cid);
                ...
                for($i=2004;$i<=date("Y");$i++)
{
        $yyarr[$i-2004]=$i;
                ...
                if(1)//in_array('MA',$priv) || in_array('MB',$priv))
                {
