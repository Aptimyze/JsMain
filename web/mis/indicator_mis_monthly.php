...
for($loopday=1;$loopday<=$thisday;$loopday++)
{
        ...
        {
                //Sharding
                $myDbName=$slave_activeServers[$activeServerId];
	...
	for($activeServerId=0;$activeServerId<$noOfActiveServers;$activeServerId++)
        {
                ...
                $mysqlObj->ping($myDbArray[$myDbName]);
                $myDb=$myDbArray[$myDbName];
	        $sql="SELECT SENDER,RECEIVER FROM newjs.CONTACTS as ct ,newjs.PROFILEID_SERVER_MAPPING as psm WHERE ct.SENDER=psm.PROFILEID  and TIME BETWEEN '".$year."-".$month."-".$day." 00:00:00' AND '".$year."-".$month."-".$day." 23:59:59' AND TYPE = 'I' and psm.SERVERID=$activeServerId";
        	$res=$mysqlObj->fetchArray($sql,$myDb) or die(mysql_error_js($myDb));
        	while($row=mysql_fetch_array($res))
        	{
