...
{	
        ...
        if ($submit)
	{
			...
			{					
				...
				$conv_sql = "SELECT COUNT(*) AS CNT, newjs.JPROFILE.SUBSCRIPTION as SUBSCRIPTION, MONTH(jsadmin.AFFILIATE_MAIN.ENTRYTIME) as mm , jsadmin.AFFILIATE_MAIN.ENTRYBY AS ENTRYBY FROM jsadmin.AFFILIATE_MAIN LEFT JOIN newjs.JPROFILE ON jsadmin.AFFILIATE_MAIN.EMAIL=newjs.JPROFILE.EMAIL WHERE JPROFILE.EMAIL IS NOT NULL";
				...
				$conv_sql.= " AND jsadmin.AFFILIATE_MAIN.ENTRYTIME BETWEEN '$myear-01-01' AND '$myear-12-31' GROUP BY newjs.JPROFILE.SUBSCRIPTION , mm , jsadmin.AFFILIATE_MAIN.ENTRYBY ";
                                ...
                                $conv_res = mysql_query_decide($conv_sql,$db) or die("$sql".mysql_error_js());
