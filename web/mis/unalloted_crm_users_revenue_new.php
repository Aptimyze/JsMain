...
while($row=mysql_fetch_assoc($res))
{
	...
	if($row1=mysql_fetch_array($res1))
	{
		...
		$sql="SELECT STATUS, if(TYPE='DOL',AMOUNT*$DOL_CONV_RATE,AMOUNT) AS AMOUNT FROM billing.PAYMENT_DETAIL WHERE PROFILEID='$pid' AND ENTRY_DT BETWEEN '$allot_time' AND DATE_ADD('$allot_time', INTERVAL $relax_days DAY) AND STATUS IN ('DONE','REFUND')";
		$res1=mysql_query_decide($sql,$db) or die("$sql".mysql_error_js($db));
