...
if(authenticated($cid))
{
	...
	function getRevenueDetail($reportType, $reportPeriod, $st_date, $end_date, $branch='',$tableName='')
	{
		...
		if($branch!='')	
			$sql = "SELECT pd.`SERVICEID`, SUM(IF(pd.CUR_TYPE =  'DOL',  `NET_AMOUNT` * DOL_CONV_RATE,  `NET_AMOUNT`)) AS AMT, $reportPeriod( pm.ENTRY_DT ) AS DD, MEM_UPGRADE AS UPGRADE
			...
			while($row = mysql_fetch_array($res))
			{
				...
				else
				{
					...
					$serviceWiseArr[$row['SERVICEID']]['TOTAL'] += $amt;
                        ...
                        if($branch=='')
                            $sql = "SELECT pur.`SERVICEID`, SUM( IF (pm.TYPE = 'DOL', AMOUNT * DOL_CONV_RATE, AMOUNT)) AS AMT, $reportPeriod( pm.ENTRY_DT ) AS DD, MEM_UPGRADE AS UPGRADE FROM billing.PURCHASES AS pur, billing.$tableName AS pm WHERE pur.BILLID = pm.BILLID AND pm.ENTRY_DT >= '$st_date' AND pm.ENTRY_DT <= '$end_date' AND pm.STATUS $condition AND pur.SERVICEID LIKE 'ES%' GROUP BY pur.`SERVICEID`, DD, UPGRADE";
                        else
...
//			}
		...
		else
		{
			...
			$sql = "SELECT pur.`SERVICEID`, SUM( IF (pm.TYPE = 'DOL', AMOUNT * DOL_CONV_RATE, AMOUNT)) AS AMT, $reportPeriod( pm.ENTRY_DT ) AS DD FROM billing.PURCHASES AS pur, billing.$tableName AS pm WHERE pur.BILLID = pm.BILLID AND pm.ENTRY_DT >= '$st_date' AND pm.ENTRY_DT <= '$end_date' AND pm.STATUS $condition AND pur.SERVICEID LIKE 'ES%' GROUP BY pur.`SERVICEID`, DD";
			$res = mysql_query_decide($sql) or die(mysql_error_js());
			while($row = mysql_fetch_array($res))
			{
				$sid = $row['SERVICEID'];
