...
$result_id=mysql_query_decide($sql) or logError("Error1:",$sql);
...
$result_id=mysql_query_decide($sql)  or logError("Error2:",$sql);
while($row_id=mysql_fetch_array($result_id)){
	$pid=$row_id['PROFILEID'];
...
function addEmailLog($profile, $deliveryStatus='', $curdate)
{
	...
	mysql_query_decide($sqlMailAdd) or  die("$sqlMailAdd".mysql_error_js());
}
