...
if(isset($data))
{
	...
	$res_user=mysql_query_decide($sql_user) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception(mysql_error_js()));
	...
	if(trim($bank_name))
	{
		if($submit_add)
		{
			...
			$res_ins=mysql_query_decide($sql_ins) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception(mysql_error_js()));
		...
		else if($submit_mod)
		{
			...
			$res_mod=mysql_query_decide($sql_mod) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception(mysql_error_js()));
