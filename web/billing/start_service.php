...
if(isset($data))
{
	...
	if($sendmail)
	{
		...
		elseif($SendMail)
		{
			...
			mysql_query_decide($sql_upd,$db) or mysql_error_js();
	...
	elseif($CMDSearch)
	{
		...
		$res_s=mysql_query_decide($sql_s,$db_slave) or mysql_error_js();
		$today=date("Y-m-d");
		$i=0;
		while($row_s=mysql_fetch_array($res_s))
		{
				...
				$res_ph=mysql_query_decide($sql_ph,$db_slave) or mysql_error_js();
	...
	elseif($CMDGo)
	{
		...
		$result_chk = mysql_query_decide($sql_chk,$db_slave) or mysql_error_js();
	...
	//		$dep_branch=$row["PICKUP_TYPE"];
			...
			$result=mysql_query_decide($sql,$db_slave) or mysql_error_js();
			...
			mysql_query_decide($sql_u,$db) or mysql_error_js();
