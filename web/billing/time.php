...
{	
	...
	if(($time_diff_in_minutes<$third_time*60) || ($time_diff_in_minutes<0) )
	{
...
function newtime($t1,$d,$h,$m)
{
	...
	if($t1_hour>=$office_hour2)
		$t1=strftime("%Y-%m-%d $office_hour1:00",JSstrToTime("$t1 + 1 days"));
	if($t1_hour<$office_hour1)
		$t1=strftime("%Y-%m-%d $office_hour1:00",JSstrToTime("$t1"));
	...
	$sql= "SELECT count(DATE) NUM from HOLIDAY where DATE>'$t1' and DATE<='$newdate'";
	$result=mysql_query_decide($sql);
	$myrow=mysql_fetch_row($result);
	$holidays=$myrow[0];
	...
	while($flag==0)
	{				
		$sql1= "SELECT count(DATE) NUM from HOLIDAY where DATE='$return_date'";
...
$t1="2004-06-25 09:08";//$argv[1];
$t2="2004-06-25 19:00";//$argv[2];
echo time_difference($t1, $t2,&$time_diff)."\n";*/
?>
