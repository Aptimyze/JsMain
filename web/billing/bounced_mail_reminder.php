...
while($row=mysql_fetch_array($res))
{
	...
	$res1=mysql_query_decide($sql,$db_slave) or mysql_error_js();
	...
	if($flag)
	{
		...
		if($row['STATUS']=='BOUNCE')
		{
			...
			$res2=mysql_query_decide($sql,$db_slave) or mysql_error_js();
			...
			$res_allot=mysql_query_decide($sql_allot,$db_slave) or mysql_error_js();
			if($row_allot=mysql_fetch_array($res_allot))
			{
				$sql_e="SELECT EMAIL FROM jsadmin.PSWRDS WHERE USERNAME='$row_allot[ALLOTED_TO]'";
				$res_e=mysql_query_decide($sql_e,$db_slave) or mysql_error_js();
