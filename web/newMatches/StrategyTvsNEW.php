...
class StrategyTvsNEW extends StrategyClass
{
	private $receiverObj; 
	private $db;
	...
	public function doProcessing()
	{
		...
		$paramArr["logic_used"] = MailerConfigVariables::$strategyTvsNewLogic;
		...
		if(count($this->profileSet))
			...
			if($totalResultsCount>MailerConfigVariables::$maxLimitNewMatchesMails)
                        ...
                        $this->logRecordsNewMatchesMail($this->profileSet,$this->receiverObj->getProfileObj()->getPROFILEID(),$this->db,MailerConfigVariables::$strategyTvsNewLogic,$this->logicLevel,$is_more_link_required,$relaxCriteria);
                }
                else
                {
                        $gap=MailerConfigVariables::getNoOfDays();
                        $zeropid=$this->receiverObj->getProfileObj()->getPROFILEID();
                        $sql_y="INSERT INTO new_matches_emails.ZEROTvNEW(PROFILEID,DATE) VALUES($zeropid,$gap)";
