...
class SMSLib{
	var $path;
        var $expDate;
        var $dis_entry_dt;
	var $minSmsSendTime = 10;
	var $maxSmsSendTime = 20;
        var $incomeDetail=array();
        var $casteDetail=array();
        var $mstatusDetail=array();
        var $mtongueDetail=array();
        var $educationDetail=array();
...
//				$expiryDate = date('d-M', JSstrToTime($messageValue['FTO_EXPIRY_DATE']. ' + 1 day'));
			...
			case "NOSLIKEME":
				...
				if($this->symfony) $count = JsMemcache::getInstance()->get("SMS_MEM_LOOK_ME");
				else $count = memcache_call("SMS_MEM_LOOK_ME");
				return $count;
			...
			case "FRDDPP":
				...
				file_get_contents($url);
				include_once $this->path."/profile/connect_functions.inc";
                                if($this->symfony) $count = JsMemcache::getInstance()->get("SMS_MEM_LOOK");
                                else $count = memcache_call("SMS_MEM_LOOK");
...
include_once(JsConstants::$docRoot."/commonFiles/sms_inc.php");
	...
	function errormail($msg_sql,$error,$commentText='')
	{
...
//echo $msg."\n\n\n";
	...
	function bestMatchMail($count,$countSmsGenerated,$countBestMatchCalculated,$num)
	{
include_once(JsConstants::$docRoot."/commonFiles/comfunc.inc");
