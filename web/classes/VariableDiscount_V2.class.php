...
class VariableDiscount 
{
	...
	private $atsCoolOffPeriod 	=37;
		...
		// truncate table 
                ...
                mysql_query_decide($sqlTrc1,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sqlTrc1.mysql_error($this->myDb)));
		...
		foreach($tableArr as $key=>$val){
			if($val=='M')	
				...
				$sql ="SELECT PROFILEID from newjs.SEARCH_FEMALE";		
	       	        $res = mysql_query_decide($sql,$this->slaveDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sql.mysql_error($this->myDb)));
       		        while($row = mysql_fetch_array($res)){
       	       		        $pid 		=$row['PROFILEID'];
       	       	        	$sql1 		="insert ignore into billing.VARIABLE_DISCOUNT_POOL_TECH_V2(`PROFILEID`) VALUES('$pid')";
       	       	        	mysql_query_decide($sql1,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sql1.mysql_error($this->myDb)));
        		}
		}
	}
	...
	public function removeVdPoolProfiles($profileArr='')
	{
		...
		mysql_query_decide($sql,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sql.mysql_error($this->myDb)));
	}
	...
	public function fetchVdPoolProfiles()
	{
		...
		$res =mysql_query_decide($sql,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sql.mysql_error($this->myDb)));
		while($row = mysql_fetch_array($res)){
	...
	// Steps involved(2)
		...
		// Filter ever paid profile
                ...
                $resP =mysql_query_decide($sqlP,$this->slaveDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sqlP.mysql_error($this->slaveDb)));
			...
			return;
                ...
                $resAts =mysql_query_decide($sqlAts,$this->slaveDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sqlAts.mysql_error($this->slaveDb)));
        ...
        public function filterOtherCondition()
        {
                ...
                mysql_query_decide($sqlN,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sqlN.mysql_error($this->myDb)));
                ...
                mysql_query_decide($sqlF,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sqlF.mysql_error($this->myDb)));
        }
	...
	public function fetchClusterProfiles($profileid)
	{
		...
		$res =mysql_query_decide($sql,$this->slaveDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sql.mysql_error($this->slaveDb)));
		if($row = mysql_fetch_array($res)){
			$duplicateId =$row['DUPLICATE_ID'];
			$sql1 ="select PROFILEID from duplicates.DUPLICATE_PROFILES where DUPLICATE_ID='$duplicateId'";
			$res1 =mysql_query_decide($sql1,$this->slaveDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sql1.mysql_error($this->slaveDb)));
	...
	// insert variable discount in VARIABLE_DISCOUNT table 
		...
		$activationDt 		=$lastVdGivenDetails['ENTRY_DT'];
                ...
                mysql_query_decide($sqlTrc,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sqlTrc.mysql_error($this->myDb)));

                $sql1 ="select PROFILEID,DISCOUNT from billing.VARIABLE_DISCOUNT_POOL_TECH_V2";
                $res1 =mysql_query_decide($sql1,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sql1.mysql_error($this->myDb)));
                while($row1 =mysql_fetch_array($res1)){
			...
			mysql_query_decide($sqlIns,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sqlIns.mysql_error($this->myDb)));

			$sqlLog ="insert ignore into billing.VD_GIVEN_LASTTIME_V2(PROFILEID) VALUES('$profileid')";
			mysql_query_decide($sqlLog,$this->myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sqlLog.mysql_error($this->myDb)));
		}
	}
