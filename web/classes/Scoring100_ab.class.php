	...
	*/
	public function setAllVariables($profileid,$myDb,$shDb,$parameter="*",$ptype)
	{
		...
		if($ptype=='F' || $ptype=='R')
		{
			...
			$resl = mysql_query_decide($sqll,$myDb) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception($sqll.mysql_error($shDb)));
			if($rowl = mysql_fetch_array($resl)){
