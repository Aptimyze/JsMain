...
class CrawlerURL
{
	...
	public function CrawlerURL($siteId='',$do='',$community='')
	{
		if($siteId && $do)
		{
			...
			$res=$mysqlObj->executeQuery($sql,$db) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception("Error while fetching url   ".mysql_error()));
			unset($query);
			...
			$res=$mysqlObj->executeQuery($sql,$db) or LoggingWrapper::getInstance()->sendLog(LoggingEnums::LOG_ERROR, new Exception("Error while fetching url parameters   ".mysql_error()));
			if($mysqlObj->numRows($res))
			{
