<!DOCTYPE HTML PUBLIC "-//SoftQuad//DTD HoTMetaL PRO 4.0::19970714::extensions to HTML 4.0//EN"
 "hmpro4.dtd">

<HTML>
  
  <HEAD>
    <TITLE>BMS : Campaign Management System</TITLE>
    <LINK HREF="gifs/style.css" REL="stylesheet" TYPE="text/css">
<link rel="stylesheet" href="http://www.jeevansathi.com/jsadmin/jeevansathi.css" type="text/css"><link rel="stylesheet" href="http://www.jeevansathi.com/profile/images/styles.css" type="text/css">
  </HEAD>
  
  <BODY BGCOLOR="#FFFFFF" TEXT="#000000" LINK="#000000" VLINK="#000000" 
  ALINK="#000000" LEFTMARGIN="0" TOPMARGIN="0" MARGINWIDTH="0" MARGINHEIGHT="0">
   
  ~if $bmsheader`
 		~$bmsheader`
	~else`
   <TABLE WIDTH="100%" BORDER="0" VSPACE="0" HSPACE="0" 
    CELLPADDING="0" CELLSPACING="0">
      <TR>
        <TD><A HREF="http://www.naukri.com" TARGET="_blank"><IMG SRC="gifs/naukri-logo.gif" ALT="naukri.com" WIDTH="251" HEIGHT="81" BORDER="0" HSPACE="0" VSPACE="0" ALIGN="LEFT"></A></TD>
        <TD ALIGN="RIGHT"><IMG SRC="gifs/campaign-management.gif" ALT="Campaign Management" BORDER="0" HSPACE="0" WIDTH="337" HEIGHT="61"></TD>
      </TR>
    </TABLE><TABLE WIDTH="100%" BORDER="0" VSPACE="0" HSPACE="0" 
    CELLPADDING="0" CELLSPACING="0">
      <TR>
        <TD BACKGROUND="gifs/query-bg.gif"><A HREF="http://www.naukri.com/demosection/demo-index.htm" TARGET="_blank"><IMG SRC="gifs/recruitments-solutions.gif" ALT="naukri.com" BORDER="0" HSPACE="0" VSPACE="0" WIDTH="155" HEIGHT="23"></A><A HREF="http://www.naukri.com/ticket/admin/ticket_client_create.php?app_id=16" TARGET=""><IMG SRC="gifs/have-query.gif" ALT="naukri.com" BORDER="0" HSPACE="0" VSPACE="0" WIDTH="121" HEIGHT="23"></A><A HREF="faqs/"  TARGET=""><IMG SRC="gifs/faq-hm.gif" ALT="naukri.com" BORDER="0" HSPACE="0" VSPACE="0"></A></TD>
        <TD BACKGROUND="gifs/query-bg.gif" ALIGN="RIGHT">&nbsp;</TD>
      </TR>
    </TABLE>
    
    <P></P>
    <DIV CLASS="text"></DIV>
	~/if`
    <FORM><TABLE WIDTH="100%" BORDER="0" CELLPADDING="0" CELLSPACING="0" 
    ALIGN="CENTER">
      <TR>
        <TD WIDTH="18" BACKGROUND="gifs/page-bg.gif"><IMG SRC="gifs/lt-curve.gif" ALT="Resdex - naukri.com" WIDTH="18" HEIGHT="31" BORDER="0" HSPACE="0"></TD>
        <TD VALIGN="MIDDLE" HEIGHT="31" WIDTH="100%"><TABLE WIDTH="100%" BORDER="0" CELLPADDING="0" CELLSPACING="0" HEIGHT="31">
          <TR>
            <TD BACKGROUND="gifs/page-bg.gif" WIDTH="70%"><DIV CLASS="text"><IMG SRC="gifs/zero.gif" ALT="New Clients" BORDER="0" HSPACE="0" VSPACE="0" WIDTH="30" HEIGHT="1"></DIV></TD>
            <TD WIDTH="30%" BACKGROUND="gifs/page-bg.gif" ALIGN="RIGHT"><IMG SRC="gifs/zero.gif" ALT="New Clients" BORDER="0" HSPACE="0" VSPACE="0" WIDTH="30" HEIGHT="1"></TD>
          </TR>
        </TABLE></TD>
        <TD ALIGN="RIGHT" WIDTH="18" BACKGROUND="gifs/page-bg.gif"><IMG SRC="gifs/rt-curve.gif" ALT="Resdex - naukri.com" WIDTH="18" HEIGHT="31" BORDER="0"></TD>
      </TR>
    </TABLE><TABLE WIDTH="100%" BORDER="0" VSPACE="0" HSPACE="0" 
    CELLPADDING="0" CELLSPACING="0" ALIGN="CENTER">
      <TR>
        <TD WIDTH="18" BACKGROUND="gifs/lt-line-bg-1.gif"><IMG SRC="gifs/zero.gif" ALT="Account Information" BORDER="0" HSPACE="0" VSPACE="0" WIDTH="18" HEIGHT="1"></TD>
        <TD WIDTH="100%"> <BR><DIV ALIGN="CENTER"><TABLE WIDTH="70%" BORDER="0">
          <TR>
            <TD WIDTH="20%" ALIGN="RIGHT"><IMG SRC="gifs/error-icon.gif" ALT="Error " BORDER="0" HSPACE="0" VSPACE="0" WIDTH="38" HEIGHT="33"></TD>
            <TD WIDTH="80%" CLASS="red"><B>~if $errormsg` ~$errormsg` ~else` Site is down for maintanance. Please try after some time.~/if` </B></TD>
          </TR>
        </TABLE></DIV><BR><BR>
        <P></P>
        <P></P></TD>
        <TD BACKGROUND="gifs/rt-line-bg-1.gif" WIDTH="18"><IMG SRC="gifs/zero.gif" ALT="Account Information" BORDER="0" HSPACE="0" VSPACE="0" WIDTH="18" HEIGHT="1"></TD>
      </TR>
    </TABLE><TABLE WIDTH="100%" BORDER="0" CELLPADDING="0" CELLSPACING="0" 
    ALIGN="CENTER">
      <TR>
        <TD WIDTH="18" BACKGROUND="gifs/page-bg-1.gif"><IMG SRC="gifs/lt-low-curve.gif" ALT="Resdex - naukri.com" WIDTH="18" HEIGHT="31" BORDER="0"></TD>
        <TD VALIGN="MIDDLE" HEIGHT="31" BACKGROUND="gifs/page-bg-1.gif">&nbsp;</TD>
        <TD ALIGN="RIGHT" WIDTH="18" BACKGROUND="gifs/page-bg-1.gif"><IMG SRC="gifs/lt-rt-curve.gif" ALT="Resdex - naukri.com" WIDTH="18" HEIGHT="31" BORDER="0"></TD>
      </TR>
    </TABLE></FORM>
    
   ~$bmsfooter`
  </BODY>
</HTML>
