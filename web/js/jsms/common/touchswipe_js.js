(function($){
	$.fn.Slider = function(options){
		...
		var initTouch=function()
		{
			...
			slider.parent.unbind('touchstart');
		...
		var onTouchStart=function(e)
		{
			...
			if(slider.working){
				...
				slider.parent.unbind('touchmove');
				slider.parent.bind('touchmove', onTouchMove);
				// bind a "touchend" event to the viewport
				slider.parent.unbind('touchend');
		...
		var onTouchMove=function(e)
		{
			...
			//Issue with andorid browser, touchend won't work until preventDefault is called
		...
		el.NextSlide=function()
		{
			...
			setSliderLocation(slider.index,index);	
			...
			el.css(slider.animProp, propValue);
			slider.index=parseInt(index);
			sliderCurrentPage=slider.index;
			FixHeight();
			//Album fix for android native
			AlbumOverlayAddRemove(index);
		}
		...
		el.gotoSlide=function(index,updateHistory)
		{
			...
			{
				slider.index=parseInt(slider.maxindex);
					if(index<0)
						slider.index=0;		
					return;
			}	
	...
	function setSliderLocation(index,curindex)
	{
		if($('div[index="'+curindex+'"]').length)
			var curindex=$('div[index="'+curindex+'"]').attr("id").replace("slidername","");
		else
			var curindex=0;
		if(typeof(index)!='undefined')
			historyStoreObj.push(function(){
				
				bxslider.gotoSlide(index,-1);
				return true;
				},"#"+curindex);
	}
	function AlbumOverlayAddRemove(index)
	{
