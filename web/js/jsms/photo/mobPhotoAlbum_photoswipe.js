...
(function(window, Util, FullSizeImageClass){
	...
	Code.PhotoSwipe.SliderItemClass = Code.PhotoSwipe.EventClass.extend({
		...
		init: function(parentEl){
			...
			Util.DOM.setAttribute(this.parentEl, "id","inverted-contain");
			if(!window.is_android)
				Util.DOM.resetTranslate(this.el);
                        //Util.DOM.addClass(this.el, "parent");
			...
			Util.DOM.setStyle(this.imageContainerEl, {
				...
				left: 0
                    ...
                    Util.DOM.setAttribute(this.imageContainerEl, "class","panzoom");
			...
			Util.DOM.hide(this.imageEl);
                        Util.DOM.setAttribute(this.imageEl, "class","panzoom-elements");
	...
	Code.PhotoSwipe.SliderClass = Code.PhotoSwipe.ElementClass.extend({
		...
		showNext: function(){
			...
			var slideBy = this.parentElWidth * -1;
				...
				Util.Animation.slideBy(this.el, slideBy, 1, this.settings.slideSpeed, this.showNextEndEventHandler);
...
(function(window, Util){
	...
	Code.PhotoSwipe.CaptionClass = Code.PhotoSwipe.ElementClass.extend({
		...
		init: function(options){
			...
			this.backButton = Util.DOM.createElement('div', { 'class': 'posabs pu_pos4' }, '<i class="up_sprite puback"></i>');
			Util.DOM.appendChild(this.backButton, this.el);
			if(purposeOfAlbumView=="OwnAlbum" || purposeOfAlbumView=="OthersAlbum"){
				var IMAGE_TOP_RIGHT = "pudelete";
				}
			else if(purposeOfAlbumView=="setProfilePic"){
				var IMAGE_TOP_RIGHT = "pu_blckplus";
				
				this.bottomProfilePicButton = Util.DOM.createElement('div', { 'class': 'posfix btmo fullwid bg17 color21 lh30 fullwid dispbl txtc lh50' }, 'Set this as profile photo');
				Util.DOM.appendChild(this.bottomProfilePicButton, this.el);
			}
			this.rightSideButton = Util.DOM.createElement('div', { 'class': 'posabs pu_pos5' }, '<i class="up_sprite '+IMAGE_TOP_RIGHT+'"></i>');
			Util.DOM.appendChild(this.rightSideButton, this.el);
		...
		setCaptionValue: function(captionValue){
			var array = captionValue.split(',');
			
			if(!albumCaptionNow)
				albumCaptionNow=1;
                        $(".ps-caption-content").hide().show().css("visibility","visible");
			captionValue=albumCaptionNow+"/"+albumCaptionTotal;
			window.ps_picNow=array[1];
			...
			if (Util.isObject(captionValue)){
				...
				if(purposeOfAlbumView=="setProfilePic"){
					Util.DOM.setAttribute(this.rightSideButton,"onclick","addMorePic()");	
					if(array[1] && array[1]!=""  && albumCaptionNow==1){
						this.bottomProfilePicButton.innerText="This is your profile photo";
						Util.DOM.setAttribute(this.bottomProfilePicButton,"class","posfix btmo fullwid bg17 color21 lh30 fullwid dispbl txtc lh50");
						Util.DOM.setAttribute(this.bottomProfilePicButton,"onclick","");
					}
					else if(array[1] && array[1]!="") {
						this.bottomProfilePicButton.innerText="Set this as profile photo";
						Util.DOM.setAttribute(this.bottomProfilePicButton,"onclick","setProfilePic('"+array[1]+"')");
						Util.DOM.setAttribute(this.bottomProfilePicButton,"class","posfix btmo fullwid bg7 white lh30 fullwid dispbl txtc lh50");
					}
					if(array[2] && array[2]!=""){
						Util.DOM.setAttribute(this.backButton,"onclick","goBack('"+array[2]+"')");		
					}
					else{
						Util.DOM.removeChildren(this.backButton);
					}
				}
				else{
					if(array[1] && array[1]!="" && ((albumCaptionNow==albumCaptionTotal) || (albumCaptionNow>1))){
						Util.DOM.setStyle(this.rightSideButton, 'display', 'block');
						Util.DOM.setAttribute(this.rightSideButton,"onclick","deletePic('"+array[1]+"')");	
					}
					else{
						Util.DOM.setStyle(this.rightSideButton, 'display', 'none');
					}
			...
			$("#inverted-contain").css("top","0px");
                       $(".ps-caption-content").delay(3000)
                            .queue(function(){
...
(function(window, Util, ElementClass, DocumentOverlayClass, FullSizeImageClass, ViewportClass, SliderClass, CaptionClass, ToolbarClass, CaptionToolbarClass, ZoomPanRotateClass){
	...
	var photoSwipe = Code.PhotoSwipe.EventClass.extend({
		...
		setImages: function(thumbEls){
			...
			this.fullSizeImages = [];
			albumCaptionTotal=thumbEls.length;
			for (var i=0; i<thumbEls.length; i++){
				...
				imageArray=this.fullSizeImages;
				thisSlideElement=this;
		...
		setSliderPreviousAndNextFullSizeImages: function(){
			...
			if(imageArray)
				this.fullSizeImages = imageArray;
		...
		onSliderDisplayCurrentFullSizeImage: function(e){
			...
			if(albumCaptionNow)
				this.currentIndex = albumCaptionNow-1;
			this.fullSizeImages = imageArray;
	...
	Code.PhotoSwipe.GetImageMetaData = function(el){
		...
		for (i=0; i<el.childNodes.length; i++){
			childEl = el.childNodes[i];
			if (el.childNodes[i].nodeName === 'IMG'){
				var array = Util.DOM.getAttribute(childEl, 'alt').split(',');
				return array; 
			}
		}
	};
