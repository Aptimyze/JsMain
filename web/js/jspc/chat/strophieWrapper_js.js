var strophieWrapper = {
    ...
    updatePresence: function(user_id, chat_status) {
        ...
        if (chat_status == "online") updatedObj["last_online_time"] = new Date();
    ...
    formatRosterObj: function(obj) {
        ...
        var chat_status = obj["attributes"]["chat_status"] || "offline",
            newObj = {};
        var fullname = "";
        if (typeof obj["attributes"]["name"] != "undefined") fullname = obj["attributes"]["name"].split("|");
        ...
        if (typeof obj["group"] != "undefined") newObj[strophieWrapper.rosterDetailsKey]["groups"].push(obj["group"]["#text"]);
        return newObj;
    },
    ...
    formatMsgObj: function(msg) {
        ...
        if ($message.find(strophieWrapper.msgStates["COMPOSING"]).length != 0) {
            ...
            msg_state = "paused";
        } else if ($message.find(strophieWrapper.msgStates["GONE"]).length != 0) msg_state = strophieWrapper.msgStates["GONE"];
        else if ($message.find(strophieWrapper.msgStates["ACTIVE"]).length != 0) msg_state = strophieWrapper.msgStates["ACTIVE"];
        else if ($message.find(strophieWrapper.msgStates["INACTIVE"]).length != 0) msg_state = strophieWrapper.msgStates["INACTIVE"];
        else if ($message.find(strophieWrapper.msgStates["RECEIVER_RECEIVED_READ"]).length != 0) msg_state = strophieWrapper.msgStates["SENDER_RECEIVED_READ"];
        else if ($message.find(strophieWrapper.msgStates["RECEIVED"]).length != 0) msg_state = strophieWrapper.msgStates["RECEIVED"];
