	...
	jRange.prototype = {
		defaults: {
			onstatechange: function() {
                            var name = this.inputNode.context.name;
                            var value = this.inputNode.context.value;
                            var valueBreak = value.split(",");
                            if(name=="INR")
                                var tag="L";
                            else
                                var tag="K$";
                            $("#min"+name).val(valueBreak[0]+tag);
                            if(valueBreak[1]==100 && name=="INR")
                                var valueMax="1Cr+";
                            else
                                var valueMax=valueBreak[1]+tag;
                            $("#max"+name).val(valueMax);
                        },
			isRange: false,
			showLabels: true,
			showScale: true,
