	...
	*/	
	...
	function duplicatePhoneHander($phone_num,$phone_type)
	{
		...
		if($phone_type =='M')
		{
			...
			if($phone){
				$sql ="SELECT `PROFILEID`,`MOB_STATUS`,`LAST_LOGIN_DT` from newjs.JPROFILE WHERE `PHONE_MOB` IN ('$phone','0$phone') AND DATE(LAST_LOGIN_DT) >='$date_1year' AND ACTIVATED IN('H','Y') AND activatedKey=1";
				$execute =1;
			}
		}
		else if($phone_type =='L')
		{
			...
			if($cnt_no >5){
				$sql ="SELECT `PROFILEID`,`LANDL_STATUS`,`LAST_LOGIN_DT` from newjs.JPROFILE WHERE `PHONE_WITH_STD` IN ('$landline_std','0$landline_std') AND DATE(LAST_LOGIN_DT) >='$date_1year' AND ACTIVATED IN('H','Y') AND activatedKey=1";
				$execute =1;
			}
